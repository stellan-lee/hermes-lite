...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using MARLOW_HOME."""
    try:
        from marlow_constants import get_marlow_home
        return get_marlow_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("MARLOW_HOME", Path.home() / ".marlow")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from marlow_constants import get_marlow_home
        directory_file = get_marlow_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("MARLOW_HOME", Path.home() / ".marlow")
...
class EventBridge:
    ...
    This is the Marlow equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from marlow_constants import get_marlow_home
            db_file = get_marlow_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("MARLOW_HOME", Path.home() / ".marlow")) / "state.db"
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Marlow MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
