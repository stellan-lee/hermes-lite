...
from marlow_constants import secure_parent_dir
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if isinstance(providers, dict) and normalized in providers:
            return normalized
        raise AuthError(
            f"Unknown provider '{normalized}'. Run 'marlow model' to configure Codex or a compatible endpoint.",
    ...
    if os.getenv("LM_BASE_URL", "").strip() or os.getenv("LM_API_KEY", "").strip():
        return "lmstudio"
    raise AuthError(
        "No inference provider configured. Run 'marlow model' to choose Codex or configure a compatible endpoint.",
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Marlow's Codex token store."""
    data = _read_codex_tokens()

    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(
        os.getenv("MARLOW_CODEX_REFRESH_TIMEOUT_SECONDS", "20")
    )
    ...
    *,
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print(
                "Marlow will create its own session to avoid conflicts with Codex CLI / VS Code."
            )
            try:
                do_import = (
                    input(
