...
def _print_setup_summary(config: dict, marlow_home):
    ...
    # Video generation — opt-in via `marlow tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from marlow_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'marlow setup terminal'"))
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``marlow model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.marlow/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Marlow auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via marlow setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Marlow delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Marlow to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from marlow_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``marlow_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from marlow_cli.config
    ...
    if section_key == "model":
        ...
        from marlow_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_marlow.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_marlow migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_marlow", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Marlow.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Marlow to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Marlow at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Marlow at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Marlow at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Marlow at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Marlow equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Marlow config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Marlow.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Marlow's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(marlow_home: Path) -> bool:
    ...
    print_info("Marlow can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: marlow claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=marlow_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: marlow claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Marlow configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Marlow config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Marlow (use marlow claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``marlow setup --portal``. Logs into a Nous Portal subscription
    ...
    provider — pick your model/provider separately with ``marlow model``.
    """
    from types import SimpleNamespace

    from marlow_cli.auth_commands import auth_add_command
    from marlow_cli.auth import get_nous_auth_status
    from marlow_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    print(color("│     ⚕ Marlow Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `marlow setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `marlow auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `marlow auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # It does NOT change the model provider — use `marlow model` for that.
    print()
    try:
    ...
    print_info("  Run `marlow portal status` to inspect routing.")
    print_info("  Run `marlow` to start chatting.")
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        migration_ran = _offer_openclaw_migration(marlow_home)
        if migration_ran:
            config = load_config()
