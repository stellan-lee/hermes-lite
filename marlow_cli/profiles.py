"""
Profile management for multiple isolated Marlow instances.

Each profile is a fully independent MARLOW_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.marlow/profiles/<name>/`` by default.

The "default" profile is ``~/.marlow`` itself — backward compatible,
zero migration needed.

Usage::

    marlow profile create coder          # fresh profile + bundled skills
    marlow profile create coder --clone  # also copy config, .env, SOUL.md, skills
    marlow profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    marlow -p coder chat                 # or via flag
    marlow profile use coder             # set as sticky default
    marlow profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See marlow_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.marlow``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   marlow-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Marlow infrastructure directories that only the default profile
         (``~/.marlow``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_marlow_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.marlow) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "marlow-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "marlow", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the marlow root, NOT to the current MARLOW_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where MARLOW_HOME points outside
    ``~/.marlow``, profiles live under ``MARLOW_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_marlow_home() / "profiles"


def _get_default_marlow_home() -> Path:
    """Return the default (pre-profile) MARLOW_HOME path.

    In standard deployments this is ``~/.marlow``.
    In Docker/custom deployments where MARLOW_HOME is outside ``~/.marlow``
    (e.g. ``/opt/data``), returns MARLOW_HOME directly.
    """
    from marlow_constants import get_default_marlow_root
    return get_default_marlow_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_marlow_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Marlow installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its MARLOW_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_marlow_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, marlow subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _MARLOW_SUBCOMMANDS:
        return f"'{canon}' conflicts with a marlow subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``marlow profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Marlow config
# (~5000 lines of defaults); ``profile.yaml`` is metadata ABOUT the
# profile itself (its role, who described it). Mixing them makes both
# harder to read.
#
# Missing file -> empty defaults; never an error. Callers tolerate empty
# descriptions and fall back to the profile name.
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_marlow_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``marlow update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.marlow)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.marlow/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `marlow update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`marlow profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `marlow profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `marlow -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches MARLOW_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``marlow profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MARLOW_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.marlow).\n"
            "To remove everything, use: marlow uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from marlow_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set MARLOW_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("MARLOW_HOME")
    try:
        os.environ["MARLOW_HOME"] = str(profile_dir)
        from marlow_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["MARLOW_HOME"] = old_home
        elif "MARLOW_HOME" in os.environ:
            del os.environ["MARLOW_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.marlow/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: marlow profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from MARLOW_HOME.

    Returns ``"default"`` if MARLOW_HOME is not set or points to ``~/.marlow``.
    Returns the profile name if MARLOW_HOME points into ``~/.marlow/profiles/<name>``.
    Returns ``"custom"`` if MARLOW_HOME is set to an unrecognized path.
    """
    from marlow_constants import get_marlow_home
    marlow_home = get_marlow_home()
    resolved = marlow_home.resolve()

    default_resolved = _get_default_marlow_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.marlow itself — its parent is ~/ and its
        # directory name is ".marlow", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: marlow profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.marlow itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.marlow). "
            "Specify a different name: marlow profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="marlow_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"marlow_{old_name}"
    legacy_old_host = f"marlow.{old_name}"
    new_host = f"marlow_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_marlow_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("marlow_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MARLOW_HOME path string.

    Called early in the CLI entry point, before any marlow modules
    are imported, to set the MARLOW_HOME environment variable.
    """
