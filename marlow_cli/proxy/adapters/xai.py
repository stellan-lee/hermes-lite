...
# xAI's public API is OpenAI-compatible for the endpoints Marlow commonly
# uses. The Responses endpoint is included because Marlow' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Marlow-managed OAuth credentials."""

    auth_hint = "marlow auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`marlow auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`marlow auth reset xai-oauth` or re-authenticate with "
                    "`marlow auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `marlow auth add xai-oauth --type oauth`."
            )
