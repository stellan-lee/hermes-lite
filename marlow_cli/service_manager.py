...
the existing module-level functions in marlow_cli.gateway directly. This protocol is a thin facade
...
# in ``marlow_cli.gateway`` (systemd/launchd). The protocol's ``name`` parameter is currently
...
# runs inside the container (Phase 4). Static services (main-marlow)
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-marlow)
