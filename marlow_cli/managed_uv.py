...
Marlow owns its own uv binary at ``$MARLOW_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$MARLOW_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Marlow keeps *its* uv binary.

    ``$MARLOW_HOME/bin/uv`` on POSIX, ``$MARLOW_HOME\\bin\\uv.exe`` on
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``marlow update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$MARLOW_HOME/bin/`` instead of ``~/.local/bin/``.
    """
