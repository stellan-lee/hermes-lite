...
Profile-aware location: ``$MARLOW_HOME/logs/dashboard-auth.log``.
...
from ``marlow_constants`` or other marlow_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$MARLOW_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``marlow_constants.get_marlow_home`` semantics: env var wins,
    else ``~/.marlow``. A local copy avoids an import cycle with the
    middleware which lives below ``marlow_cli``.
    """
    home = os.environ.get("MARLOW_HOME") or str(Path.home() / ".marlow")
