...
Pure display functions with no MarlowCLI state dependency.
"""
...
from marlow_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _fetch_pypi_latest(package: str = "marlow-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    cache_file = marlow_home / ".update_check"
    embedded_rev = os.environ.get("MARLOW_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $MARLOW_HOME/marlow-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = marlow_home / "marlow-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Marlow git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$MARLOW_HOME/marlow-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``marlow_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from marlow_cli.build_info import get_build_sha
...
def format_banner_version_label() -> str:
    ...
    base = f"Marlow Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else MARLOW_CADUCEUS
    except Exception:
        _bskin = None
        _hero = MARLOW_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from marlow_cli.codex_runtime_switch import get_current_runtime
        from marlow_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from marlow_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    # Pip-install warning — `pip install marlow-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else MARLOW_AGENT_LOGO
        console.print(_logo)
        console.print()
