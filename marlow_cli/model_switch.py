...
- ``marlow_cli.providers``        -- canonical provider identity + overlays
- ``marlow_cli.model_normalize``  -- per-provider name formatting
...
_MARLOW_MODEL_WARNING = (
    "Nous Research Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with Marlow Agent. They lack the tool-calling capabilities "
...
def _check_marlow_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_hermes_non_agentic(model_name):
        return _MARLOW_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``marlow config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from marlow_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'marlow model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'marlow doctor' — config issues detected:"
        ...
        from marlow_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from marlow_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from marlow_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in marlow_cli.runtime_provider.resolve_runtime_provider;
    ...
    marlow_warn = _check_marlow_model_warning(new_model)
    if marlow_warn:
        warnings.append(marlow_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from marlow_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from marlow_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by marlow provider ID
    ...
    # --- 1. Check Marlow-mapped providers ---
    from marlow_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from marlow_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for marlow_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(marlow_id)
        if (
            _alias_target
            and _alias_target != marlow_id
        ...
        pconfig = PROVIDER_REGISTRY.get(marlow_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (MARLOW_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from marlow_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(marlow_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `marlow model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(marlow_id)
        if not model_ids:
            model_ids = curated.get(marlow_id, [])
            if marlow_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(marlow_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Marlow-only providers (openai-codex, copilot, opencode-go) ---
    from marlow_cli.providers import MARLOW_OVERLAYS
    from marlow_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Marlow provider ID.
    # MARLOW_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Marlow IDs ("copilot").
    _mdev_to_marlow = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in MARLOW_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Marlow slug — e.g. "github-copilot" → "copilot"
        marlow_slug = _mdev_to_marlow.get(pid, pid)
        if marlow_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(marlow_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, marlow_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or marlow_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(marlow_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", marlow_slug, exc)
        ...
        # configured.
        if not has_creds and marlow_slug == "anthropic":
            try:
                ...
                if (marlow_creds and marlow_creds.get("accessToken")) or \
        ...
        if marlow_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(marlow_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(marlow_slug)
                model_ids = _ids if _ids else (curated.get(marlow_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(marlow_slug, []) or curated.get(pid, [])
        else:
            ...
            if not model_ids:
                model_ids = curated.get(marlow_slug, []) or curated.get(pid, [])
                if marlow_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(marlow_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": marlow_slug,
            "name": get_label(marlow_slug),
            "is_current": marlow_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "marlow",
        ...
        seen_slugs.add(marlow_slug.lower())
        _record_builtin_endpoint(marlow_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or MARLOW_OVERLAYS (keeps /model in sync
    # with `marlow model`).
    try:
        from marlow_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Marlow's canonical write key (matches
            ...
            # Marlow writes ``models:`` as a dict keyed by model id
            # (see marlow_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Marlow's own writer uses; a hyphen variant is accepted
            ...
            # active model. Marlow's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, marlow_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`marlow_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from marlow_cli.models import fetch_openrouter_models
