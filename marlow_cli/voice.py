...
#
# ``super`` is intentionally absent: prompt_toolkit
...
# lookup, and marlow-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` → ``c-b`` (TUI-only modifier —
    ...
    # in prompt_toolkit (a-c-r form) and marlow-ink rejects them; collapse
    ...
    # ``super`` is TUI-only (prompt_toolkit has
...
def format_voice_record_key_for_status(raw: Any) -> str:
    ...
    if normalized.startswith("c-"):
        ...
        # ``super+<key>`` — CLI won't bind it, but
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when MARLOW_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("MARLOW_VOICE_DEBUG", "").strip() != "1":
        return
    try:
