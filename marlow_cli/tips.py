...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or marlow -c.",
    ...
    "Ctrl+Z suspends Marlow to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "marlow -c resumes your most recent CLI session. marlow -c \"project name\" resumes by title.",
    "marlow -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "marlow -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "marlow chat -t web,terminal enables only specific toolsets for a focused session.",
    "marlow chat -s github-pr-workflow preloads a skill at launch.",
    "marlow chat -q \"query\" runs a single non-interactive query and exits.",
    "marlow chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "marlow chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "marlow --yolo bypasses all dangerous command approval prompts for the entire session.",
    "marlow chat --source telegram tags the session for filtering in marlow sessions list.",
    "marlow -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "marlow doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "marlow dump outputs a compact setup summary — great for bug reports.",
    "marlow config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "marlow config edit opens config.yaml in your default editor.",
    "marlow config check scans for missing or stale configuration options.",
    "marlow sessions browse opens an interactive session picker with search.",
    "marlow sessions stats shows session counts by platform and database size.",
    "marlow sessions prune --older-than 30 cleans up old sessions.",
    "marlow skills search react --source skills-sh searches the skills.sh public directory.",
    "marlow skills check scans installed hub skills for upstream updates.",
    "marlow skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "marlow skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "marlow mcp add github --command npx adds MCP servers from the command line.",
    "marlow mcp serve runs Marlow itself as an MCP server for other agents.",
    "marlow auth add lets you add multiple API keys for credential pool rotation.",
    "marlow completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "marlow logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "marlow backup creates a zip backup of your entire Marlow home directory.",
    "marlow profile create coder creates an isolated profile that becomes its own command.",
    "marlow profile create work --clone copies your current config and keys to a new profile.",
    "marlow update syncs new bundled skills to ALL profiles automatically.",
    "marlow gateway install sets up Marlow as a system service (systemd/launchd).",
    "marlow memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "marlow webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: marlow tools disables unused tools, marlow skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "marlow models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as marlow -w).",
    ...
    "MARLOW_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Marlow tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'marlow profile create coder' creates the 'coder' command.",
    "marlow profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Marlow prints a resume command with session ID and stats.",
    "marlow sessions export backup.jsonl exports all sessions for backup or analysis.",
    "marlow -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "marlow skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .marlow/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: marlow cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Marlow runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "marlow gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Marlow via a local macOS server.",
    ...
    "Context files (.marlow.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Marlow loads project context from .marlow.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "marlow mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "marlow mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "marlow mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — marlow tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.marlow/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "marlow chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "marlow plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "marlow acp runs Marlow as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "MARLOW_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "marlow auth add nous or marlow auth add openai-codex sets up OAuth-based providers.",
    ...
    "marlow status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.marlow/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Marlow your own.",
    ...
    "Before a gateway session reset, Marlow auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.marlow/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at MARLOW_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "MARLOW_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in MARLOW_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.marlow/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'marlow profile create ls' would be rejected.",
    "marlow profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "marlow claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "MARLOW_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via marlow tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.marlow/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.marlow/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Marlow auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Marlow config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Marlow is working.',
    ...
    '/reload reloads ~/.marlow/.env into the running session — pick up new API keys without restarting.',
    ...
    "MARLOW_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.marlow/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.marlow/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'marlow curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "marlow curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'marlow curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'marlow auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'MARLOW_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "MARLOW_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'marlow dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'marlow dashboard --tui embeds the full Marlow TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.marlow/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.marlow/dashboard-plugins/ — no npm build required.',
    ...
    'MARLOW_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'MARLOW_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'MARLOW_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'MARLOW_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.marlow/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.marlow/skills/ so drop-in skills appear without restarting the session.',
    ...
    'marlow -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'marlow chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'marlow chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'marlow chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "marlow chat --source tool tags programmatic chats so they don't clutter marlow sessions list.",
    'marlow dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'marlow sessions rename <ID> "new title" renames any past session; marlow sessions delete <ID> removes one.',
    'marlow import restores a session export or profile archive produced by sessions export or profile export.',
    'marlow fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'marlow pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'marlow setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'marlow status --deep runs the full health sweep across every component; plain marlow status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'MARLOW_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'MARLOW_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.marlow/plugins/ — trust-gated by design.',
    "MARLOW_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'MARLOW_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'MARLOW_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'MARLOW_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'MARLOW_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'MARLOW_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'MARLOW_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'MARLOW_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'MARLOW_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'MARLOW_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'MARLOW_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Marlow block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from marlow sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.marlow/dashboard-plugins/.',
]
