"""``marlow journey`` — what Marlow has learned, on a timeline.
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if not payload.get("nodes"):
        console.print(
            "[grey62]No learning yet — use Marlow a while and your learned skills and "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_del.add_argument("node", help="Node id (skill name or memory:<source>:<hash>; see `journey list`).")
    ...
    p_edit.add_argument("node", help="Node id (skill name or memory:<source>:<hash>; see `journey list`).")
...
if __name__ == "__main__":
    _p = argparse.ArgumentParser(prog="marlow journey")
    register_cli(_p)
    _a = _p.parse_args()
    sys.exit(_a.func(_a))
