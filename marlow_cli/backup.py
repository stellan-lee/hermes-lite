"""
Backup and import commands for marlow CLI.

`marlow backup` creates a zip archive of the entire ~/.marlow/ directory
(excluding the marlow-agent repo and transient files).

`marlow import` restores from a backup zip, overlaying onto the current
MARLOW_HOME root.
"""
...
from marlow_constants import get_default_marlow_root, get_marlow_home, display_marlow_home
...
_EXCLUDED_DIRS = {
    "marlow-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to marlow root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside MARLOW_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Marlow home directory."""
    marlow_root = get_default_marlow_root()

    if not marlow_root.is_dir():
        print(f"Error: Marlow home directory not found at {marlow_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"marlow-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"marlow-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_marlow_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(marlow_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(marlow_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(marlow_root)
    ...
    print(f"\nRestore with: marlow import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Marlow backup.
    ...
    # Look for telltale files that a marlow home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Marlow backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.marlow/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a marlow dir name
        if prefix in {".marlow", "marlow"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_marlow_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (marlow_root / "config.yaml").exists()
        has_env = (marlow_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Marlow configuration.")
        ...
        for member in members:
            ...
            target = marlow_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(marlow_root.resolve())
            except ValueError:
        ...
        profiles_dir = marlow_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # marlow_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: marlow profile list  (after installing marlow)")

        # Guidance
        print()
        if not (marlow_root / "marlow-agent").is_dir():
            print("Note: The marlow-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: marlow update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  marlow -p {pname} gateway install")

        print("Done. Your Marlow configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and marlow backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to MARLOW_HOME).
...
# — `marlow update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(marlow_home: Optional[Path] = None) -> Path:
    home = marlow_home or get_marlow_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    marlow_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(marlow_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside MARLOW_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``marlow update``.
    ...
    Args:
        ...
        marlow_home: Override for the Marlow home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(marlow_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for marlow backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_marlow_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, marlow_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``marlow_root`` to ``out_path``.
...
def _pre_update_backup_dir(marlow_home: Optional[Path] = None) -> Path:
    home = marlow_home or get_marlow_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of MARLOW_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<MARLOW_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``marlow update``) should continue even if the backup fails.
    """
    marlow_root = marlow_home or get_default_marlow_root()
    if not marlow_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(marlow_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, marlow_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `marlow claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of MARLOW_HOME under ``backups/`` before a
    ``marlow claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``marlow import <archive>``.  Writes to
    ``<MARLOW_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `marlow import` and the
