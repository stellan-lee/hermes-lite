...
# $MARLOW_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $MARLOW_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Marlow refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Marlow that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Marlow that doesn't ship
...
# this list:  marlow model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `marlow model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Marlow provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `marlow model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``marlow model``, the
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    marlow-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from marlow_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — marlow-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    ``marlow_cli/data/model-catalog.json`` snapshot as the offline fallback);
    ...
    """
    try:
        from marlow_cli.model_catalog import get_curated_nous_models
...
def _resolve_static_model_alias(
    ...
    try:
        from marlow_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Marlow should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``marlow auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.marlow/.env``.
    ...
    """
    try:
        from marlow_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from marlow_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Marlow release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Marlow release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from marlow_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Marlow release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``marlow model`` shows.
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$MARLOW_HOME/auth.json`` and external
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Marlow' default Claude IDs elsewhere use
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from marlow_constants import get_marlow_home
    return get_marlow_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Marlow will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Marlow cannot verify the model name."
