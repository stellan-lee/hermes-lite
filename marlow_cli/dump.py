"""
Dump command for marlow CLI.

Outputs a compact, plain-text summary of the user's Marlow setup
...
from marlow_cli.config import get_marlow_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.marlow_build_sha`` by the Dockerfile's
    ``MARLOW_GIT_SHA`` build-arg (see ``marlow_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``marlow dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from marlow_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(marlow_home: Path) -> int:
...
def _cron_summary(marlow_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = marlow_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"marlow_home:      {display_marlow_home()}")
    ...
    api_keys = [
        ("OPENAI_API_KEY", "openai_media_or_custom"),
    ...
    toolsets = config.get("toolsets", ["marlow-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(marlow_home)}")
    lines.append(f"  skills:             {_count_skills(marlow_home)}")
