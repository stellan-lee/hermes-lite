...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_marlow_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:marlow_pkce",
    ...
    if provider == "nous":
        ...
        # <marlow-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `marlow --profile <name>
    ...
    raise SystemExit(f"`marlow auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Marlow reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `marlow auth status openai-codex`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `marlow auth` is called bare."""
