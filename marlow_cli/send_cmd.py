...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"marlow send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"marlow send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"marlow send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"marlow send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"marlow send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `marlow gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.marlow/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_marlow_env() -> None:
    """Populate ``os.environ`` from ``~/.marlow/.env`` AND bridge top-level
    ...
    startup that ``marlow send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.marlow/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.marlow/config.yaml`` into
       ...
       via ``marlow config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``marlow send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.marlow/.env and ~/.marlow/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "marlow send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  marlow send --to telegram \"hello\"\n"
            "  marlow send --to discord:#ops --file report.md\n"
            "  marlow send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "marlow send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `marlow send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Marlow "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.marlow/.env + ~/.marlow/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  marlow send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | marlow send --to telegram:-1001234567890\n"
            "  marlow send --to discord:#ops --file /tmp/report.md\n"
            "  marlow send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  marlow send --list                  # all platforms\n"
            "  marlow send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `marlow send --list telegram`.",
    )
