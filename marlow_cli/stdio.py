...
on Japanese locales, etc.).  Marlow's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``marlow_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Marlow is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``MARLOW_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("MARLOW_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Marlow-managed Git install directories so
    ...
    # already-running shells don't see the change — which means marlow
    ...
    # install without Marlow-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Marlow's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Marlow's
    ``marlow config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Marlow (User env var in Windows Settings, or
    export in a PowerShell profile) and Marlow picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Marlow-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\marlow\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``marlow`` from right after install) retain their old PATH.

    Any subprocess Marlow spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Marlow-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "marlow", "git", "cmd"),
        os.path.join(local_appdata, "marlow", "git", "bin"),
        os.path.join(local_appdata, "marlow", "git", "usr", "bin"),
        # Marlow venv Scripts directory — host of the marlow.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes marlow, but harmless to include.
        os.path.join(local_appdata, "marlow", "marlow-agent", "venv", "Scripts"),
