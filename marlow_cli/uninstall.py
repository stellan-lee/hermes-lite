"""
Marlow Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.marlow/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Marlow PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing marlow-agent or marlow PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Marlow Agent" comment and following line
                if '# Marlow Agent' in line or '# marlow-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('marlow' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing marlow
                if 'marlow' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the marlow wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "marlow",
        Path("/usr/local/bin/marlow"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains marlow_cli reference)
                content = wrapper.read_text()
                if 'marlow_cli' in content or 'marlow-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(marlow_home: Path) -> list:
    ...
    creates::
        ...
        ~/.local/bin/node -> $MARLOW_HOME/node/bin/node
        ~/.local/bin/npm  -> $MARLOW_HOME/node/bin/npm
        ~/.local/bin/npx  -> $MARLOW_HOME/node/bin/npx
    ...
    when the link still resolves into this Marlow home's ``node`` directory.
    A link the user has since repointed at nvm (or anything else outside
    Marlow) is left untouched so we never break unrelated tooling.
    """
    node_dir = (marlow_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``marlow gateway run`` processes
    ...
    try:
        from marlow_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from marlow_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``MARLOW_HOME`` and ``MARLOW_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\marlow\git\cmd``, ``%LOCALAPPDATA%\marlow\git\bin``,
#      ``%LOCALAPPDATA%\marlow\git\usr\bin``, ``%LOCALAPPDATA%\marlow\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\marlow\git\`` and Node to
#      ``%LOCALAPPDATA%\marlow\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``marlow dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\marlow\gateway-service\`` and sometimes
...
def _marlow_path_markers(marlow_home: Path) -> list[str]:
    """Path-entry substrings that identify Marlow-owned User-PATH entries."""
    root = str(marlow_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare marlow-agent install dir.
    markers = [root + "\\marlow-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if MARLOW_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "marlow".  We don't want to catch
    # unrelated entries like "cmarlow-foo" or "ephermeral", so we look for
    # backslash-marlow as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(marlow_home: Path) -> list[str]:
    """Strip Marlow-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _marlow_path_markers(marlow_home)
            kept: list[str] = []
...
def remove_marlow_env_vars_windows() -> list[str]:
    """Delete MARLOW_HOME and MARLOW_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("MARLOW_HOME", "MARLOW_GIT_BASH_PATH"):
                try:
...
def _is_default_marlow_home(marlow_home: Path) -> bool:
    """Return True when ``marlow_home`` points at the default (non-profile) root."""
    try:
        from marlow_constants import get_default_marlow_root
        return marlow_home.resolve() == get_default_marlow_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its MARLOW_HOME directory.

    We shell out to ``marlow -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current MARLOW_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m marlow_cli.main` so we don't depend on a `marlow`
    #    wrapper that may be half-removed mid-uninstall.
    marlow_invocation = [_sys.executable, "-m", "marlow_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                marlow_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's MARLOW_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.marlow/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.marlow/ for future reinstall
    """
    project_root = get_project_root()
    marlow_home = get_marlow_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie MARLOW_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_marlow_home(marlow_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Marlow Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {marlow_home / 'config.yaml'}")
    print(f"  Secrets: {marlow_home / '.env'}")
    print(f"  Data:    {marlow_home / 'cron/'}, {marlow_home / 'sessions/'}, {marlow_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their MARLOW_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Marlow data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Marlow code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in marlow_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\marlow\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(marlow_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Marlow-owned PATH entries in User environment")

        log_info("Removing MARLOW_HOME / MARLOW_GIT_BASH_PATH User env vars...")
        removed_env = remove_marlow_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Marlow-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing marlow command...")
    ...
    #     (only when they still point into this Marlow home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Marlow-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(marlow_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Marlow-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.marlow/, just remove the marlow-agent subdir
            if marlow_home in project_root.parents or project_root.parent == marlow_home:
    ...
    #     under MARLOW_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(marlow_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(marlow_home)
    ...
    # 5. Optionally remove ~/.marlow/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile MARLOW_HOME dirs live under
        ...
        try:
            if marlow_home.exists():
                shutil.rmtree(marlow_home)
                log_success(f"Removed {marlow_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {marlow_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {marlow_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/marlow-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
    ...
    print()
    print("Thank you for using Marlow Agent! ⚕")
    print()
