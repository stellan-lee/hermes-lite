"""`marlow checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.marlow/checkpoints/``.  Actions:

    marlow checkpoints               # same as `status`
    marlow checkpoints status        # total size, project count, breakdown
    marlow checkpoints list          # per-project checkpoint counts + workdir
    marlow checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    marlow checkpoints clear [-f]    # nuke the entire base (asks first)
    marlow checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    marlow checkpoints
    marlow checkpoints prune --retention-days 3 --max-size-mb 200
    marlow checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: marlow checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``marlow checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `marlow checkpoints` → status
