"""Profile distributions — shareable, packaged Marlow profiles via git.

A distribution is a Marlow profile published as a git repository (or
...
Subcommands (all live under ``marlow profile``, not a parallel tree):
    ...
    marlow profile install <source> [--name N] [--alias] [--force] [--yes]
    marlow profile update  <name>  [--force-config] [--yes]
    marlow profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "marlow-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            marlow_requires=str(data.get("marlow_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.marlow_requires:
            out["marlow_requires"] = self.marlow_requires
        if self.author:
...
def check_marlow_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Marlow {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Marlow distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``marlow profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Marlow profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Marlow distribution."
        )

    # Version check up-front so we fail fast
    check_marlow_requires(manifest.marlow_requires, marlow_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.marlow).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="marlow_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `marlow profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`marlow profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="marlow_dist_update_") as tmp:
        plan = plan_install(
