"""marlow hooks — inspect and manage shell-script hooks.

Usage::

    marlow hooks list
    marlow hooks test <event> [--for-tool X] [--payload-file F]
    marlow hooks revoke <command>
    marlow hooks doctor

Consent records live under ``~/.marlow/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.marlow/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``marlow hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: marlow hooks {list|test|revoke|doctor}")
        print("Run 'marlow hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.marlow/config.yaml.")
        print("See `marlow hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `marlow hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `marlow hooks test` and `marlow hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Marlow wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `marlow hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `marlow hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `marlow hooks doctor`.")
