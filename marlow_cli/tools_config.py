"""
Unified tool configuration for Marlow Agent.

`marlow tools` and `marlow setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.marlow/config.yaml under
...
from marlow_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from marlow_cli.nous_account import format_nous_portal_entitlement_message
...
from marlow_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _MARLOW_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `marlow tools` → Video Generation, which walks
...
# `marlow tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from marlow_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `marlow tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    already appears in ``CONFIGURABLE_TOOLSETS`` is skipped — plugin
    toolsets can share keys with built-in entries, and we want the built-in
    label/description to win.
    Without the dedupe, ``marlow tools`` → "reconfigure existing" would
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Marlow routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. MARLOW_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "MARLOW_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "MARLOW_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "MARLOW_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "MARLOW_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "MARLOW_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_marlow_home()}/marlow-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in marlow-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.marlow/config.yaml")
        ...
        try:
            from marlow_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: marlow plugins enable observability/langfuse")
        _print_info("    Restart Marlow for tracing to take effect.")
        _print_info("    Verify: marlow plugins list")
        ...
        try:
            ...
            _print_info("    Run later: marlow auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `marlow auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: marlow auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: marlow auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "marlow-cli" (which include all _MARLOW_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "marlow-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``marlow tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `marlow tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"marlow-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("marlow-")}
    ...
    # A plugin toolset is "known" for a platform once `marlow tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by marlow tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (marlow-cli, marlow-telegram, etc.)
    ...
    # Opening `marlow tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `marlow tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `marlow model`).
    if managed_feature:
        from marlow_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `marlow tools` and `marlow setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Marlow Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://marlow-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from marlow_constants import display_marlow_home
    print(color(f"  Tool configuration saved to {display_marlow_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'marlow' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, marlow mcp
