"""
Marlow Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m marlow_cli.main web          # Start on http://127.0.0.1:9119
    python -m marlow_cli.main web --port 8080
"""
...
try:
    ...
    # running `marlow dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["MARLOW_WEB_DIST"]) if "MARLOW_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
app = FastAPI(title="Marlow Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# MARLOW_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("MARLOW_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Marlow-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``marlow dashboard --tui``
# or MARLOW_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
#
# This list is defined in ``marlow_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from marlow_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from marlow_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    # and which providers are registered so ``marlow status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from marlow_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the marlow home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_marlow_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_marlow_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from marlow_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from marlow_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_marlow_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_marlow_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_marlow_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
# streamed to a per-action log file under ``~/.marlow/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_marlow_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "marlow-update": "marlow-update.log",
...
def _spawn_marlow_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``marlow <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``marlow_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "marlow_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "MARLOW_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``marlow gateway restart`` in the background."""
    try:
        proc = _spawn_marlow_action(["gateway", "restart"], "gateway-restart")
    except Exception as exc:
...
@app.post("/api/marlow/update")
async def update_marlow():
    """Kick off ``marlow update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("marlow-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "marlow-update",
    ...
    try:
        proc = _spawn_marlow_action(["update"], "marlow-update")
    except Exception as exc:
        _log.exception("Failed to spawn marlow update")
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.marlow/config.yaml`` under ``tts.``.
    """
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Marlow supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in marlow_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `marlow model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.marlow/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (marlow_cli/main.py
            ...
            if provider.strip().lower() == "nous":
                try:
                    from marlow_cli.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Marlow from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Marlow to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Marlow from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Marlow to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Marlow in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Marlow through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Marlow via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Marlow through an IMAP/SMTP mailbox.",
        "docs_url": "https://marlow-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Marlow to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Marlow inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Marlow through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Marlow to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Marlow to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Marlow as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://marlow-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://marlow-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
# the canonical ``marlow auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Marlow resolves Anthropic creds in this order at runtime:
    1. ``~/.marlow/.anthropic_oauth.json`` — Marlow-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_marlow_oauth_credentials,
            read_claude_code_credentials,
            _MARLOW_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_marlow_oauth_credentials = None  # type: ignore
        _MARLOW_OAUTH_FILE = None  # type: ignore

    marlow_creds = None
    if read_marlow_oauth_credentials:
        try:
            marlow_creds = read_marlow_oauth_credentials()
        except Exception:
            marlow_creds = None
    if marlow_creds and marlow_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "marlow_pkce",
            "source_label": f"Marlow PKCE ({_MARLOW_OAUTH_FILE})",
            "token_preview": _truncate_token(marlow_creds.get("accessToken")),
            "expires_at": marlow_creds.get("expiresAt"),
            "has_refresh_token": bool(marlow_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Marlow even
    when they also have a separate Marlow-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "marlow auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "marlow auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "marlow auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "marlow auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "marlow auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "marlow auth add xai-oauth",
        "docs_url": "https://marlow-agent.nousresearch.com/docs/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from marlow_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("marlow_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Marlow-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _MARLOW_OAUTH_FILE
            if _MARLOW_OAUTH_FILE.exists():
                _MARLOW_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from marlow_cli.auth import clear_provider_auth
...
#          → persists to ~/.marlow/.anthropic_oauth.json AND credential pool
...
# Guarded so marlow web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Marlow file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``marlow auth add anthropic``.
    """
    from agent.anthropic_adapter import _MARLOW_OAUTH_FILE
    payload = {
    ...
    _MARLOW_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _MARLOW_OAUTH_FILE.with_name(
        f"{_MARLOW_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _MARLOW_OAUTH_FILE)
        try:
            _MARLOW_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "marlow-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("MARLOW_PORTAL_BASE_URL")
...
# the tokens exactly like `marlow auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("MARLOW_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `marlow auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from marlow_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from marlow_cli.auth import _save_codex_tokens
...
async def import_sessions_endpoint(body: SessionImport):
    """Import dashboard/CLI session JSON or JSONL records."""
    from marlow_state import SessionDB
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `marlow sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `marlow sessions prune`)."""
...
async def get_logs(
    ...
    from marlow_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_marlow_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from marlow_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from marlow_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, MARLOW_HOME)."""
    from marlow_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["marlow_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process MARLOW_HOME at import time. The dashboard is a single
...
async def list_cron_blueprints():
    ...
    try:
        ...
        try:
            ...
            deliver_options = ["origin", "local", "all", *platforms]
        except Exception:
...
#
# Wraps the same config data layer the CLI uses (marlow_cli.mcp_config), so
# servers managed here show up under `marlow mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from marlow_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from marlow_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from marlow_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from marlow_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `marlow mcp catalog` / `marlow mcp install` read.
    """
    try:
        from marlow_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_marlow_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (marlow_cli.webhook); the webhook
...
async def list_webhooks():
    import marlow_cli.webhook as wh
...
# spawn the real `marlow gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_marlow_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_marlow_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from marlow_cli.auth import read_credential_pool
...
# `marlow memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `marlow memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_marlow_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_marlow_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_marlow_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_marlow_action(["import", archive], "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from marlow_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from marlow_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <marlow_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_marlow_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_marlow_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_marlow_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        proc = _spawn_marlow_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_marlow_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_marlow_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "marlow setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from marlow_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from marlow_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``marlow tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``marlow tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``marlow tools``
...
#
# The endpoint spawns the same ``marlow --tui`` binary the CLI uses, behind
...
try:
    from marlow_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from marlow_cli.dashboard_auth.audit import AuditEvent, audit_log
        from marlow_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
            consume_ticket,
        )
...
def _resolve_chat_argv(
    ...
    Default: whatever ``marlow --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``MARLOW_TUI_RESUME`` env var —
    matching what ``marlow_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``MARLOW_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``MARLOW_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from marlow_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("MARLOW_TUI_DISABLE_MOUSE", "1")
    env.setdefault("MARLOW_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["MARLOW_TUI_RESUME"] = resume

    if sidecar_url:
        env["MARLOW_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["MARLOW_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from marlow_cli.dashboard_auth.ws_tickets import mint_ticket
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Marlow inside WSL2 to use the dashboard's /chat "
...
# MARLOW_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`marlow_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from marlow_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/marlow/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /marlow`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__MARLOW_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/marlow``)
        ...
        ``__MARLOW_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__MARLOW_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__MARLOW_BASE_PATH__="{prefix}";'
                f"window.__MARLOW_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__MARLOW_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__MARLOW_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__MARLOW_BASE_PATH__="{prefix}";'
                f"window.__MARLOW_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/marlow`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Marlow backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Marlow Teal",         "description": "Classic dark teal — the canonical Marlow look"},
    {"name": "default-large", "label": "Marlow Teal (Large)", "description": "Marlow Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.marlow/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.marlow/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_marlow_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.marlow/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as marlow_cli.plugins):
    1. User plugins:    ~/.marlow/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.marlow/plugins/  (only if MARLOW_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # ``marlow_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("MARLOW_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".marlow" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from marlow_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_marlow_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"marlow auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from marlow_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from marlow_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from marlow_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from marlow_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.marlow/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.marlow/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"marlow_dashboard_plugin_{plugin['name']}"
...
from marlow_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from marlow_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing MARLOW_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Marlow Agent exposes a
    ...
    print(f"  Marlow Web UI → http://{host}:{port}")
