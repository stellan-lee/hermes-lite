...
The Marlow docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.marlow/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = (
    "https://marlow-agent.nousresearch.com/docs/api/model-catalog.json"
)
...
# browser clients (urllib, curl). When the primary URL fails (or there is no
# network at all) we fall back to the snapshot bundled with the package
# (``marlow_cli/data/model-catalog.json``, regenerated by
# ``scripts/build_model_catalog.py``) rather than another remote URL, so the
# CLI keeps working fully offline.
...
def _cache_path() -> Path:
    ...
    return get_marlow_home() / "cache" / "model_catalog.json"


# Snapshot shipped with the package (see scripts/build_model_catalog.py). Used
# as the offline fallback when neither the network fetch nor a disk cache copy
# is available, so the model picker still works without connectivity.
_BUNDLED_CATALOG_PATH = Path(__file__).resolve().parent / "data" / "model-catalog.json"


def _read_bundled_catalog() -> dict[str, Any] | None:
    """Return the package-bundled catalog manifest, or None if absent/invalid."""
    try:
        with open(_BUNDLED_CATALOG_PATH, encoding="utf-8") as fh:
            data = json.load(fh)
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    # Last resort: the snapshot bundled with the package. Seed the disk cache
    # so subsequent calls hit the fast path and the TTL governs the next fetch.
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``marlow model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
