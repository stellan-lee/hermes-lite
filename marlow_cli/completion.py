"""Shell completion script generation for marlow CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_marlow_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Marlow Agent bash completion
# Add to ~/.bashrc:
#   eval "$(marlow completion bash)"

_marlow_profiles() {{
    local profiles_dir="$HOME/.marlow/profiles"
...
_marlow_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_marlow_profiles)" -- "$cur"))
        return
    fi
...
complete -F _marlow_completion marlow
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef marlow
# Marlow Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(marlow completion zsh)"

_marlow_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.marlow/profiles" ]]; then
        profiles+=($HOME/.marlow/profiles/*(N/:t))
    fi
...
_marlow() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_marlow_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'marlow command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Marlow Agent fish completion",
        "# Add to your config:",
        "#   marlow completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __marlow_profiles",
        "    echo default",
        "    if test -d $HOME/.marlow/profiles",
        "        for d in $HOME/.marlow/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c marlow -f -s p -l profile"
        " -d 'Profile name' -xa '(__marlow_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__marlow_profiles)' -d 'Profile name'"
                )
