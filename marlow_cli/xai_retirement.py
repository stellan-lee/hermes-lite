...
Pure logic: walks a Marlow config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `marlow doctor` and a future `marlow migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Marlow config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Marlow config and return retirement issues.
