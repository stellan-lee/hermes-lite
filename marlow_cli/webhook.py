"""marlow webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    marlow webhook subscribe <name> [options]
    marlow webhook list
    marlow webhook remove <name>
    marlow webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.marlow/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _marlow_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: marlow gateway run
"""
...
def webhook_command(args):
    """Entry point for 'marlow webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: marlow webhook {subscribe|list|remove|test}")
        print("Run 'marlow webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (marlow gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: marlow webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from marlow webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (marlow gateway run)")
