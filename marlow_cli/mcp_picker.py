"""MCP picker — interactive `marlow mcp picker` (also the default `marlow mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``marlow mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `marlow plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from marlow_cli.cli_output import prompt_yes_no
from marlow_cli.curses_ui import curses_single_select
from marlow_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Marlow session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from marlow_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `marlow mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: marlow mcp install <name>    Picker: marlow mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Marlow is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Marlow — run `marlow update` "
...
def show_catalog() -> None:
    """`marlow mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`marlow mcp picker` (and default `marlow mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`marlow mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `marlow mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
