...
def _has_any_provider_configured() -> bool:
    ...
    from marlow_cli.auth import get_codex_auth_status
    from marlow_cli.config import get_env_value, load_config
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Marlow — chat, TUI, and gateway. The skills_sync
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  If the main runtime is unavailable, Marlow uses your explicit")
...
def _update_node_dependencies() -> None:
    ...
    # workspaces — but `marlow update` only needs the CLI/TUI build, so we
...
""",
    ...
    skills_parser.set_defaults(
        func=lambda args: __import__(
            "marlow_cli.skills_config", fromlist=["skills_command"]
        ).skills_command(args)
    )
