#!/usr/bin/env python3
"""
Marlow CLI - Main entry point.

Usage:
    marlow                     # Interactive chat (default)
    marlow chat                # Interactive chat
    marlow gateway             # Run gateway in foreground
    marlow gateway start       # Start gateway as service
    marlow gateway stop        # Stop gateway service
    marlow gateway status      # Show gateway status
    marlow gateway install     # Install gateway service
    marlow gateway uninstall   # Uninstall gateway service
    marlow setup               # Interactive setup wizard
    marlow logout              # Clear stored authentication
    marlow status              # Show status of all components
    marlow cron                # Manage cron jobs
    marlow cron list           # List cron jobs
    marlow cron status         # Check if cron scheduler is running
    marlow doctor              # Check configuration and dependencies
    marlow honcho setup                    # Configure Honcho AI memory integration
    marlow honcho status                   # Show Honcho config and connection status
    marlow honcho sessions                 # List directory → session name mappings
    marlow honcho map <name>               # Map current directory to a session name
    marlow honcho peer                     # Show peer names and dialectic settings
    marlow honcho peer --user NAME         # Set user peer name
    marlow honcho peer --ai NAME           # Set AI peer name
    marlow honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    marlow honcho mode                     # Show current memory mode
    marlow honcho mode [hybrid|honcho|local]  # Set memory mode
    marlow honcho tokens                   # Show token budget settings
    marlow honcho tokens --context N       # Set session.context() token cap
    marlow honcho tokens --dialectic N     # Set dialectic result char cap
    marlow honcho identity                 # Show AI peer identity representation
    marlow honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    marlow honcho migrate                  # Step-by-step migration guide: OpenClaw native → Marlow + Honcho
    marlow version             Show version
    marlow update              Update to latest version
    marlow uninstall           Uninstall Marlow Agent
    marlow acp                 Run as an ACP server for editor integration
    marlow sessions browse     Interactive session picker with search

    marlow claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: marlow_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``marlow_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``marlow update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``marlow_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, marlow crashes on import and the user can't run
# ``marlow update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'marlow' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``marlow tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``marlow.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("marlow")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"marlow", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"marlow")
        # Windows: the .exe name is already ``marlow.exe`` — nothing to do.
    except Exception:
        pass
...
# `entry.tsx`; this is just the earlier cousin. ``MARLOW_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("MARLOW_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not (os.environ.get("MARLOW_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`marlow --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from marlow_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Marlow Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``marlow --version`` before config/logging imports on Termux."""
    if os.environ.get("MARLOW_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to MARLOW_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (marlow tools, marlow setup, marlow model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'marlow {command_name}' requires an interactive terminal.\n"
...
#
# Many modules cache MARLOW_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("MARLOW_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.marlow/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set MARLOW_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors marlow_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If MARLOW_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.marlow/profiles/coder or
    # /opt/data/profiles/coder).  If MARLOW_HOME points to the marlow root
    # instead (e.g. systemd hardcodes MARLOW_HOME=/root/.marlow), we must
    # still read active_profile — the user may have switched profiles via
    # `marlow profile use` and the gateway should honour that choice.
    # See issue #22502.
    marlow_home_env = os.environ.get("MARLOW_HOME", "")
    if profile_name is None and marlow_home_env:
        if Path(marlow_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the marlow root
    if profile_name is None:
        try:
            from marlow_constants import get_default_marlow_root

            active_path = get_default_marlow_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set MARLOW_HOME
    if profile_name is not None:
        try:
            from marlow_cli.profiles import resolve_profile_env

            marlow_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent marlow from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["MARLOW_HOME"] = marlow_home
...
# Load .env from ~/.marlow/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from marlow_cli.config import get_marlow_home
from marlow_cli.env_loader import load_marlow_dotenv

load_marlow_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → MARLOW_REDACT_SECRETS env
# var BEFORE marlow_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_marlow_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "MARLOW_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["MARLOW_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `marlow` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
# The dashboard bootstraps with GUI mode so gui.log is always present
# during dashboard runs, including pre-dispatch startup failures.
try:
    from marlow_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from marlow_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if marlow_constants not importable yet
...
from marlow_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_marlow_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("MARLOW_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("MARLOW_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from marlow_cli.config import get_env_path, get_marlow_home, load_config
    from marlow_cli.auth import get_auth_status

    # Determine whether Marlow itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_marlow_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_marlow_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Marlow has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Marlow to use their tokens.
    if _has_marlow_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, marlow_bin
        cli_args: the original CLI arguments (everything after 'marlow')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    marlow_bin = container_info["marlow_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo marlow {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo marlow {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  marlow --tui --resume {target}")
    if title:
        print(f'  marlow --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @marlow/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@marlow" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/marlow-ink/package.json",
    "packages/marlow-ink/index.js",
    "packages/marlow-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``MARLOW_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("MARLOW_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``MARLOW_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("MARLOW_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    marlow_home = os.environ.get("MARLOW_HOME") or str(Path.home() / ".marlow")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "MARLOW_HOME": marlow_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(marlow_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(marlow_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if marlow_cli_dir is None:
        marlow_cli_dir = Path(__file__).parent
    bundled = marlow_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (MARLOW_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("MARLOW_NODE")
    ...
    ext_dir = os.environ.get("MARLOW_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with MARLOW_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset MARLOW_TUI_DIR (e.g. `unset MARLOW_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("MARLOW_QUIET"):
    ...
    if tui_dev:
        # Keep the local @marlow/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @marlow/ink resolves through
        # packages/marlow-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "marlow-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from marlow_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="marlow-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["MARLOW_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["MARLOW_PYTHON_SRC_ROOT"] = os.environ.get(
        "MARLOW_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("MARLOW_PYTHON", sys.executable)
    env.setdefault("MARLOW_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["MARLOW_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["MARLOW_MODEL"] = model
        env["MARLOW_INFERENCE_MODEL"] = model
    if provider:
        env["MARLOW_TUI_PROVIDER"] = provider
        env["MARLOW_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["MARLOW_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["MARLOW_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["MARLOW_TUI_SKILLS"] = value
    if query:
        env["MARLOW_TUI_QUERY"] = query
    if image:
        env["MARLOW_TUI_IMAGE"] = image
    if checkpoints:
        env["MARLOW_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["MARLOW_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["MARLOW_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["MARLOW_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["MARLOW_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["MARLOW_ACCEPT_HOOKS"] = "1"
    ...
    # MARLOW_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `marlow --tui` try to
    ...
    # still set MARLOW_TUI_RESUME themselves.
    env.pop("MARLOW_TUI_RESUME", None)
    if resume_session_id:
        env["MARLOW_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `marlow update` so
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.marlow/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Marlow — chat, dashboard, and gateway. The skills_sync
    ...
    dependency. Marlow still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("MARLOW_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'marlow sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from marlow_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from marlow_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'marlow doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Marlow isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  marlow setup")
        print()

        from marlow_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'marlow setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from marlow_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["MARLOW_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.marlow/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["MARLOW_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["MARLOW_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["MARLOW_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from marlow_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from marlow_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Marlow?")
        print()
    ...
    # has no creds.json.  Every subsequent `marlow gateway` then paid a 30s
    ...
    session_dir = get_marlow_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: marlow gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `marlow gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  marlow gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Marlow Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  marlow gateway")
        ...
        print()
        print("  Or install as a service: marlow gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'marlow whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from marlow_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from marlow_cli.config import stamp_install_method
    from marlow_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Marlow post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from marlow_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``marlow model``) and the setup wizard
    ...
    from marlow_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("MARLOW_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'marlow model' for "
                "available providers, or run 'marlow doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in marlow_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.marlow/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.marlow/.env if the active provider is not 'custom'.
    ...
    """
    from marlow_cli.config import get_env_value, save_env_value, load_config
...
#
# Marlow uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `marlow model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `marlow model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`marlow_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Marlow only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``marlow model`` flow, then route aux tasks to them here.
    """
    from marlow_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from marlow_cli.auth import _prompt_model_selection
    from marlow_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from marlow_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from marlow_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.marlow/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `marlow setup` from scratch. OpenRouter
    ...
    from marlow_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `marlow auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from marlow_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``marlow model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``marlow model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``marlow auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from marlow_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from marlow_cli.auth import _save_model_choice, deactivate_provider
    from marlow_cli.config import get_env_value, load_config, save_config
    from marlow_cli.secret_prompt import masked_secret_prompt
    ...
    from marlow_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `marlow model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from marlow_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Marlow URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from marlow_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from marlow_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from marlow_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Marlow will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Marlow will install it now (the preflight below "
...
# downstream call sites read `marlow_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `marlow` invocation — including
# fast paths like `marlow --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from marlow_cli.config import save_env_value, load_config, save_config
    from marlow_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from marlow_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from marlow_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from marlow_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Marlow turns to `copilot --acp`.")
    print("  Marlow currently starts its own ACP subprocess for each request.")
    print("  Marlow uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set MARLOW_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``marlow setup`` / ``marlow model``.
    ...
    recover from a malformed paste without editing ``~/.marlow/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from marlow_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from marlow_cli.config import save_env_value
    from marlow_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `marlow setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from marlow_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so marlow knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Marlow typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Marlow, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from marlow_cli.models import fetch_lmstudio_models
        ...
        from marlow_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from marlow_constants import display_marlow_home as _dhh_fn

            print(
                f"    Marlow will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'marlow model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Marlow CLI with a provider."""
    from marlow_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from marlow_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from marlow_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from marlow_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from marlow_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from marlow_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``marlow slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: marlow slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `marlow slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from marlow_cli.slack_cli import slack_manifest_command
...
def cmd_hooks(args):
    ...
    from marlow_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from marlow_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `marlow security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from marlow_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from marlow_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from marlow_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from marlow_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Marlow home directory to a zip file."""
    if getattr(args, "quick", False):
        from marlow_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from marlow_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Marlow backup from a zip file."""
    from marlow_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from marlow_cli.banner import check_for_updates
        from marlow_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Marlow Agent."""
    _require_tty("uninstall")
    from marlow_cli.uninstall import run_uninstall
...
# Critical files that every ``marlow`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``marlow update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "marlow_cli/main.py",
    "marlow_cli/config.py",
    "marlow_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``marlow`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="marlow-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``marlow update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``marlow_cli/web_dist/`` (per vite.config.ts
    outDir: "../marlow_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "marlow_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``marlow`` launcher present but ``marlow_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``marlow update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``marlow web``).
    ...
    # build path usable on a stock `py -m marlow_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (marlow web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``marlow dashboard`` processes other than ourselves.

    ``marlow dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``marlow update`` replaces files on disk, the running
    ...
    patterns = [
        "marlow dashboard",
        "marlow_cli.main dashboard",
        "marlow_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "marlow.*dashboard"`) keeps us consistent
            # with `marlow_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `marlow update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``marlow update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``marlow update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: marlow curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``marlow dashboard`` processes.

    Called at the end of ``marlow update`` (default ``reason``) and also
    from ``marlow dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    marlow dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Marlow Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `marlow update "
            f"--branch {branch}`, or update against main with `marlow update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/marlow-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="marlow-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"marlow-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for marlow-agent itself
        ...
        # GitHub ZIPs extract to marlow-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"marlow-agent-{branch}")
    ...
    from marlow_cli.managed_uv import ensure_uv, rebuild_venv, update_managed_uv
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "marlow-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Marlow behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes marlow completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Marlow couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Marlow couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Marlow behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `marlow update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_marlow_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_marlow_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``marlow update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from marlow_constants import get_default_marlow_root

    default_home = get_default_marlow_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``marlow update`` is still progressing even if pip/uv itself is silent.
    """
...
def _marlow_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "marlow.exe",
        scripts_dir / "marlow-gateway.exe",
    ]


def _detect_concurrent_marlow_instances(
    ...
    The Marlow Desktop Electron app spawns ``marlow.EXE`` as a backend child,
    so during ``marlow update`` the user-invoked process and the desktop's
    ...
    shims (``marlow.exe`` / ``marlow-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``marlow update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``marlow.exe``) is a separate process from the Python
    ...
    for shim in _marlow_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated marlow.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``marlow update``
    ...
    #      marlow.exe sitting *under* a non-Marlow parent (e.g. a Marlow
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "marlow.exe"
    lines = ["✗ Another marlow.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Marlow Desktop, exit any open `marlow` REPLs, and")
    lines.append("  stop the gateway (`marlow gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `marlow update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_marlow_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``marlow.exe``.
    ...
    when ``marlow update`` runs, ``marlow.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``marlow.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next marlow invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Marlow Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Marlow Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _marlow_exe_shims(scripts_dir):
        ...
        print(
            "    Close Marlow Desktop, exit other `marlow` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `marlow update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_marlow_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``marlow.exe.old.*`` left by prior updates.

    Called early on every marlow invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``marlow update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `marlow update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``marlow.exe`` / ``marlow-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_marlow_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_marlow_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from marlow_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$MARLOW_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from marlow_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # workspaces — but `marlow update` only needs the CLI/TUI/web build, so we
    ...
    # needed during `marlow update` and are skipped.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``marlow update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.marlow/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``marlow update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``marlow update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.marlow/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``marlow update --gateway``) the update is already
    ...
    try:
        ...
        # marlow_cli.config.get_marlow_home to simulate setup failure.
        from marlow_cli.config import get_marlow_home as _get_marlow_home

        logs_dir = _get_marlow_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``marlow update --check``: fetch and report without installing.
    ...
    if method == "pip":
        from marlow_cli.config import recommended_update_command
        from marlow_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``marlow update`` without requiring a reinstall.  The
    ...
    ``marlow`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/marlow.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v marlow'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/marlow, code at /usr/local/lib/marlow-agent).
    fhs_link = Path("/usr/local/bin/marlow")
    ...
    path_comment = (
        "# Marlow Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of MARLOW_HOME before running the update.
    ...
    MARLOW_HOME directories.  The ``--backup`` flag on ``marlow update``
    ...
    try:
        from marlow_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_marlow_home so the user sees ~/.marlow/...
    try:
        from marlow_constants import get_marlow_home, display_marlow_home

        home = get_marlow_home()
        try:
            display_path = f"{display_marlow_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  marlow import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``marlow update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Marlow Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Marlow Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `marlow update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Marlow via pip (for PyPI installs)."""
    from marlow_cli import __version__
    from marlow_cli.config import is_uv_tool_install

    print(f"→ Current version: {__version__}")
    print("→ Checking PyPI for updates...")

    from marlow_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "marlow-agent"]
        ...
        cmd = [pipx, "upgrade", "marlow-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "marlow-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "marlow-agent"]
    ...
    print("✓ Update complete! Restart marlow to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Marlow Agent...")
    print()

    # On Windows, abort early if another marlow.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_marlow_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $MARLOW_HOME, this is cheap
        ...
        try:
            from marlow_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in marlow_cli/config.py bricked
        # every user who ran ``marlow update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``marlow
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``marlow update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_marlow_home added to marlow_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload marlow_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_marlow_home() added since the last release.
        try:
            import importlib
            import marlow_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit MARLOW_HOME so
        # it is not affected by sync_skills()'s module-level MARLOW_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's MARLOW_HOME env var points at the default or a named profile.
        try:
            from marlow_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'marlow config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: marlow config migrate")
            else:
                print()
                print("Skipped. Run 'marlow config migrate' later to configure.")
        else:
        ...
        try:
            from marlow_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `marlow curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``marlow update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from marlow_cli.tools_config import install_cua_driver
        ...
        # When running as ``marlow update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_marlow_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all marlow-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. marlow-gateway.service or marlow-gateway-coder.service
                            ...
                            # path in `marlow gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: marlow gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: marlow -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Marlow gateway unit files are still installed.
        # When both marlow.service (from a pre-rename install) and the
        # current marlow-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `marlow update` surfaces the issue until the user migrates.
        try:
            from marlow_cli.gateway import (
                has_legacy_marlow_units,
                _find_legacy_marlow_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_marlow_units():
                print()
                print("⚠ Legacy Marlow gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_marlow_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (marlow.service) fight the current")
                print("  marlow-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    marlow gateway migrate-legacy")
                print()
        ...
        print("  marlow model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``marlow -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `marlow profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_marlow_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → marlow -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.marlow)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  marlow profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     marlow -p {name} chat")
                else:
        ...
        from marlow_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from marlow_constants import get_marlow_home as _hh
        ...
        from marlow_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `marlow profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from marlow_cli.profiles import profile_exists
        ...
        from marlow_cli.profiles import rename_profile
        ...
        from marlow_cli.profiles import export_profile
        ...
        from marlow_cli.profiles import import_profile
        ...
        import tempfile
        from marlow_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="marlow_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  marlow -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      marlow -p {plan.manifest.name} chat")
        ...
        from marlow_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `marlow profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"marlow -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from marlow_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("marlow_requires"):
            print(f"Requires:     Marlow {data['marlow_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from marlow_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.marlow_requires:
        print(f"  Requires: Marlow {mf.marlow_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``marlow dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``marlow dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No marlow dashboard processes running.")
        return 0

    print(f"{len(pids)} marlow dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No marlow dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `marlow update`.
    ...
    # the same logs directory as every other Marlow surface.
    try:
        from marlow_logging import setup_logging as _setup_logging_gui
    ...
    # Seed bundled skills on first dashboard launch so the dashboard's
    ...
    # backend needs the same.
    _sync_bundled_skills_quietly()

    if "MARLOW_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["MARLOW_WEB_DIST"])
            if "MARLOW_WEB_DIST" in os.environ
            else PROJECT_ROOT / "marlow_cli" / "web_dist"
        )
    ...
    from marlow_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("MARLOW_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from marlow_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from marlow_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Marlow log files."""
    from marlow_cli.logs import tail_log, list_logs
...
# so that in ``marlow -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``marlow_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``marlow -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``marlow`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("MARLOW_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("MARLOW_TUI") == "1" or "--tui" in argv:
        return False
    ...
    from marlow_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from marlow_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["MARLOW_DEFER_AGENT_STARTUP"] = "1"
            os.environ["MARLOW_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["MARLOW_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `marlow --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("MARLOW_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("MARLOW_TUI") == "1"):
        return False
...
def main():
    """Main entry point for marlow CLI."""
    # Cosmetic: make the process show up as 'marlow' instead of 'python3.11'
    ...
    # Sweep stale ``marlow.exe.old.*`` quarantine files left by previous
    # ``marlow update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_marlow_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: marlow-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from marlow_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `marlow model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.marlow/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://marlow-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from marlow_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from marlow_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if MARLOW_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy marlow.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Marlow gateway unit files "
            "(e.g. marlow.service) left over from older installs. Profile "
            "units (marlow-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `marlow proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Marlow Agent with an interactive wizard. "
        "Run a specific section: marlow setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'marlow setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Marlow.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $MARLOW_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Marlow")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from marlow_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Marlow CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: marlow-cli)"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Marlow Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.marlow/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Marlow profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.marlow/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Marlow profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from marlow_cli.portal_cli import add_parser as _add_portal_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.marlow/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.marlow/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Marlow Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `marlow "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Marlow "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.marlow/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Marlow Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Marlow setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Marlow Agent. Use 'marlow debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    marlow debug share              Upload debug report and print URL
    marlow debug share --lines 500  Include more log lines
    marlow debug share --expire 30  Keep paste for 30 days
    marlow debug share --local      Print report locally (no upload)
    marlow debug share --no-redact  Disable upload-time secret redaction
    marlow debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'marlow debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Marlow home directory to a zip file",
        description="Create a zip archive of your entire Marlow configuration, "
        "skills, sessions, and data (excludes the marlow-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/marlow-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.marlow/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo marlow uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from marlow_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Marlow backup from a zip file",
        description="Extract a previously created Marlow backup into your "
        "Marlow home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Marlow Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from marlow_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.marlow/skills/.bundled_manifest) "
            "so future 'marlow update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`marlow update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `marlow update`. Pass --sync to re-seed now."
        ),
    )
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from marlow_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from marlow_cli.skills_hub import skills_command
    ...
    from marlow_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/marlow-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `marlow plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from marlow_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``marlow --help``, ``marlow version``, ``marlow logs``,
    ...
    try:
        from marlow_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from marlow_cli.journey import register_cli as _register_journey_cli
    ...
    from marlow_cli.experience import register_cli as _register_experience_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_marlow_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_marlow_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_marlow_home()}/memories/\n")
        else:
            from marlow_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'marlow tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # marlow tools list [--platform cli]
    ...
    # marlow tools disable <name...> [--platform cli]
    ...
    # marlow tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from marlow_cli.tools_config import tools_disable_enable_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `marlow computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `marlow tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: marlow computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: marlow computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Marlow as an MCP server",
        description=(
            "Manage MCP server connections and run Marlow as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'marlow mcp add' to connect to a new server, or\n"
            "'marlow mcp serve' to expose Marlow conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Marlow as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `marlow mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `marlow mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `marlow mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from marlow_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from marlow_state import workspace_key as _workspace_key
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_marlow_home() / "sessions"
            ...
            # Launch marlow --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Marlow",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Marlow",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.marlow/ (by default a "
        "single restore-point archive is written to ~/.marlow/backups/ "
        "before apply; restorable with 'marlow import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from marlow_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Marlow Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'marlow config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, marlow will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another marlow.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Marlow Agent",
        description="Remove Marlow Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Marlow Agent as an ACP (Agent Client Protocol) server",
        description="Start Marlow Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Marlow ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Marlow provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.marlow/node/ "
    ...
    def cmd_acp(args):
        """Launch Marlow Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Marlow instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `marlow update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             "Skip and add later via `marlow profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Marlow profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Marlow Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `marlow --tui` via PTY/WebSocket). "
            "Alternatively set MARLOW_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `marlow dashboard` cmdlines and SIGTERM them directly — the same
    # path `marlow update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running marlow dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running marlow dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Marlow log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    marlow logs                    Show last 50 lines of agent.log
    marlow logs -f                 Follow agent.log in real time
    marlow logs errors             Show last 50 lines of errors.log
    marlow logs gateway -n 100     Show last 100 lines of gateway.log
    marlow logs gui -f             Follow gui.log in real time
    marlow logs --level WARNING    Only show WARNING and above
    marlow logs --session abc123   Filter by session ID
    marlow logs --component tools  Only show tool-related lines
    marlow logs --since 1h         Lines from the last hour
    marlow logs --since 30m -f     Follow, starting from 30 min ago
    marlow logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``marlow -c Pokemon Agent Dev`` → ``marlow -c 'Pokemon Agent Dev'``
    ...
    from marlow_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'marlow -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (marlow hooks list, cron
