...
version: 1.0.0
author: arceus77-7, enhanced by Marlow Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Marlow terminal calls (desktop app flow only)
...
### Service Account (recommended for Marlow)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.marlow/.env` (the skill will prompt for this on first load).
...
## Marlow Execution Pattern (desktop app flow)

Marlow terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/marlow-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/marlow-op.sock"
