---
name: honcho
description: Configure and use Honcho memory with Marlow -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Marlow Agent
license: MIT
...
# Honcho Memory for Marlow

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Marlow profile its own peer identity while sharing a unified view of the user.
...
```bash
marlow honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Marlow profile's AI peer
...
Honcho models conversations as interactions between **peers**. Marlow creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Marlow instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Marlow run |
| `global` | Single session across all directories |

Manual override: `marlow honcho map my-project-name`
...
Each Marlow profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
marlow profile create coder --clone
# creates host block marlow.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `marlow.coder` host block in `honcho.json`
...
```bash
marlow honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Marlow when Honcho memory is active.
...
Config file: `$MARLOW_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `marlow honcho setup`. Ensure `memory.provider: honcho` is in `~/.marlow/config.yaml`.

### Memory not persisting across sessions
Check `marlow honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `marlow profile create <name> --clone`. For existing profiles: `marlow honcho sync`.
...
| Command | Description |
|---------|-------------|
| `marlow honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `marlow honcho status` | Show resolved config, connection test, peer info for active profile |
| `marlow honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `marlow honcho disable` | Disable Honcho for the active profile |
| `marlow honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `marlow honcho peers` | Show peer identities across all profiles |
| `marlow honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `marlow honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `marlow honcho sessions` | List known directory-to-session-name mappings |
| `marlow honcho map <name>` | Map current working directory to a Honcho session name |
| `marlow honcho identity` | Seed AI peer identity or show both peer representations |
| `marlow honcho sync` | Create host blocks for all Marlow profiles that don't have one yet |
| `marlow honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Marlow + Honcho |
| `marlow memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `marlow memory status` | Show active memory provider and config |
| `marlow memory off` | Disable external memory provider |
