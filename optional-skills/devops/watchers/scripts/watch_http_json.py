...
Usage (via cron with --no-agent):
    ...
    marlow cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$MARLOW_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Marlow-Watcher/1.0"})
