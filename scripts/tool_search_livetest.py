#!/usr/bin/env python3
"""Live test harness for Marlow Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any marlow imports.
ORIGINAL_HOME = os.environ.get("MARLOW_HOME")
ORIGINAL_AUTH = Path.home() / ".marlow" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.marlow/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.marlow/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="marlow_ts_live_"))
    marlow_home = home_dir / ".marlow"
    marlow_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, marlow_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".marlow" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, marlow_home / ".env")
        ...
        load_marlow_dotenv(marlow_home=str(Path.home() / ".marlow"))
    ...
    (marlow_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return marlow_home
...
def reset_module_state():
    """Drop cached modules so the new MARLOW_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "marlow_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["MARLOW_HOME"] = str(home)
...
def main():
    ...
    # Restore original MARLOW_HOME
    if ORIGINAL_HOME is not None:
        os.environ["MARLOW_HOME"] = ORIGINAL_HOME
    else:
