# ============================================================================
# Marlow Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/NousResearch/marlow-agent/main/scripts/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$MarlowHome = $(if ($env:MARLOW_HOME) { $env:MARLOW_HOME } else { "$env:LOCALAPPDATA\marlow" }),
    [string]$InstallDir = $(if ($env:MARLOW_HOME) { "$env:MARLOW_HOME\marlow-agent" } else { "$env:LOCALAPPDATA\marlow\marlow-agent" }),
...
$RepoUrlSsh = "git@github.com:NousResearch/marlow-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/marlow-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Marlow Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $MarlowHome)) {
        New-Item -ItemType Directory -Force -Path $MarlowHome | Out-Null
    }
    $envFile = Join-Path $MarlowHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $MarlowHome "node"
...
function Install-Uv {
    # Marlow owns its own uv at $MarlowHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (marlow_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `marlow update` stay in sync.
    $managedUv = Join-Path $MarlowHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $MarlowHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $MarlowHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $MarlowHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $MarlowHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and MARLOW_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Marlow uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\marlow\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Marlow needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Marlow copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\marlow\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``MARLOW_GIT_BASH_PATH`` (User scope) so Marlow can find it in a fresh
    ...
    # Download PortableGit into $MarlowHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $MarlowHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Marlow features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Marlow needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``MARLOW_GIT_BASH_PATH`` (User env scope) so Marlow can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $MarlowHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $MarlowHome\git\usr\bin\bash.exe
    $candidates += "$MarlowHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$MarlowHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("MARLOW_GIT_BASH_PATH", $candidate, "User")
            $env:MARLOW_GIT_BASH_PATH = $candidate
            Write-Info "Set MARLOW_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Marlow may not find Git Bash."
    Write-Info "If needed, set MARLOW_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$MarlowHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$MarlowHome\node;$env:Path"
        Write-Success "Node.js $version found (Marlow-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $MarlowHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $MarlowHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\marlow-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$MarlowHome\node") { Remove-Item -Recurse -Force "$MarlowHome\node" }
                Move-Item $extractedDir.FullName "$MarlowHome\node"
                ...
                $version = & "$MarlowHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $MarlowHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\marlow-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running marlow processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/marlow-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/marlow-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/marlow-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\marlow-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\marlow-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `marlow.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install marlow-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and marlow.exe broken with
    ...
    # here, not 30 seconds later when the user runs `marlow`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `marlow dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `marlow dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `marlow dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up marlow command..."
    
    if ($NoVenv) {
        $marlowBin = "$InstallDir"
    } else {
        $marlowBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so marlow is globally available
    # On Windows, the marlow.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$marlowBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$marlowBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $marlowBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set MARLOW_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\marlow instead
    # of the Unix default ~/.marlow
    $currentMarlowHome = [Environment]::GetEnvironmentVariable("MARLOW_HOME", "User")
    if (-not $currentMarlowHome -or $currentMarlowHome -ne $MarlowHome) {
        [Environment]::SetEnvironmentVariable("MARLOW_HOME", $MarlowHome, "User")
        Write-Success "Set MARLOW_HOME=$MarlowHome"
    }
    $env:MARLOW_HOME = $MarlowHome
    
    # Update current session
    $env:Path = "$marlowBin;$env:Path"
    
    Write-Success "marlow command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.marlow-bootstrap-complete which tells the Marlow
    ...
    # Marlow-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `marlow update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".marlow-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.marlow directory structure
    New-Item -ItemType Directory -Force -Path "$MarlowHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarlowHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarlowHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarlowHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarlowHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarlowHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarlowHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarlowHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarlowHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$MarlowHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.marlow/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.marlow/.env"
        }
    } else {
        Write-Info "~/.marlow/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$MarlowHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.marlow/config.yaml from template"
        }
    } else {
        Write-Info "~/.marlow/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Marlow's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.marlow/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.marlow/"
    
    # Seed bundled skills into ~/.marlow/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.marlow/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.marlow/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$MarlowHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.marlow/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Marlow-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'marlow setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\marlow-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\marlow-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\marlow-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.marlow/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'marlow setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m marlow_cli.main setup
    } else {
        python -m marlow_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $marlowCmd = "$InstallDir\venv\Scripts\marlow.exe"
    if (-not (Test-Path $marlowCmd)) {
        $marlowCmd = "marlow"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$MarlowHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'marlow whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: marlow gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$MarlowHome\logs\gateway.log"
            Start-Process -FilePath $marlowCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$MarlowHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: marlow gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: marlow gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$MarlowHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$MarlowHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MarlowHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MarlowHome\marlow-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   marlow              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   marlow setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   marlow config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   marlow config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   marlow gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   marlow update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Marlow repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Marlow to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/marlow-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
