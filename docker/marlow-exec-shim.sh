#!/bin/sh
# shellcheck shell=sh
# /opt/marlow/bin/marlow — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `marlow` user (UID 10000). When an operator runs `docker exec <c> marlow ...`
# the default UID is root (0), and any file the command writes under
# $MARLOW_HOME — auth.json, .env, config.yaml — ends up root-owned and
...
# Fix
# ---
# This shim sits at /opt/marlow/bin/marlow and is placed earliest on PATH.
# When invoked as root, it drops to the marlow user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $MARLOW_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves
# and `docker exec --user marlow` — it short-circuits
...
# (/opt/marlow/.venv/bin/marlow), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set MARLOW_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the marlow CLI. Default is to drop.

set -e

REAL=/opt/marlow/.venv/bin/marlow

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "marlow-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user marlow`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${MARLOW_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the marlow user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "marlow-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "marlow-shim: re-run with --user marlow or set MARLOW_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the marlow user's home before dropping privileges. Without
