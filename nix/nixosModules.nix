# nix/nixosModules.nix — NixOS module for marlow-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: marlow runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $MARLOW_HOME/.env
# and read by marlow at startup — no container recreation needed for env changes.
#
# Tool resolution: the marlow wrapper uses --suffix PATH for nix store tools,
...
#
# Usage:
#   services.marlow-agent = {
#     enable = true;
#     settings.model = "anthropic/claude-sonnet-4";
#     environmentFiles = [ config.sops.secrets."marlow/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.marlow-agent;
    effectivePackage =
    ...
    marlow-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-marlow-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "marlow-config-attrs";
      description = "Marlow YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "marlow-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "marlow-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'MARLOW_DOC_EOF'\n${value}\nMARLOW_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "marlow-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/marlow";
    ...
    # marlow user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "marlow-container-entrypoint" ''
      set -eu

      MARLOW_UID="''${MARLOW_UID:?MARLOW_UID must be set}"
      MARLOW_GID="''${MARLOW_GID:?MARLOW_GID must be set}"

      # ── Group: ensure a group with GID=$MARLOW_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$MARLOW_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="marlow"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$MARLOW_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$MARLOW_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$MARLOW_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$MARLOW_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="marlow"
        TARGET_HOME="/home/marlow"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$MARLOW_UID" -g "$MARLOW_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$MARLOW_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$MARLOW_UID:$MARLOW_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure MARLOW_HOME is owned by the target user.
      ...
      if [ -n "''${MARLOW_HOME:-}" ] && [ -d "$MARLOW_HOME" ]; then
        find "$MARLOW_HOME" \! -user "$MARLOW_UID" -exec chown "$MARLOW_UID:$MARLOW_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/marlow-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/marlow-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/marlow ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/marlow
        chmod 0440 /etc/sudoers.d/marlow
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$MARLOW_UID" --regid="$MARLOW_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $MARLOW_HOME/.env.
    ...
    # Default: /var/lib/marlow/workspace → /data/workspace.
  ...
  in {
    options.services.marlow-agent = with lib; {
      enable = mkEnableOption "Marlow Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = marlow-agent;
        description = "The marlow-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/marlow";
        description = "State directory. Contains .marlow/ subdir (MARLOW_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Marlow config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $MARLOW_HOME/.env at activation time.
          Marlow reads this file on every startup via load_marlow_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $MARLOW_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $MARLOW_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `marlow gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the marlow user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the marlow plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Marlow discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          marlow_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as marlow (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in marlow-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the marlow CLI to environment.systemPackages and export
          MARLOW_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.marlow symlink to the service
            stateDir. These users are automatically added to the marlow group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.marlow-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the marlow CLI to system PATH and export MARLOW_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.marlow/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.MARLOW_HOME = "${cfg.stateDir}/.marlow";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.marlow-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the marlow user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.marlow-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed marlow
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure marlow is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.marlow        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.marlow/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.marlow/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.marlow/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.marlow/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.marlow/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."marlow-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.marlow
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.marlow ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.marlow ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.marlow -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.marlow/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.marlow/$_subdir"
            chmod 2770 "${cfg.stateDir}/.marlow/$_subdir"
            find "${cfg.stateDir}/.marlow/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.marlow/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.marlow/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.marlow/config.yaml
            chmod 0640 ${cfg.stateDir}/.marlow/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.marlow/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.marlow/.managed
          chmod 0644 ${cfg.stateDir}/.marlow/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.marlow/.container-mode <<'MARLOW_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    marlow_bin=${containerDataDir}/current-package/bin/marlow
    MARLOW_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.marlow/.container-mode
            chmod 0644 ${cfg.stateDir}/.marlow/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.marlow/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.marlow";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.marlow" ]; then
                  rm -f "${symlinkPath}"
                  echo "marlow-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.marlow -> stateDir/.marlow for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.marlow";
                target = "${cfg.stateDir}/.marlow";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "marlow-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.marlow/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.marlow/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.marlow/auth.json
              fi
            ''}
          ''}
          ...
          # Marlow reads $MARLOW_HOME/.env at startup via load_marlow_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.marlow/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'MARLOW_NIX_ENV_EOF'
    ${envFileContent}
    MARLOW_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.marlow/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.marlow/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.marlow/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.marlow-agent = {
          description = "Marlow Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            MARLOW_HOME = "${cfg.stateDir}/.marlow";
            MARLOW_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $MARLOW_HOME/.env by the activation script. load_marlow_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/marlow"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the marlow group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.marlow-agent = {
          description = "Marlow Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              MARLOW_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              MARLOW_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env MARLOW_UID="$MARLOW_UID" \
                --env MARLOW_GID="$MARLOW_GID" \
                --env MARLOW_HOME=${containerDataDir}/.marlow \
                --env MARLOW_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/marlow gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
