...
in
{
  ...
  mkNpmPassthru =
    ...
    in
    {
      ...
      # --ignore-scripts: workspace packages may have postinstall hooks that
      # fetch from the network. nix builds are offline, so all scripts must be
      # skipped.  Each package sets up its own build commands in buildPhase
      # instead.
