...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      marlow-agent = self'.packages.default;
      marlowVenv = marlow-agent.marlowVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "marlow-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${marlowVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "marlow-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "marlow-build-package" { } ''
          echo "PASS: package built at ${marlow-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "marlow-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "marlow-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${marlow-agent}/bin/marlow || (echo "FAIL: marlow binary missing"; exit 1)
          test -x ${marlow-agent}/bin/marlow-agent || (echo "FAIL: marlow-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${marlow-agent}/bin/marlow version 2>&1 | grep -qi "marlow" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "marlow-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in marlow marlow-agent marlow-acp; do
            test -x ${marlow-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "marlow-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking marlow --help ==="
          ${marlow-agent}/bin/marlow --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${marlow-agent}/bin/marlow --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "marlow-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${marlow-agent}/share/marlow-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${marlow-agent}/share/marlow-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "MARLOW_BUNDLED_SKILLS" ${marlow-agent}/bin/marlow || \
            (echo "FAIL: MARLOW_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: MARLOW_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "marlow-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${marlow-agent}/share/marlow-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${marlow-agent}/share/marlow-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "MARLOW_BUNDLED_PLUGINS" ${marlow-agent}/bin/marlow || \
            (echo "FAIL: MARLOW_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: MARLOW_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "marlow-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${marlow-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${marlow-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "MARLOW_TUI_DIR" ${marlow-agent}/bin/marlow || \
            (echo "FAIL: MARLOW_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: MARLOW_TUI_DIR set in wrapper"
        ...
        # Verify MARLOW_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        marlow-node = pkgs.runCommand "marlow-node-version" { } ''
          set -e
          echo "=== Checking MARLOW_NODE in wrapper ==="
          grep -q "MARLOW_NODE" ${marlow-agent}/bin/marlow || \
            (echo "FAIL: MARLOW_NODE not set in wrapper"; exit 1)
          echo "PASS: MARLOW_NODE present in wrapper"

          MARLOW_NODE=$(sed -n "s/^export MARLOW_NODE='\(.*\)'/\1/p" ${marlow-agent}/bin/marlow)
          test -x "$MARLOW_NODE" || (echo "FAIL: MARLOW_NODE=$MARLOW_NODE not executable"; exit 1)
          echo "PASS: MARLOW_NODE executable at $MARLOW_NODE"

          NODE_MAJOR=$("$MARLOW_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All MARLOW_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify MARLOW_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "marlow-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(MARLOW_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking MARLOW_MANAGED guards ==="
          check_blocked "config set" ${marlow-agent}/bin/marlow config set model foo
          check_blocked "config edit" ${marlow-agent}/bin/marlow config edit
        ...
        extra-python-packages = let
          ...
          marlowWithExtra = marlow-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "marlow-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${marlowWithExtra}/bin/marlow || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${marlowWithExtra}/bin/marlow || \
          ...
          if grep -q "PYTHONPATH" ${marlow-agent}/bin/marlow; then
        ...
        extra-dependency-groups = let
          marlowWithGroups = marlow-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "marlow-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${marlowWithGroups}"
          echo "venv: ${marlowWithGroups.marlowVenv}"
        ...
        messaging-variant = pkgs.runCommand "marlow-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.marlowVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "marlow-config-roundtrip" {
          ...
          merge_and_load() {
            local marlow_home="$1"
            export MARLOW_HOME="$marlow_home"
            ${configMergeScript} ${nixSettings} "$marlow_home/config.yaml"
            ${marlowVenv}/bin/python3 -c '
import json, sys
