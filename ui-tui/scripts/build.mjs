...
await build({
  ...
  // Skip the prebuilt @marlow/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@marlow/ink': resolve(root, 'packages/marlow-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The marlow_cli launcher
