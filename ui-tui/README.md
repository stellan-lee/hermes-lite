...
React + Ink terminal UI for Marlow. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `MARLOW_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.marlow/.marlow_history` or under `MARLOW_HOME`.
...
```text
ui-tui/
  packages/marlow-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      marlow-ink.d.ts    type declarations for @marlow/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent MarlowCLI subprocess for slash commands
```
