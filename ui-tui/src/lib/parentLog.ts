...
const logDir = join(process.env.MARLOW_HOME?.trim() || join(homedir(), '.marlow'), 'logs')
...
// a real ~/.marlow (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('marlow-tui: parent lifecycle log unavailable\n')
    }
  }
}
