...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.MARLOW_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.marlow', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `marlow-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
