...
export const opsCommands: SlashCommand[] = [
  ...
  {
    help: 're-read ~/.marlow/.env into the running gateway (CLI parity)',
    name: 'reload',
    run: (_arg, ctx) => {
      ctx.gateway
  ...
  {
    help: 'list locally installed skills',
    name: 'skills',
    run: (_arg, ctx, cmd) => {
      ctx.gateway.gw
        ...
        .then(r => {
          ...
          const body = r?.output || 'No local skills installed.'
          const formatted = r?.warning ? `warning: ${r.warning}\n${body}` : body
          ctx.transcript.page(formatted, 'Local Skills')
        })
        .catch(ctx.guardedErr)
    }
  },
