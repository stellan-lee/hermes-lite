import type { RunExternalProcess } from '@marlow/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`marlow ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching marlow: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`marlow ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
