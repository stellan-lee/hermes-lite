...
def _telegramize_command_mentions(text: str, platform: Any) -> str:
    ...
    from marlow_cli.commands import _sanitize_telegram_name
...
# ``marlow_state.get_messages`` carries on every persisted message.  This
...
def _coerce_gateway_timestamp(value: Any) -> Optional[float]:
    ...
    if isinstance(value, (int, float)):
        # Some platform events use milliseconds; Marlow state rows use seconds.
...
def _auto_continue_freshness_window() -> float:
    ...
    Reads ``MARLOW_AUTO_CONTINUE_FRESHNESS`` (bridged from
    ``config.yaml`` ``agent.gateway_auto_continue_freshness`` at gateway
    startup, same pattern as ``MARLOW_AGENT_TIMEOUT``).  Falls back to the
    ...
    """
    raw = os.environ.get("MARLOW_AUTO_CONTINUE_FRESHNESS")
...
def _float_env(name: str, default: float) -> float:
    ...
    A misconfigured env var (e.g. ``MARLOW_AGENT_TIMEOUT=abc``) must not
...
def _restart_notification_pending() -> bool:
    ...
    return (_marlow_home / ".restart_notify.json").exists()


def _planned_restart_notification_path() -> Path:
    return _marlow_home / ".restart_pending.json"
...
# Resolve Marlow home directory (respects MARLOW_HOME override)
from marlow_constants import get_marlow_home
from utils import atomic_json_write, atomic_yaml_write, base_url_host_matches, is_truthy_value
_marlow_home = get_marlow_home()

# Load environment variables from ~/.marlow/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # noqa: F401  # backward-compat for tests that monkeypatch this symbol
from marlow_cli.env_loader import load_marlow_dotenv
_env_path = _marlow_home / '.env'
load_marlow_dotenv(marlow_home=_marlow_home, project_env=Path(__file__).resolve().parents[1] / '.env')


def _reload_runtime_env_preserving_config_authority() -> None:
    """Reload .env for fresh credentials without letting stale .env override config.

    Gateway processes are long-lived, so per-turn code reloads ~/.marlow/.env to
    pick up rotated API keys. config.yaml remains authoritative for agent budget
    settings such as agent.max_turns; otherwise a stale MARLOW_MAX_ITERATIONS in
    .env can replace the startup bridge on later turns.
    """
    load_marlow_dotenv(
        marlow_home=_marlow_home,
    ...
    if isinstance(agent_cfg, dict) and "max_turns" in agent_cfg:
        os.environ["MARLOW_MAX_ITERATIONS"] = str(agent_cfg["max_turns"])
...
_config_path = _marlow_home / 'config.yaml'
if _config_path.exists():
    try:
        ...
        # .env entries (e.g. MARLOW_MAX_ITERATIONS=60 written by an old
        # `marlow setup` run) silently shadow the user's current config.
        # See PR #18413 / the 60-vs-500 max_turns incident.
        _agent_cfg = _cfg.get("agent", {})
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["MARLOW_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
            if "gateway_timeout" in _agent_cfg:
                os.environ["MARLOW_AGENT_TIMEOUT"] = str(_agent_cfg["gateway_timeout"])
            if "gateway_timeout_warning" in _agent_cfg:
                os.environ["MARLOW_AGENT_TIMEOUT_WARNING"] = str(_agent_cfg["gateway_timeout_warning"])
            if "gateway_notify_interval" in _agent_cfg:
                os.environ["MARLOW_AGENT_NOTIFY_INTERVAL"] = str(_agent_cfg["gateway_notify_interval"])
            if "restart_drain_timeout" in _agent_cfg:
                os.environ["MARLOW_RESTART_DRAIN_TIMEOUT"] = str(_agent_cfg["restart_drain_timeout"])
            if "gateway_auto_continue_freshness" in _agent_cfg:
                os.environ["MARLOW_AUTO_CONTINUE_FRESHNESS"] = str(
                    _agent_cfg["gateway_auto_continue_freshness"]
                )
        _display_cfg = _cfg.get("display", {})
        if _display_cfg and isinstance(_display_cfg, dict):
            if "busy_input_mode" in _display_cfg:
                os.environ["MARLOW_GATEWAY_BUSY_INPUT_MODE"] = str(_display_cfg["busy_input_mode"])
            if "busy_text_mode" in _display_cfg:
                os.environ["MARLOW_GATEWAY_BUSY_TEXT_MODE"] = str(_display_cfg["busy_text_mode"])
            if "busy_ack_enabled" in _display_cfg:
                os.environ["MARLOW_GATEWAY_BUSY_ACK_ENABLED"] = str(_display_cfg["busy_ack_enabled"])
        # Timezone: bridge config.yaml → MARLOW_TIMEZONE env var.
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str):
            os.environ["MARLOW_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["MARLOW_REDACT_SECRETS"] = str(_redact).lower()
        # Gateway settings (media delivery allowlist + recency trust + strict mode)
        _gateway_cfg = _cfg.get("gateway", {})
        if isinstance(_gateway_cfg, dict):
            _strict = _gateway_cfg.get("strict")
            if _strict is not None:
                os.environ["MARLOW_MEDIA_DELIVERY_STRICT"] = (
                    "1" if _strict else "0"
                )
            ...
            if _allow_dirs:
                if isinstance(_allow_dirs, str):
                    ...
                    _allow_dirs_str = ""
                if _allow_dirs_str:
                    os.environ["MARLOW_MEDIA_ALLOW_DIRS"] = _allow_dirs_str
            _trust_recent = _gateway_cfg.get("trust_recent_files")
            if _trust_recent is not None:
                os.environ["MARLOW_MEDIA_TRUST_RECENT_FILES"] = (
                    "1" if _trust_recent else "0"
                )
            _trust_recent_seconds = _gateway_cfg.get("trust_recent_files_seconds")
            if _trust_recent_seconds is not None:
                os.environ["MARLOW_MEDIA_TRUST_RECENT_SECONDS"] = str(_trust_recent_seconds)
        ...
        print(
            ...
            "your current config.yaml. Run `marlow doctor` to investigate.",
            file=sys.stderr,
        )

# Apply IPv4 preference if configured (before any HTTP clients are created).
try:
    from marlow_constants import apply_ipv4_preference
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from marlow_cli.auth import AuthError, is_rate_limited_auth_error
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if slug == normalized and declared_name in disabled:
                    return (
                        ...
                        f"Enable it with: `marlow skills config`"
                    )
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if slug == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `marlow skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.marlow/config.yaml, returning {} on any error.

    Uses the module-level ``_marlow_home`` (so tests that monkeypatch it
    still see their fixture) and shares the mtime-keyed raw-yaml cache
    from ``marlow_cli.config.read_raw_config`` when the paths match.
    """
    config_path = _marlow_home / 'config.yaml'
    try:
        from marlow_cli.config import get_config_path, read_raw_config
        # Fast path: if _marlow_home agrees with the canonical config
        # location, reuse the shared cache. Otherwise fall through to a
        # direct read (keeps test fixtures with a monkeypatched
        # _marlow_home working).
...
def _load_gateway_runtime_config() -> dict:
    ...
    ``gateway.run._marlow_home``. Build on ``_load_gateway_config()`` rather
...
def _resolve_marlow_bin() -> Optional[list[str]]:
    ...
    try:
        ...
        if importlib.util.find_spec("marlow_cli") is not None:
...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        try:
            ...
            # MARLOW_HOME silently lost /resume, /title, /history, /branch, and
            # session search without this.  The underlying cause (usually
            # "locking protocol" from NFS) is now also captured by
            # marlow_state.get_last_init_error() for slash-command error strings.
    ...
    def _warn_if_docker_media_delivery_is_risky(self) -> None:
        ...
        logger.warning(
            ...
            "output mount (for example '/home/user/.marlow/cache/documents:/output') is configured. "
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the marlow-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("marlow-agent-setup") is not None
        except Exception:
            return False

    # -- Voice mode persistence ------------------------------------------

    _VOICE_MODE_PATH = _marlow_home / "gateway_voice_mode.json"
    ...
    def _sync_voice_mode_state_to_adapter(self, adapter) -> None:
        ...
        # Lazy import to avoid adding a module-level dep from gateway → marlow_cli.
        try:
            from marlow_cli.config import load_config as _load_full_config
    ...
    def _adapter_disconnect_timeout_secs(self) -> float:
        ...
        raw = os.getenv("MARLOW_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                timeout = float(raw)
            except ValueError:
                logger.warning(
                    "Ignoring invalid MARLOW_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT=%r",
                    raw,
                )
            else:
    ...
    def _platform_connect_timeout_secs(self) -> float:
        ...
        raw = os.getenv("MARLOW_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                timeout = float(raw)
            except ValueError:
                logger.warning(
                    "Ignoring invalid MARLOW_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                    raw,
                )
            else:
                return max(0.0, timeout)
    ...
    def _telegram_topic_root_lobby_message(self) -> str:
        return (
            ...
            "To start a new Marlow chat, open the All Messages topic at the top "
            "of this bot interface and send any message there. Telegram will "
            "create a new topic for that message; each topic works as an "
            "independent Marlow session."
        )

    def _telegram_topic_root_new_message(self) -> str:
        return (
            "To start a new parallel Marlow chat, open the All Messages topic "
            "at the top of this bot interface and send any message there. "
            "Telegram will create a new topic for it.\n\n"
            "Each topic is an independent Marlow session. Use /new inside an "
    ...
    def _telegram_topic_new_header(self, source: SessionSource) -> Optional[str]:
        if not self._is_telegram_topic_lane(source):
            return None
        return (
            "Started a new Marlow session in this topic.\n\n"
    ...
    def _record_telegram_topic_binding(
        ...
        """Persist the Telegram topic -> Marlow session binding for topic lanes."""
        session_db = getattr(self, "_session_db", None)
    ...
    def _sync_telegram_topic_binding(
        ...
        *,
        ...
        so reopening a topic in a fresh process resumes the right Marlow
    ...
    def _resolve_session_agent_runtime(
        ...
        *,
        ...
        # (e.g. user ran `marlow auth add openai-codex` without `marlow model`),
    ...
    def _pause_failed_platform(self, platform, *, reason: str = "") -> None:
        ...
        logger.warning(
            ...
            "to retry, or `marlow gateway restart` to restart the gateway.",
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks MARLOW_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.marlow/config.yaml.
        Relative paths are resolved from ~/.marlow/.
        """
        file_path = os.getenv("MARLOW_PREFILL_MESSAGES_FILE", "")
        if not file_path:
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks MARLOW_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.marlow/config.yaml.
        """
        prompt = os.getenv("MARLOW_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        cfg = _load_gateway_runtime_config()
    ...
    def _load_busy_input_mode() -> str:
        ...
        mode = os.getenv("MARLOW_GATEWAY_BUSY_INPUT_MODE", "").strip().lower()
        if not mode:
            cfg = _load_gateway_runtime_config()
    ...
    def _load_busy_text_mode() -> str:
        ...
        mode = os.getenv("MARLOW_GATEWAY_BUSY_TEXT_MODE", "").strip().lower()
        if not mode:
            cfg = _load_gateway_runtime_config()
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("MARLOW_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            cfg = _load_gateway_runtime_config()
    ...
    async def _handle_active_session_busy_message(self, event: MessageEvent, session_key: str) -> bool:
        ...
        busy_ack_enabled = os.environ.get("MARLOW_GATEWAY_BUSY_ACK_ENABLED", "true").lower() == "true"
    ...
    def _increment_restart_failure_counts(self, active_session_keys: set) -> None:
        ...
        path = _marlow_home / self._STUCK_LOOP_FILE
        try:
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        marlow_cmd = _resolve_marlow_bin()
        if not marlow_cmd:
            logger.error("Could not locate marlow binary for detached /restart")
            return
    ...
    def _launch_systemd_restart_shortcut(self) -> None:
        ...
        try:
            ...
            try:
                from marlow_cli.gateway import get_service_name

                service_name = get_service_name()
            except Exception:
                service_name = "marlow-gateway"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Marlow Gateway...")
        try:
        ...
        # window.  When the user upgraded marlow-agent without re-running
        # ``marlow setup``, their unit file may still encode the old
        ...
        try:
            ...
            if _alignment is not None and _alignment.get("mismatch"):
                logger.warning(
                    ...
                    "gateway mid-drain. Run `marlow gateway service install --replace` "
        ...
        try:
            _effective_max_iter = int(os.getenv("MARLOW_MAX_ITERATIONS", "90"))
            logger.info(
                "Agent budget: max_iterations=%d (agent.max_turns from config.yaml, "
                "or MARLOW_MAX_ITERATIONS from .env, or default 90)",
                _effective_max_iter,
            )
        except Exception:
            pass
        ...
        try:
            ...
            if _redact_on:
                ...
                logger.warning(
                    "Secret redaction: DISABLED (MARLOW_REDACT_SECRETS=%s). "
        ...
        # in gateway.log and `marlow status` surfaces it; we do NOT block
        ...
        # rotate credentials).  See marlow_cli/security_advisories.py.
        try:
            ...
            if _adv_msg:
                ...
                logger.warning(
                    "Run `marlow doctor` on the gateway host for full "
                    "remediation steps."
                )
        except Exception:
            logger.debug(
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.marlow/.env to allow open access, "
                "or configure platform allowlists (e.g., TELEGRAM_ALLOWED_USERS=your_id)."
            )
        
        # Discover Python plugins before shell hooks so plugin block
        # decisions take precedence in tie cases.  The CLI startup path
        # does this via an explicit call in marlow_cli/main.py; the
        ...
        # channels (--accept-hooks on launch, MARLOW_ACCEPT_HOOKS env var,
        ...
        # This prevents unwanted auto-resets after `marlow update`,
        # `marlow gateway restart`, or `/restart`.
        _clean_marker = _marlow_home / ".clean_shutdown"
        ...
        if not notified and any(
            path.exists()
            for path in (
                _marlow_home / ".update_pending.json",
                _marlow_home / ".update_pending.claimed.json",
            )
        ):
    ...
    async def _process_handoff(self, row: Dict[str, Any]) -> None:
        ...
        thread_name = f"Marlow — {cli_title}"
        try:
    ...
    async def stop(
        ...
        *,
        ...
        async def _stop_impl() -> None:
            ...
            if not timed_out:
                try:
                    (_marlow_home / ".clean_shutdown").touch()
                except Exception:
                    pass
            else:
                logger.info(
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            _notify_mode = os.getenv("MARLOW_TELEGRAM_NOTIFICATIONS", "")
            if not _notify_mode:
                try:
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`marlow pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in {"approve", "yes"}:
                ...
                if cmd:
                    try:
                        from marlow_cli.commands import resolve_command as _resolve_update_cmd
                    except Exception:
                ...
                if _recognized_cmd:
                    ...
                    response_text = raw
            if response_text:
                response_path = _marlow_home / ".update_response"
                prompt_path = _marlow_home / ".update_prompt.json"
                try:
        ...
        _raw_stale_timeout = _float_env("MARLOW_AGENT_TIMEOUT", 1800)
        ...
        if _quick_key in self._running_agents:
            ...
            _telegram_followup_grace = float(
                os.getenv("MARLOW_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "3.0")
            )
        ...
        if command:
            try:
                from marlow_cli.plugins import get_plugin_command_handler
                # Normalize underscores to hyphens so Telegram's underscored
                # autocomplete form matches plugin commands registered with
                # hyphens. See marlow_cli/commands.py:_build_telegram_menu.
        ...
        *,
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`uv pip install faster-whisper` in the Marlow venv; "
                                "`pip install faster-whisper` also works if pip is on PATH) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill marlow-agent-setup`"
        ...
        if event.media_urls and event.message_type == MessageType.DOCUMENT:
            ...
            for i, path in enumerate(event.media_urls):
                ...
                # cache directories are auto-mounted at /root/.marlow/cache/* by get_cache_directory_mounts().
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`marlow gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            ...
            atomic_json_write(
                _marlow_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        except Exception as e:
        ...
        try:
            ...
            atomic_json_write(
                _marlow_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    def _is_stale_restart_redelivery(self, event: MessageEvent) -> bool:
        ...
        try:
            marker_path = _marlow_home / ".restart_last_processed.json"
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from marlow_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        import yaml
        from marlow_cli.model_switch import (
        ...
        if persist_global:
            try:
                ...
                from marlow_cli.config import save_config
                save_config(cfg)
            except Exception as e:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Marlow default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_marlow_home())
    ...
    def _goal_max_turns_from_config(self) -> int:
        ...
        therefore only available through marlow_cli.config.load_config().
        """
        try:
            goals_cfg = (
        ...
        if session_key and hasattr(adapter, "register_post_delivery_callback"):
            try:
                ...
                if active is not None:
                    generation = getattr(active, "_marlow_run_generation", None)
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _marlow_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _ensure_telegram_system_topic(self, source: SessionSource) -> None:
        ...
        try:
            send_result = await adapter.send(
                source.chat_id,
                "System topic for Marlow commands and status.",
    ...
    async def _rename_telegram_topic_for_session_title(
        ...
        """Best-effort rename of a Telegram DM topic when Marlow auto-titles a session."""
    ...
    def _telegram_topic_help_text(self) -> str:
        return (
            ...
            "\n"
            "How it works:\n"
            "1. Run /topic once in this DM — Marlow checks BotFather Threads\n"
            "   Settings are enabled and flips on multi-session mode.\n"
            "2. Tap All Messages at the top of the bot and send any message.\n"
            "   Telegram creates a new topic for that message; each topic is\n"
            "   an independent Marlow session (fresh history, fresh context).\n"
    ...
    def _disable_telegram_topic_mode_for_chat(self, source: SessionSource) -> str:
        ...
        return (
            ...
            "normal Marlow chat again. Run /topic to re-enable later."
        )
    ...
    def _telegram_topic_root_status_message(self, source: SessionSource) -> str:
        lines = [
            ...
            "",
            "To create a new Marlow chat, open All Messages at the top of this "
    ...
    async def _restore_telegram_topic_session(self, event: MessageEvent, raw_session_id: str) -> str:
        """Restore an existing Telegram-owned Marlow session into this topic."""
        source = event.source
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``marlow debug share`` from the CLI.
        """
        import asyncio
        from marlow_cli.debug import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Marlow Agent to the latest version.

        Spawns ``marlow update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``marlow update`` may trigger. Marker
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Marlow Agent')}"
        ...
        marlow_cmd = _resolve_marlow_bin()
        if not marlow_cmd:
            return t("gateway.update.marlow_cmd_not_found")

        pending_path = _marlow_home / ".update_pending.json"
        output_path = _marlow_home / ".update_output.txt"
        exit_code_path = _marlow_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `marlow update --gateway` detached so it survives gateway restart.
    ...
    async def _watch_update_progress(
        ...
        """Watch ``marlow update --gateway``, streaming output + forwarding prompts.
        ...
        """
        pending_path = _marlow_home / ".update_pending.json"
        claimed_path = _marlow_home / ".update_pending.claimed.json"
        output_path = _marlow_home / ".update_output.txt"
        exit_code_path = _marlow_home / ".update_exit_code"
        prompt_path = _marlow_home / ".update_prompt.json"
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ Marlow update finished.", metadata=metadata)
                    else:
                        await adapter.send(
                            chat_id,
                            "❌ Marlow update failed (exit code {}).".format(exit_code),
                            metadata=metadata,
                        )
                ...
                (_marlow_home / ".update_response").unlink(missing_ok=True)
        ...
        if not exit_code_path.exists():
            ...
            try:
                await adapter.send(
                    chat_id,
                    "❌ Marlow update timed out after 30 minutes.",
                    metadata=metadata,
                )
            except Exception:
                pass
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Marlow update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Marlow update failed.\n\n```\n{output}\n```"
                elif exit_code == 0:
                    msg = "✅ Marlow update finished successfully."
                else:
                    msg = "❌ Marlow update failed. Check the gateway logs or run `marlow update` manually for details."
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        """Notify the chat that initiated /restart that the gateway is back."""
        notify_path = _marlow_home / ".restart_notify.json"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        *,
        ...
        message = "♻️ Gateway online — Marlow is back and ready."
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called marlow-agent-setup "
                                "that can help users configure Marlow features "
    ...
    def _bind_adapter_run_generation(
        ...
        try:
            ...
            if interrupt_event is not None:
                setattr(interrupt_event, "_marlow_run_generation", int(generation))
        except Exception:
            pass
    ...
    async def _run_agent(
        ...
        # - Telegram forum topics use message_thread_id; Marlow-created private
        ...
        # MARLOW_AGENT_NOTIFY_INTERVAL env var.  Default 180s (3 min).
        # 0 = disable notifications.
        _NOTIFY_INTERVAL_RAW = _float_env("MARLOW_AGENT_NOTIFY_INTERVAL", 180)
        ...
        try:
            ...
            # MARLOW_AGENT_TIMEOUT env var (env var takes precedence).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = _float_env("MARLOW_AGENT_TIMEOUT", 1800)
            _agent_timeout = _agent_timeout_raw if _agent_timeout_raw > 0 else None
            _agent_warning_raw = _float_env("MARLOW_AGENT_TIMEOUT_WARNING", 900)
            ...
            if pending and pending.strip().startswith("/"):
                ...
                if _pending_cmd_word:
                    try:
                        from marlow_cli.commands import resolve_command as _rc_pending
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, loop=None, interval: int = 60):
    ...
    needing a separate `marlow cron daemon` or system cron entry.

    When ``adapters`` and ``loop`` are provided, passes them through to the
    cron delivery path so live adapters can be used for E2EE rooms.

    Also refreshes the channel directory every 5 minutes and prunes the
    image/audio/document cache + expired ``marlow debug share`` pastes
    once per hour.
    """
    from cron.scheduler import tick as cron_tick
    from gateway.platforms.base import cleanup_image_cache, cleanup_document_cache
    from marlow_cli.debug import _sweep_expired_pastes
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same MARLOW_HOME.
    # The PID file is scoped to MARLOW_HOME, so future multi-profile
    # setups (each profile using a distinct MARLOW_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            try:
                (get_marlow_home() / "gateway.pid").unlink(missing_ok=True)
            except Exception:
                pass
            ...
            logger.error(
                "Another gateway instance is already running (PID %d, MARLOW_HOME=%s). "
                "Use 'marlow gateway restart' to replace it, or 'marlow gateway stop' first.",
                existing_pid, marlow_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'marlow gateway restart' to replace it,\n"
                f"   or 'marlow gateway stop' to kill it first.\n"
                f"   Or use 'marlow gateway run --replace' to auto-replace.\n"
            )
            return False
    ...
    setup_logging(marlow_home=_marlow_home, mode="gateway")
    ...
    def shutdown_signal_handler(received_signal=None):
        ...
        # both services are enabled, e.g. marlow.service + marlow-
        ...
        # Planned stop check: service managers and `marlow gateway stop`
        ...
        if _shutdown_ctx is not None:
            ...
            try:
                _diag_log = _marlow_home / "logs" / "gateway-shutdown-diag.log"
                spawn_async_diagnostic(
