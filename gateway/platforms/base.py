...
# Audio file extensions Marlow recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Marlow' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Marlow-created Telegram private-chat
...
from marlow_constants import get_default_marlow_root, get_marlow_dir, get_marlow_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.marlow/.env manually."
)
...
# Default location: {MARLOW_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_marlow_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_marlow_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_marlow_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_marlow_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_marlow_dir("cache/screenshots", "browser_screenshots")
_MARLOW_HOME = get_marlow_home()
_MARLOW_ROOT = get_default_marlow_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "MARLOW_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "MARLOW_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "MARLOW_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "MARLOW_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _MARLOW_HOME / "image_cache",
    _MARLOW_HOME / "audio_cache",
    _MARLOW_HOME / "video_cache",
    _MARLOW_HOME / "document_cache",
    _MARLOW_HOME / "browser_screenshots",
    ...
    _MARLOW_HOME / "cache" / "images",
    _MARLOW_HOME / "cache" / "audio",
    _MARLOW_HOME / "cache" / "videos",
    _MARLOW_HOME / "cache" / "documents",
    _MARLOW_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Marlow profile and shared Marlow root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for marlow_root in (_MARLOW_HOME, _MARLOW_ROOT):
        denied.append(marlow_root / ".env")
        denied.append(marlow_root / "auth.json")
        denied.append(marlow_root / "credentials")
        denied.append(marlow_root / "config.yaml")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``MARLOW_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Marlow-managed cache, under an operator-allowlisted root
    (``MARLOW_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.marlow/.env,
    # ~/.marlow/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?marlow\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+marlow[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("MARLOW_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "MARLOW_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "MARLOW_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.marlow/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from marlow_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          MARLOW_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          MARLOW_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          MARLOW_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("MARLOW_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("MARLOW_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("MARLOW_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _marlow_run_generation is set on
