...
# group mention gating without custom aliases, use conservative Marlow wake
# words so `require_mention: true` is a one-line enablement path.
DEFAULT_MENTION_PATTERNS = [
    r"(?<![\w@])@?marlow\s+agent\b[,:\-]?",
    r"(?<![\w@])@?marlow\b[,:\-]?",
]
...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> List[re.Pattern]:
        ...
        JSON list, or comma/newline-separated), or None (use Marlow defaults).
        """
        if raw is None:
