"""
Email platform adapter for the Marlow gateway.

Allows users to interact with Marlow by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "marlow-agent" "version" "{_marlow_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Marlow Agent")
        ...
        msg_id = f"<marlow-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
