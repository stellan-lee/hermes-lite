...
There are two ways to add a platform to the Marlow gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.marlow/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Marlow code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `marlow gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `marlow_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Marlow core.
...
And add it to the `marlow-gateway` composite:

```python
"marlow-gateway": {
    "includes": [..., "marlow-your-platform"]
}
```
...
## 12. Status Display (`marlow_cli/status.py`)
...
## 13. Gateway Setup Wizard (`marlow_cli/gateway.py`)
