...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if marlow_home_str:
            takeover_path = Path(marlow_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(marlow_home_str) / ".gateway-planned-stop.json"
...
def spawn_async_diagnostic(
    ...
    *,
    ...
    # Inline shell so we don't have to ship a helper script.
    script = (
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded marlow-agent but never re-ran ``marlow setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../marlow-gateway.service"
    ...
    # that's the common case for marlow.
    ...
    # constant in marlow_cli/gateway.py.
    headroom = 30.0
