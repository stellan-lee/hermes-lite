...
_FORCE_SYNC_ENV = "MARLOW_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.marlow") -> list[tuple[str, str]]:
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``MARLOW_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, marlow_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.marlow/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="marlow-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.marlow/skills/a.md`` →
        ``~/.marlow/skills/a.md``, a new remote file at
        ``/root/.marlow/skills/b.md`` maps to ``~/.marlow/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
