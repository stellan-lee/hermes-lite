...
# Marlow-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Marlow inference secret exactly
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("MARLOW_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\marlow\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\marlow\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _marlow_portable_git = os.path.join(_local_appdata, "marlow", "git") if _local_appdata else ""
    if _marlow_portable_git:
        for candidate in (
            os.path.join(_marlow_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_marlow_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Marlow Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set MARLOW_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    # Marlow is driving via _find_bash (Git Bash), and Git Bash itself
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``MARLOW_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under MARLOW_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_marlow_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "marlow_terminal"
