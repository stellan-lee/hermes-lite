...
# Must match marlow_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (marlow-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("marlow-")
...
def _get_max_async_children() -> int:
    """Return the configured cap for detached background delegations."""
    cfg = _load_config()
    ...
    if val is not None:
        try:
            ...
            logger.warning(
                "delegation.max_async_children=%r is invalid; using %d",
                val,
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``marlow-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. marlow-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.marlow/logs/subagent-<sid>-<ts>.log``
...
def delegate_task(
    ...
    # Background delegation applies to both single tasks and batches. A batch
    # is one detached unit: its children run concurrently and their summaries
    # re-enter the conversation together as one consolidated completion.
    ...
    if background:
        ...
        try:
            ...
            _origin_ui_session_id = get_session_env("MARLOW_UI_SESSION_ID") or ""
        except Exception:
        ...
        # Pool at capacity / schedule failure: restore parent ownership before
        # running inline so a parent interrupt still propagates to the children.
        if hasattr(parent_agent, "_active_children"):
            _ac_lock = getattr(parent_agent, "_active_children_lock", None)
            if _ac_lock:
                with _ac_lock:
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'marlow auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (marlow_cli/config.py load_config()) so that
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "you and the user keep working. A single task re-enters when it finishes; "
        "a batch runs its children concurrently and re-enters once with their "
        "consolidated results. Do NOT wait or poll; just continue with other "
        ...
        "notify_on_complete=True) instead. Background delegations persist "
        "completed results across process restarts, but if the process exits "
        "before a subagent finishes, that subagent's work is discarded; "
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Marlow subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
...
def _model_background_value(args: dict, parent_agent=None) -> bool:
    ...
    batch (one detached unit with a consolidated completion). The one
