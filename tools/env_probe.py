...
machine as Marlow itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Marlow ships under one Python (e.g. 3.11 in a bundled venv) while the
...
Remote terminal backends (Docker and SSH) are skipped: the
