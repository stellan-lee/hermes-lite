...
def _db_path():
    return get_marlow_home() / "delegations.db"
...
def dispatch_async_delegation(
    *,
    ...
    try:
        ...
        # get_marlow_home() under the right profile.
...
def interrupt_for_session(
    ...
    - ``parent_session_id`` / ``parent_session_ids``: one durable parent id or
      the compression-only lineage that owns the ending gateway chat.  The
      gateway route key survives ``/new``, so it is intentionally not used
      there as the sole cancellation selector.

    Returns how many were interrupted.
    """
    durable_parent_ids = {
        str(value)
        for value in (parent_session_ids or [])
        if str(value)
    }
    if parent_session_id:
        durable_parent_ids.add(str(parent_session_id))
    if not session_key and not origin_ui_session_id and not durable_parent_ids:
        return 0
    count = 0
    with _records_lock:
        targets = [
            ...
            and (
                ...
                or (
                    durable_parent_ids
                    and str(r.get("parent_session_id") or "") in durable_parent_ids
                )
            )
        ]
    for r in targets:
