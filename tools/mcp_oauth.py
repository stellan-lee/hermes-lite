...
This module provides the glue:
    - ``MarlowTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Marlow Agent"
"""
...
_USER_SKIPPED_SENTINEL = "__marlow_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses MARLOW_HOME so each profile gets its own OAuth tokens.
    Layout: ``MARLOW_HOME/mcp-tokens/``
    """
    try:
        from marlow_constants import get_marlow_home
        base = Path(get_marlow_home())
    except ImportError:
        base = Path(os.environ.get("MARLOW_HOME", str(Path.home() / ".marlow")))
...
# ---------------------------------------------------------------------------
# MarlowTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class MarlowTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        MARLOW_HOME/mcp-tokens/<server_name>.json         -- tokens
        MARLOW_HOME/mcp-tokens/<server_name>.client.json   -- client info
        MARLOW_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Marlow records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Marlow.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `marlow mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Marlow Agent")
    scope = cfg.get("scope")
