...
runs on the host machine unless TERMINAL_ENV=local. For Docker and SSH
backends, the command runs inside the sandbox.
...
CHECKPOINT_PATH = get_marlow_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        For Docker/SSH: runs the command inside the sandbox
        ...
        log_path = f"{temp_dir}/marlow_bg_{session.id}.log"
        pid_path = f"{temp_dir}/marlow_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/marlow_bg_{session.id}.exit"
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `marlow update` restarting the gateway) is still
