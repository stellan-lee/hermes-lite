...
selectable via ``marlow tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `marlow tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `marlow tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`marlow setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `marlow tools` "
        "→ Image Generation (run `marlow plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping marlow with a FAL key configured
...
IMAGE_GENERATE_SCHEMA = {
    "name": "image_generate",
    "description": (
        "Generate high-quality images from text, or edit an existing image "
        "when the active model supports it. Pass `image_url` for the primary "
        "source and `reference_image_urls` for additional references; omit "
        "both for text-to-image. The backend and model are user-configured. "
        "Returns a URL or absolute file path in the `image` field."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "prompt": {
                "type": "string",
                "description": "The desired image or the edit to apply. Be detailed and descriptive.",
            },
            "aspect_ratio": {
                "type": "string",
            ...
            "image_url": {
                "type": "string",
                "description": "Optional primary source image URL or absolute local path to edit.",
            },
            "reference_image_urls": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Optional additional style, character, or composition references.",
            },
        },
        "required": ["prompt"],
    },
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `marlow plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `marlow tools` → Image Generation."
                ),
...
def _active_image_capabilities() -> Dict[str, Any]:
    """Return the active provider/model's declared image modalities."""
        ...
        try:
            ...
            if provider is not None:
                ...
                info.update(
                    ...
                    model=_read_configured_image_model() or provider.default_model() or "",
                    modalities=list(capabilities.get("modalities") or ["text"]),
    ...
    try:
        model_id, meta = _resolve_fal_model()
        info.update(provider="FAL.ai", model=meta.get("display", model_id))
...
def _build_dynamic_image_schema() -> Dict[str, Any]:
    """Describe edit capability for the currently configured backend."""
    info = _active_image_capabilities()
    provider = info.get("provider")
    model = info.get("model")
    modalities = set(info.get("modalities") or ["text"])
    lines = [_GENERIC_IMAGE_DESCRIPTION]
    if provider or model:
        lines.append(
            "Active backend: "
            + " · ".join(str(value) for value in (provider, model) if value)
        )
    if "image" in modalities:
        maximum = int(info.get("max_reference_images") or 1)
        lines.append(
            f"- supports image-to-image editing with up to {maximum} source/reference image(s)"
        )
    else:
        lines.append("- text-to-image only; do not pass image_url or reference_image_urls")
    return {"description": "\n".join(lines)}
