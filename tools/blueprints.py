...
`marlow skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.marlow.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.marlow.blueprint`` is a mapping containing
    ...
    marlow = meta.get("marlow") if isinstance(meta, dict) else None
    blueprint = marlow.get("blueprint") if isinstance(marlow, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.marlow.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.marlow.blueprint`` block) they can hand
    to ``marlow skills publish`` to share. ``body`` is the plain-language
