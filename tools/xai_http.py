...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``marlow tools`` repaint, tool-registration scans,
    ...
    2. ``~/.marlow/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_marlow_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.marlow/.env`` first, then ``os.environ``.

    Wraps :func:`marlow_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from marlow_cli.config import get_env_value as _marlow_get_env_value

        value = _marlow_get_env_value(name)
...
def marlow_xai_user_agent() -> str:
    """Return a stable Marlow-specific User-Agent for xAI HTTP calls."""
    try:
        from marlow_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Marlow-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Marlow-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``marlow_cli.config.get_env_value`` so keys
    stored in ``~/.marlow/.env`` (the standard Marlow location) are honored —
