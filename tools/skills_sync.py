...
Copies bundled skills from the repo's skills/ directory into ~/.marlow/skills/
...
The manifest lives at ~/.marlow/skills/.bundled_manifest.
"""
...
from marlow_constants import get_bundled_skills_dir, get_marlow_home, get_optional_skills_dir
...
# Marker file written by `marlow profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.marlow profile).
# When present in MARLOW_HOME, sync_skills() is a no-op so neither the
# installer, `marlow update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# marlow_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks MARLOW_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.marlow/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.marlow/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.marlow/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.marlow) that wrote the
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.marlow/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `marlow update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `marlow skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `marlow skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `marlow update` runs "
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes MARLOW_HOME/.no-bundled-skills so the
    installer, ``marlow update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``marlow skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = MARLOW_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `marlow update` (or `marlow skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (MARLOW_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.marlow/skills/ ...")
