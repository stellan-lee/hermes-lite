#!/usr/bin/env python3
"""
Marlow Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Marlow Agent, inspired by Claude Code.
...
os.environ["MARLOW_QUIET"] = "1"  # Our own modules

import yaml

from marlow_cli.fallback_config import get_fallback_chain
...
try:
    from marlow_cli.pt_input_extras import (
...
from marlow_cli.banner import _format_context_length, format_banner_version_label
...
from marlow_cli.browser_connect import (
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.marlow/.
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.marlow/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If MARLOW_IGNORE_USER_CONFIG=1 is set (via ``marlow chat --ignore-user-config``),
    the user config at ``~/.marlow/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({MARLOW_HOME}/config.yaml)
    user_config_path = _marlow_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("MARLOW_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Marlow, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Marlow - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see marlow_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and MarlowCLI.__init__ checks "default" first.
    ...
    from marlow_cli.config import _expand_env_vars
    ...
    # UNLESS we're inside a gateway process (detected by _MARLOW_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_MARLOW_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["MARLOW_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.marlow/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from marlow_logging import setup_logging
...
try:
    from marlow_cli.config import print_config_warnings
...
try:
    from marlow_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``marlow --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from marlow_cli.mcp_startup import wait_for_mcp_discovery
...
from marlow_cli.banner import build_welcome_banner
from marlow_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from marlow_cli.callbacks import prompt_for_secret
...
def _reset_terminal_input_modes_on_exit() -> None:
    ...
    SIGTERM/SIGHUP. ``kill -9`` is uncatchable.
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run marlow -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"marlow-{short_id}"
    branch_name = f"marlow/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`marlow_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from marlow_cli.config import load_config as _load_full_config
        from marlow_constants import get_marlow_home as _get_marlow_home
        _marlow_home_maint = _get_marlow_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_marlow_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`marlow_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``marlow/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("marlow-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``marlow/marlow-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``marlow -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("marlow/marlow-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. MARLOW_LIGHT / MARLOW_TUI_LIGHT env (true/false) — explicit override
#   2. MARLOW_TUI_THEME=light|dark — explicit theme
#   3. MARLOW_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Marlow assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("MARLOW_LIGHT", "MARLOW_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("MARLOW_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("MARLOW_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from marlow_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_marlow_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._marlow_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Marlow output it's common to display cron schedules verbatim.
    #
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_marlow_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_marlow_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._marlow_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_marlow_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._marlow_bp_start = now
        ...
        _vt100_mod._marlow_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``MarlowCLI._busy_command()``.
        """
        yield self

# ASCII Art - MARLOW-AGENT logo (full width, single line - requires ~95 char terminal)
MARLOW_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Marlow Caduceus (compact, fits in left panel)
MARLOW_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS MARLOW - AI Agent Framework"
        tiny_line = "⚕ NOUS MARLOW"
    else:
        agent_name = _skin.get_branding("agent_name", "Marlow Agent") if _skin else "Marlow Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("MARLOW_FAST_STARTUP_BANNER") == "1":
        from marlow_cli import __release_date__ as _release_date
        from marlow_cli import __version__ as _version

        version_line = f"Marlow Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from marlow_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.marlow/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.marlow/config.yaml on first use)
...
class MarlowCLI:
    """
    Interactive CLI for the Marlow Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from marlow_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("MARLOW_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        # by `marlow chat --ignore-rules` in marlow_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("MARLOW_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("MARLOW_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Marlow processes for this MARLOW_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.marlow/checkpoints/.  Opt-in via
        ...
        self._history_file = _marlow_home / ".marlow_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from marlow_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Marlow'}"
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from marlow_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Marlow")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - MarlowCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from marlow_cli.runtime_provider import (
        ...
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: marlow setup")
            return False
        ...
        # `marlow chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `marlow auth add openai-codex`
        # without `marlow model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from marlow_cli.models import get_default_model_for_provider
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (marlow sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (marlow sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``marlow doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``marlow doctor`` so the banner stays
        small.
        """
        try:
            from marlow_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Marlow needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Marlow: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Marlow: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.marlow/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from marlow_cli.clipboard import save_clipboard_image

        img_dir = get_marlow_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: marlow --checkpoints")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'marlow setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from marlow_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from marlow_cli.skin_engine import get_active_help_header
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in marlow_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from marlow_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("MARLOW_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `marlow gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `marlow sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from marlow_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `marlow sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.marlow/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``marlow --resume <id>``
        ...
        saved_dir = get_marlow_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"marlow_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: marlow --resume {self.session_id}")
        except Exception as e:
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `marlow model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from marlow_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from marlow_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from marlow_cli.model_switch import switch_model, parse_model_flags
        from marlow_cli.providers import get_label
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to marlow_cli.curator so the CLI and the `marlow curator`
        ...
        try:
            from marlow_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_marlow_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_marlow_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in marlow_cli/commands.py instead of touching every dispatch site.
        from marlow_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from marlow_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from marlow_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_marlow_home()}/plugins/ to get started.")
                else:
            ...
            # Check for plugin-registered slash commands
                ...
                from marlow_cli.plugins import (
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    _PendingCLIInput(
                        ...
                        "just because CDP is connected. This is typically a Marlow-managed isolated debug "
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from marlow_cli.goals import GoalManager
            from marlow_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Marlow keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from marlow_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_marlow_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from marlow_cli.config import load_config
        from marlow_cli.colors import Colors as _Colors
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``MARLOW_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``MARLOW_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Marlow is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Marlow is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Marlow is busy."
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from marlow_cli.partial_compress import (
    ...
    def _handle_debug_command(self):
        ...
        from marlow_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Marlow Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``marlow update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Marlow Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Marlow Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Marlow Agent",
            detail="This will exit the current session and run `marlow update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from marlow_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.marlow/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_marlow_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "marlow_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "marlow_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _marlow_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard MARLOW_HOME, so no -p needed.
            try:
                from marlow_cli.profiles import get_active_profile_name
            ...
            print(f"  marlow --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  marlow -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from marlow_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from marlow_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from marlow_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Marlow Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Marlow Agent! Type your message or /help for commands."
        ...
        try:
            _redact_raw = os.getenv("MARLOW_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(MARLOW_REDACT_SECRETS={_redact_raw}). "
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in marlow_cli.pt_input_extras
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_marlow_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Marlow binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from marlow_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Marlow Agent")
        ...
        # Dynamic prompt: shows Marlow symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Marlow needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Marlow needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Marlow needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = MarlowCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_marlow_osd_patched", False):
                ...
                _pt_renderer._marlow_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``MARLOW_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("MARLOW_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: marlow setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Marlow Agent CLI - Interactive AI Assistant
    ...
    os.environ["MARLOW_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Marlow Gateway (messaging platforms)...")
    ...
    # Default to marlow-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside MarlowCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  MARLOW_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Marlow" box, no tool-gen
            ...
            # Single-query mode (`marlow chat -q "…"`): skip the welcome
