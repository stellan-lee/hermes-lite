"""Centralized logging setup for Marlow Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.marlow/logs/`` (profile-aware via ``get_marlow_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``marlow_cli.web_server``,
    ``marlow_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from marlow_constants import get_config_path, get_marlow_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_marlow_session_injector", False):
    ...
    _session_record_factory._marlow_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``marlow logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "marlow_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("marlow_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "marlow_cli.web_server",
        "marlow_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Marlow logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    marlow_home
        Override for the Marlow home directory.  Falls back to
        ``get_marlow_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_marlow_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._marlow_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the marlow group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from marlow_cli.config import is_managed
