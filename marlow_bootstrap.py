"""Windows UTF-8 bootstrap for Marlow entry points.
...
should be imported at the very top of every Marlow entry point
(``marlow``, ``marlow-agent``, ``marlow-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import marlow_bootstrap``
# (or ``from marlow_bootstrap import apply_windows_utf8_bootstrap``) at
