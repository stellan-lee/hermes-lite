...
This restored module keeps the original Codex runtime state machine available
without restoring the removed gateway, MCP, or plugin-migration surfaces.
"""
...
class CodexRuntimeStatus:
    """Result of a /codex-runtime invocation."""
...
def apply(
    ...
    *,
    ...
    """Apply or inspect the retained Codex app-server runtime setting.
    ...
    if new_value is None:
        ...
        unavailable = ver or "install with `npm i -g @openai/codex`"
        binary_status = f"OK {ver}" if ok else f"not available — {unavailable}"
        msg = f"openai_runtime: {current}\ncodex CLI: {binary_status}"
