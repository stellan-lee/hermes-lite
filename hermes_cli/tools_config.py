...
from hermes_cli.cli_output import print_error as _print_error, print_info as _print_info, print_success as _print_success, print_warning as _print_warning
...
def _get_platform_tools(config,platform,*,include_default_mcp_servers=True):
 ...
 selected=[str(x) for x in raw] if explicit else [PLATFORMS.get(platform,{}).get("default_toolset",f"hermes-{platform}")]
