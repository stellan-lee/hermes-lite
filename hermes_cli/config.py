...
def is_managed() -> bool:
    ...
    The ``HERMES_MANAGED`` environment variable names the package manager.
    """
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Hermes was installed: docker, Homebrew, git, or pip.

    Resolution order:
    1. Stamped ``~/.hermes/.install_method`` file (written by installers)
    2. HERMES_MANAGED environment variable (Homebrew)
...
DEFAULT_CONFIG = {
    ...
    "display": {
        ...
        # Hermes Lite ships English UI strings.
        "language": "en",
...
OPTIONAL_ENV_VARS = {'LM_API_KEY': {'description': 'LM Studio bearer token for auth-enabled local servers',
 ...
 'HONCHO_BASE_URL': {'description': 'Base URL for self-hosted Honcho instances (no API key needed)',
                     ...
                     'category': 'tool'},
 'HERMES_LANGFUSE_PUBLIC_KEY': {'description': 'Langfuse project public key (pk-lf-...)',
                                'prompt': 'Langfuse public key',
                                'url': 'https://cloud.langfuse.com',
                                'password': False,
                                'category': 'tool'},
 'HERMES_LANGFUSE_SECRET_KEY': {'description': 'Langfuse project secret key (sk-lf-...)',
                                'prompt': 'Langfuse secret key',
                                'url': 'https://cloud.langfuse.com',
                                'password': True,
                                'category': 'tool'},
 'HERMES_LANGFUSE_BASE_URL': {'description': 'Langfuse server URL (default: '
 ...
 'SUDO_PASSWORD': {'description': 'Sudo password for terminal commands requiring root access; set '
                   ...
                   'password': True,
                   'category': 'setting'},
 'HERMES_MAX_ITERATIONS': {'description': 'Maximum tool-calling iterations per conversation '
                                          '(default: 90)',
                           'prompt': 'Max iterations',
                           'url': None,
                           'password': False,
                           'category': 'setting'},
 'HERMES_PREFILL_MESSAGES_FILE': {'description': 'Path to JSON file with ephemeral prefill '
                                                 'messages for few-shot priming',
                                  'prompt': 'Prefill messages file path',
                                  'url': None,
                                  'password': False,
                                  'category': 'setting'},
 'HERMES_EPHEMERAL_SYSTEM_PROMPT': {'description': 'Ephemeral system prompt injected at API-call '
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if providers and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "providers defined but no 'model' section — Hermes won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            ...
            print()
            print("Examples:")
            print("  hermes config set model gpt-5.3-codex")
            print("  hermes config set terminal.backend docker")
            print("  hermes config set LM_API_KEY local-token")
            sys.exit(1)
...
# coupling core Hermes to a connector.
#
