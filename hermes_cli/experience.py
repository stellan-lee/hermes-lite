...
def _state_db_path() -> Path:
    ...
    return Path(get_hermes_home()).expanduser().resolve() / "state.db"
...
def _cmd_why_last(args: argparse.Namespace) -> int:
    ...
    if diagnostic is _LATEST_RETRIEVAL_UNAVAILABLE:
        print("Latest recall diagnostics are not available in this Hermes build.")
        return 0
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    """Attach ``hermes experience`` governance commands to *parent*."""
