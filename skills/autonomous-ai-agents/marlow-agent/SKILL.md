...
Marlow Agent is an open-source AI agent framework by Nous Research that runs in your terminal, messaging platforms, and IDEs. It belongs to the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Marlow works with any LLM provider (OpenRouter, Anthropic, OpenAI, DeepSeek, local models, and 15+ others) and runs on Linux, macOS, and WSL.
...
marlow auth                 Interactive credential manager
marlow auth add PROVIDER    Add OAuth or API-key credential (e.g. nous, openai-codex, qwen-oauth)
marlow auth list            List stored credentials
marlow auth remove PROVIDER Remove a stored credential
...
marlow skills list          List installed skills
marlow skills search QUERY  Search the skills hub
marlow skills install ID    Install a skill (ID can be a hub identifier OR a direct https://…/SKILL.md URL; pass --name to override when frontmatter has no name)
marlow skills inspect ID    Preview without installing
marlow skills config        Enable/disable skills per platform
marlow skills check         Check for updates
marlow skills update        Update outdated skills
marlow skills uninstall N   Remove a hub skill
marlow skills publish PATH  Publish to registry
marlow skills browse        Browse all available skills
marlow skills tap add REPO  Add a GitHub repo as skill source
```
...
```
marlow auth add             Interactive credential wizard
marlow auth list [PROVIDER] List pooled credentials
marlow auth remove P INDEX  Remove by provider + index
marlow auth reset PROVIDER  Clear exhaustion status
```
...
marlow acp                  ACP server (IDE integration)
marlow claw migrate         Migrate from OpenClaw
...
20+ providers supported. Set via `marlow model` or `marlow setup`.

| Provider | Auth | Key env var |
|----------|------|-------------|
| OpenRouter | API key | `OPENROUTER_API_KEY` |
| Anthropic | API key | `ANTHROPIC_API_KEY` |
| Nous Portal | OAuth | `marlow auth` |
| OpenAI Codex | OAuth | `marlow auth` |
...
| Qwen OAuth | OAuth | `marlow auth add qwen-oauth` |
...
Marlow runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
without BOM. `marlow config edit` writes without BOM; manual edits in
...
Marlow's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Marlow-installed venv at
...
every Marlow tool and most Windows APIs. Prefer forward slashes in code
...
2. `marlow auth` — re-authenticate OAuth providers (or `marlow auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `marlow model` → GitHub Copilot.
...
### Skills not showing
1. `marlow skills list` — verify installed
...
| Skills catalog | `marlow skills browse` or [Skills catalog](https://marlow-agent.nousresearch.com/docs/reference/skills-catalog) |
...
**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The Marlow-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/marlow_cli/test_auth_toctou_file_modes.py`)
- `signal.SIGALRM` → Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
