...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``marlow://`` deep-link
...
_DELIVER = BlueprintSlot(
    ...
    default="origin", options=("origin", "local", "all", "telegram", "discord", "email"),
    ...
    "all = every configured delivery target; "
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``marlow://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"marlow://blueprint/{quote(blueprint.key)}{qs}"
