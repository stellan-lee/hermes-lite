...
def _store_lock():
    return interprocess_file_lock(SUGGESTIONS_FILE.with_suffix(".json.lock"))
...
def add_suggestion(
    *,
    ...
    with _suggestions_lock, _store_lock():
        ...
        for existing in suggestions:
            if existing.get("dedup_key") == dedup_key:
                if existing.get("status") in (_STATUS_DISMISSED, _STATUS_ACCEPTED):
                    return None
                if existing.get("status") in (_STATUS_PENDING, _STATUS_ACCEPTING):
                    return None
...
def _set_status(suggestion_id: str, status: str, *, expected: Optional[str] = None) -> bool:
    with _suggestions_lock, _store_lock():
        suggestions = _load_raw().get("suggestions", [])
        changed = False
        for s in suggestions:
...
def _claim_pending(ref: str) -> Optional[Dict[str, Any]]:
    """Resolve and atomically move one pending suggestion to accepting."""
    with _suggestions_lock, _store_lock():
        suggestions = _load_raw().get("suggestions", [])
        pending = [s for s in suggestions if s.get("status") == _STATUS_PENDING]
        claimed = next((s for s in suggestions if s.get("id") == ref), None)
        if claimed is None and ref.isdigit():
            index = int(ref) - 1
            claimed = pending[index] if 0 <= index < len(pending) else None
        if claimed is None:
            claimed = next(
                (s for s in suggestions if s.get("title", "").lower() == ref.lower()),
                None,
            )
...
def dismiss_suggestion(ref: str) -> bool:
    ...
    with _suggestions_lock, _store_lock():
        suggestions = _load_raw().get("suggestions", [])
        pending = [s for s in suggestions if s.get("status") == _STATUS_PENDING]
        resolved = next((s for s in suggestions if s.get("id") == ref), None)
        if resolved is None and ref.isdigit():
            index = int(ref) - 1
            resolved = pending[index] if 0 <= index < len(pending) else None
        if resolved is None:
            resolved = next(
                (s for s in suggestions if s.get("title", "").lower() == ref.lower()),
                None,
...
def accept_suggestion(ref: str, *, origin: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # The claim is not an acknowledgement. Restore pending so the user can
        # retry after a transient job-store failure.
        _set_status(s["id"], _STATUS_PENDING, expected=_STATUS_ACCEPTING)
        raise
    _set_status(s["id"], _STATUS_ACCEPTED, expected=_STATUS_ACCEPTING)
    return job
