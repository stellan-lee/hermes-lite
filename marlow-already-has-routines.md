# Marlow Agent Has Had "Routines" Since March
...
Marlow equivalent — works today:
```bash
marlow cron create "0 2 * * *" \
...
Marlow equivalent — works today:
```bash
marlow webhook subscribe auth-watch \
  --events "pull_request" \
...
Marlow equivalent — works today:
```bash
marlow webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Marlow implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Marlow Agent |
|---|---|---|
...
## Things Marlow Does That Routines Can't
...
Marlow has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Marlow Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install marlow-agent
marlow setup
```

Set up a scheduled task in 30 seconds:
```bash
marlow cron create "0 9 * * 1" \
...
```bash
marlow gateway setup    # enable webhooks
marlow webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [marlow-agent.nousresearch.com/docs/guides/automation-templates](https://marlow-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [marlow-agent.nousresearch.com](https://marlow-agent.nousresearch.com)

GitHub: [github.com/NousResearch/marlow-agent](https://github.com/NousResearch/marlow-agent)

---

*Marlow Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
