...
``{title, url, description, position}`` rows every other Marlow web
provider produces.
...
prefers Marlow-managed xAI Grok OAuth (via ``marlow auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.marlow/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``marlow tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `marlow auth` to sign in with "
