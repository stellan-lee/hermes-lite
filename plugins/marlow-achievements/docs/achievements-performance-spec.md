# Marlow Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: marlow-achievements plugin
...
### `GET /api/plugins/marlow-achievements/achievements`
...
### `POST /api/plugins/marlow-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/marlow-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/marlow-achievements/overview`
...
- Persist plugin data under:
  - `~/.marlow/plugins/marlow-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.marlow/plugins/marlow-achievements/scan_checkpoint.json`
...
- `~/.marlow/plugins/marlow-achievements/`
