...
def _provider_usable_reference(source: str) -> str:
    """Return a public/data URL, encoding a local image as a data URL."""
    source = source.strip()
    if source.lower().startswith(("http://", "https://", "data:")):
        return source
    path = Path(source).expanduser()
    if not path.is_file():
        raise ValueError(f"Krea reference image not found: {source}")
    mime = mimetypes.guess_type(path.name)[0] or "image/png"
    if not mime.startswith("image/"):
        raise ValueError(f"Krea reference must be an image file: {source}")
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            style_refs = [
                _provider_usable_reference(ref) if isinstance(ref, str) else ref
