...
```bash
marlow memory setup    # select "supermemory"
```

Or manually:

```bash
marlow config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.marlow/.env
```

## Config

Config file: `$MARLOW_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `marlow` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `marlow-{identity}` → `marlow-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Marlow profile:

```json
{
  "container_tag": "marlow-{identity}"
}
```

For a profile named `coder`, this resolves to `marlow-coder`. The default profile resolves to `marlow-default`. Without `{identity}`, all profiles share the same container.
