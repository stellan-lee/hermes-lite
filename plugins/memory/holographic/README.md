...
```bash
marlow memory setup    # select "holographic"
```

Or manually:
```bash
marlow config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.marlow-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$MARLOW_HOME/memory_store.db` | SQLite database path |
