...
```bash
marlow memory setup honcho   # configure Honcho directly (works on a fresh install)
marlow memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
marlow config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.marlow/.env
```

> `marlow honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `marlow memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MARLOW_HOME/honcho.json` | Profile-local (isolated Marlow instances) |
| 2 | `~/.marlow/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Marlow profile: `marlow` (default) or `marlow_<profile>`.
...
**Deployment shapes** (`marlow memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Marlow to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Marlow session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`marlow-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`marlow`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-marlow-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening marlow in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Marlow session ID (timestamp + hex). Every `marlow` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Marlow profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`marlow`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Marlow profile: `marlow` (default) or `marlow_<profile>` (e.g. `marlow -p coder` -> host key `marlow_coder`). Older `marlow.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `MARLOW_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `marlow memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `marlow honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `marlow memory setup`) |
| `marlow honcho status` | Show resolved config for active profile |
| `marlow honcho enable` / `disable` | Toggle Honcho for active profile |
| `marlow honcho mode <mode>` | Change recall or observation mode |
| `marlow honcho peer --user <name>` | Update user peer name |
| `marlow honcho peer --ai <name>` | Update AI peer name |
| `marlow honcho tokens --context <N>` | Set context token budget |
| `marlow honcho tokens --dialectic <N>` | Set dialectic max chars |
| `marlow honcho map <name>` | Map current directory to a session name |
| `marlow honcho sync` | Create host blocks for all Marlow profiles |
