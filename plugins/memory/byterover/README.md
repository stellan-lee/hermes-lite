...
```bash
marlow memory setup    # select "byterover"
```

Or manually:
```bash
marlow config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.marlow/.env
```
...
Working directory: `$MARLOW_HOME/byterover/` (profile-scoped).
