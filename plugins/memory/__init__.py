...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with marlow-agent)
2. User-installed providers: ``$MARLOW_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$MARLOW_HOME/plugins/`` or None if unavailable."""
    try:
        from marlow_constants import get_marlow_home
        d = get_marlow_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$MARLOW_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_marlow_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_marlow_user_memory.{active_provider}.cli"
    try:
