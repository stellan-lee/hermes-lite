...
Auto-tracks and cleans up ephemeral files created during Marlow Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$MARLOW_HOME` and `/tmp/marlow-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MARLOW_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under MARLOW_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `MARLOW_HOME` or `/tmp/marlow-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MARLOW_HOME/disk-cleanup/` is itself excluded
- `$MARLOW_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
