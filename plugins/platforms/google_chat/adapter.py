...
# ``model_tools`` import time, so ``marlow status``, ``marlow chat``, etc.
...
# ``marlow_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Marlow is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a marlow MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${MARLOW_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _marlow_home = _get_marlow_home()
        except (ModuleNotFoundError, ImportError):
            _marlow_home = _Path.home() / ".marlow"
        self._thread_count_store = _ThreadCountStore(
            _marlow_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Marlow is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("MARLOW_HOME", str(_Path.home() / ".marlow"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${MARLOW_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a marlow MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Marlow is thinking…' marker message.
        ...
        "Marlow is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Marlow is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Marlow is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Marlow is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Marlow is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``marlow setup``.

    The setup wizard at ``marlow_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.marlow/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. marlow-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.marlow/.env")
    print_info("Restart the gateway: marlow gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``marlow cron`` running as a
    separate process from ``marlow gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Marlow plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'marlow-agent[google_chat]'",
        ...
        # the "Marlow is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Marlow is thinking…' marker message "
