"""
LINE Messaging API platform adapter for Marlow Agent.
...
to Marlow Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``MARLOW_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            marlow_home = Path.home().joinpath(".marlow").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``marlow status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``marlow status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``marlow setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.marlow/.env`` via ``marlow_cli.config``.
    """
    print()
    ...
    try:
        from marlow_cli.config import get_env_var, set_env_var
    except ImportError:
        print("marlow_cli.config not available; set LINE_* vars manually in ~/.marlow/.env")
        return
