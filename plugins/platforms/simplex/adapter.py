"""SimpleX Chat platform adapter (Marlow plugin).
...
This adapter ships as a Marlow platform plugin under
``plugins/platforms/simplex/``. The Marlow plugin loader scans the
...
discoverable and `marlow setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Marlow — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"marlow-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``marlow setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.marlow/.env`` via ``marlow_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("marlow_cli.config not available; set SIMPLEX_* vars manually in ~/.marlow/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `marlow gateway status` without
