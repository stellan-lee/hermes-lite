# Marlow Agent Security Policy

This document describes Marlow Agent's trust model, names the one
...
- Environment details (`marlow version`, commit SHA, OS, Python
  version).
...
Marlow Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Marlow Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Marlow Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Marlow Agent's documentation
...
Marlow Agent supports two OS-level isolation postures. They address
...
Marlow Agent supports this in two ways:

- **Marlow Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Marlow Agent's in-process heuristics
...
Marlow Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Marlow Agent
itself. Bugs in Marlow Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Marlow Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Marlow Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Marlow Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  marlow-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Marlow Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Marlow Agent. Bugs in Marlow
...
- Marlow Agent includes supply-chain guards for MCP server
