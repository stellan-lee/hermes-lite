...
def server():
    with patch.dict(
        "sys.modules",
        {
            "marlow_constants": MagicMock(
                get_marlow_home=MagicMock(return_value="/tmp/marlow_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'marlow-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'marlow-release' patched"
    }
