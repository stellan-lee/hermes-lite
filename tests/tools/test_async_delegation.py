...
def test_rejected_background_dispatch_reattaches_child_for_sync_interrupts():
    """A sync fallback remains owned by the parent after async rejection."""
    from unittest.mock import MagicMock, patch
    import json
    import tools.delegate_tool as dt
    ...
    fake_child = MagicMock()
    fake_child._delegate_role = "leaf"
    fake_child._subagent_id = "child-1"

    def build_and_register(**kwargs):
        parent._active_children.append(fake_child)
        return fake_child
    ...
    with patch.object(dt, "_build_child_agent", side_effect=build_and_register), \
         patch.object(dt, "_run_single_child", side_effect=run_child), \
         patch.object(dt, "_resolve_delegation_credentials", return_value=creds), \
         patch("tools.async_delegation.dispatch_async_delegation_batch", return_value={"status": "rejected", "error": "full"}):
        result = json.loads(
...
def test_cli_completion_ack_waits_for_successful_synthetic_turn(monkeypatch):
    import cli

    calls = []
    monkeypatch.setattr(ad, "mark_completion_delivered", lambda ident: calls.append(ident) or True)

    assert cli._acknowledge_completion_after_turn("deleg-1", False) is False
    assert calls == []
    assert cli._acknowledge_completion_after_turn("deleg-1", True) is True
    assert calls == ["deleg-1"]


def test_interrupt_for_session_matches_compression_lineage_only():
    interrupted = []
    with ad._records_lock:
        for ident, parent in (
            ("ancestor", "compression-parent"),
            ("tip", "compression-tip"),
            ...
            ad._records[ident] = {
                ...
                "interrupt_fn": lambda value=ident: interrupted.append(value),
            }

    count = ad.interrupt_for_session(
        parent_session_ids=["compression-parent", "compression-tip"],
        reason="test",
    )

    assert count == 2
    assert set(interrupted) == {"ancestor", "tip"}


def test_cli_queue_requires_parent_session_ownership(monkeypatch):
    import cli

    events = [
        (_make_async_evt(parent_session_id="old"), "old result"),
        (_make_async_evt(delegation_id="deleg-current", parent_session_id="current"), "current result"),
    ]
    monkeypatch.setattr(process_registry, "drain_notifications", lambda: events)
    pending = queue.Queue()

    assert cli._queue_cli_process_notifications(pending, "current") == 1
    item = pending.get_nowait()
    assert item.text == "current result"
    assert item.delegation_id == "deleg-current"


def test_cli_queue_accepts_marked_compression_parent_not_unrelated_child(
    ...
    db.create_session(
        "unrelated-child", source="delegate", parent_session_id="parent"
    )
    db.rotate_session_for_compression(
        "parent", "continuation", source="cli"
    )
    event = (_make_async_evt(parent_session_id="parent"), "continued result")
    monkeypatch.setattr(process_registry, "drain_notifications", lambda: [event])
    pending = queue.Queue()

    assert cli._queue_cli_process_notifications(
        pending, "continuation", db
    ) == 1
    item = pending.get_nowait()
    assert item.text == "continued result"

    assert cli._queue_cli_process_notifications(
        pending, "unrelated-child", db
    ) == 0
...
def test_gateway_delivery_requires_current_parent_session():
    from types import SimpleNamespace
    from gateway.run import GatewayRunner

    runner = object.__new__(GatewayRunner)
    runner.session_store = SimpleNamespace(
        _entries={"route": SimpleNamespace(session_id="current")}
    )

    assert runner._async_delegation_owned_by_current_session(
        _make_async_evt(session_key="route", parent_session_id="current")
    ) is True
    assert runner._async_delegation_owned_by_current_session(
        _make_async_evt(session_key="route", parent_session_id="old")
    ) is False
    assert runner._async_delegation_owned_by_current_session(
        _make_async_evt(session_key="route", parent_session_id=None)
    ) is False


def test_gateway_delivery_accepts_verified_compression_descendant_only():
    from types import SimpleNamespace
    from gateway.run import GatewayRunner

    runner = object.__new__(GatewayRunner)
    runner.session_store = SimpleNamespace(
        _entries={"route": SimpleNamespace(session_id="compression-child")},
        _db=None,
    )
    runner._session_db = SimpleNamespace(
        get_compression_tip=lambda parent: (
            "compression-child" if parent == "captured-parent" else parent
        )
    )

    assert runner._async_delegation_owned_by_current_session(
        _make_async_evt(session_key="route", parent_session_id="captured-parent")
    )
    assert not runner._async_delegation_owned_by_current_session(
        _make_async_evt(session_key="route", parent_session_id="unrelated-parent")
    )
