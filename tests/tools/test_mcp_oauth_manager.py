...
def test_marlow_provider_subclass_exists():
    """MarlowMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _MARLOW_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _MARLOW_PROVIDER_CLS is not None
    assert issubclass(_MARLOW_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_marlow_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns MarlowMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _MARLOW_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("MARLOW_HOME", str(tmp_path))
    ...
    assert _MARLOW_PROVIDER_CLS is not None
    assert isinstance(provider, _MARLOW_PROVIDER_CLS)
    assert provider._marlow_server_name == "srv"
