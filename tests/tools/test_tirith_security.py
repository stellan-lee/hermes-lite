...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._marlow_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(marlow_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(marlow_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_marlow_bin(self):
        """After _INSTALL_FAILED, manual install in MARLOW_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        marlow_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(marlow_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(marlow_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._marlow_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == marlow_bin
            assert _tirith_mod._resolved_path == marlow_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# MARLOW_HOME isolation
# ---------------------------------------------------------------------------

class TestMarlowHomeIsolation:
    def test_marlow_bin_dir_respects_marlow_home(self):
        """_marlow_bin_dir must use MARLOW_HOME, not hardcoded ~/.marlow."""
        from tools.tirith_security import _marlow_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"MARLOW_HOME": tmpdir}):
            result = _marlow_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_marlow_home(self):
        """_failure_marker_path must use MARLOW_HOME, not hardcoded ~/.marlow."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"MARLOW_HOME": "/custom/marlow"}):
            result = _failure_marker_path()
        assert result == "/custom/marlow/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets MARLOW_HOME; verify it's active."""
        marlow_home = os.getenv("MARLOW_HOME")
        assert marlow_home is not None, "MARLOW_HOME should be set by conftest"
        assert "marlow_test" in marlow_home, "Should point to test temp dir"

    def test_get_marlow_home_fallback(self):
        """Without MARLOW_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_marlow_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove MARLOW_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".marlow")
            result = _get_marlow_home()
