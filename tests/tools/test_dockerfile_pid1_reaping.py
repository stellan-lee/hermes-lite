...
- Signal forwarding runs through the init so ``docker stop`` triggers
  marlow's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, marlow accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, marlow accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into marlow,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, marlow still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``marlow-ink`` workspace package is
    ...
    assert "ui-tui/packages/marlow-ink/" in dockerfile_text
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``marlow-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` — not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@marlow/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@marlow/ink``)
    ...
    assert "ui-tui/packages/marlow-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled marlow-ink workspace package "
        "so ``await import('@marlow/ink')`` resolves at runtime."
    )
