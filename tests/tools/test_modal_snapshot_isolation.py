...
def _restore_tool_modules():
    original_marlow_home = os.environ.get("MARLOW_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_marlow_home is None:
            os.environ.pop("MARLOW_HOME", None)
        else:
            os.environ["MARLOW_HOME"] = original_marlow_home
        _reset_modules(("tools", "marlow_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    marlow_cli = types.ModuleType("marlow_cli")
    marlow_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["marlow_cli"] = marlow_cli
    marlow_home = tmp_path / "marlow-home"
    os.environ["MARLOW_HOME"] = str(marlow_home)
    sys.modules["marlow_cli.config"] = types.SimpleNamespace(
        get_marlow_home=lambda: marlow_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="marlow-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": marlow_home / "modal_snapshots.json",
