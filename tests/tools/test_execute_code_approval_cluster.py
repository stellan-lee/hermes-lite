...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad MARLOW_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: MARLOW_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("MARLOW_GATEWAY_SESSION", "1")
...
def test_env_scrub_marlow_allowlist_and_secret_blocks():
...
def test_env_scrub_logs_dropped_marlow_vars(caplog):
    """Dropping a non-allowlisted, non-secret MARLOW_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset MARLOW_* var."""
    import logging
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped MARLOW_* vars."""
    import logging
