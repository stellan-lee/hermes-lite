...
def _clean_state():
    ...
    saved = {}
    for k in ("MARLOW_INTERACTIVE", "MARLOW_GATEWAY_SESSION", "MARLOW_EXEC_ASK", "MARLOW_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["MARLOW_INTERACTIVE"] = "1"
        session_key = os.getenv("MARLOW_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No MARLOW_INTERACTIVE or MARLOW_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
