...
class TestWriteDenyExactPaths:
    ...
    def test_marlow_env(self):
        # ``.env`` under the active MARLOW_HOME (profile-aware, not just
        # ``~/.marlow``) must be write-denied. The hermetic test conftest
        # points MARLOW_HOME at a tempdir — resolve via get_marlow_home()
        # to match the denylist.
        from marlow_constants import get_marlow_home
        path = str(get_marlow_home() / ".env")
        assert _is_write_denied(path) is True

    def test_marlow_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.marlow/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "marlow_root"
        ...
        # Sanity check: MARLOW_HOME does point to the profile dir, not the root.
        from marlow_constants import get_marlow_home, get_default_marlow_root
        assert get_marlow_home() == profile_home
        assert get_default_marlow_root() == root
...
class TestWriteAllowed:
    ...
    def test_marlow_config_not_env(self):
        path = os.path.join(str(Path.home()), ".marlow", "config.yaml")
