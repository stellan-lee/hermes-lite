...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("marlow_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("marlow_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"MARLOW_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_marlow_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.marlow/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_marlow_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $MARLOW_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_marlow_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$MARLOW_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestMarlowConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.marlow/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.marlow/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.marlow/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.marlow/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.marlow/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.marlow/config.yaml")
        assert dangerous is True
        assert "marlow config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.marlow/config.yaml")
        assert dangerous is True

    def test_custom_marlow_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $MARLOW_HOME/config.yaml")
        assert dangerous is True

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.marlow/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Marlow config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_marlow_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $MARLOW_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.marlow/marlow-agent && source venv/bin/activate && python -m marlow_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m marlow_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m marlow_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "marlow_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart marlow-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_marlow_detected(self):
        """pkill targeting marlow/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_marlow_detected(self):
        cmd = "killall marlow"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep marlow) bypasses the pkill/killall name-matching
    ...
    def test_pkill_marlow_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("MARLOW_GATEWAY_SESSION", "MARLOW_CRON_SESSION",
                      "MARLOW_YOLO_MODE",
                      "MARLOW_SESSION_KEY", "MARLOW_INTERACTIVE")
        }
        os.environ.pop("MARLOW_YOLO_MODE", None)
        os.environ.pop("MARLOW_INTERACTIVE", None)
        # MARLOW_CRON_SESSION takes priority over MARLOW_GATEWAY_SESSION in
        ...
        os.environ.pop("MARLOW_CRON_SESSION", None)
        os.environ["MARLOW_GATEWAY_SESSION"] = "1"
        os.environ["MARLOW_SESSION_KEY"] = self.SESSION_KEY
