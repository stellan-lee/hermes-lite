...
Coverage:
  marlow_cli/clipboard.py  — platform-specific image extraction (macOS, WSL, Wayland, X11)
...
class TestSaveClipboardImage:
    ...
    def test_dispatches_to_windows_on_win32(self, tmp_path):
        ...
        with patch("marlow_cli.clipboard.sys") as mock_sys:
            ...
            with patch("marlow_cli.clipboard._windows_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
...
class TestIsWsl:
    def setup_method(self):
        # _is_wsl is marlow_constants.is_wsl; reset the function's own module
        # globals so this stays stable even if marlow_constants was imported
        # through a different module object earlier in a large xdist run.
        import marlow_constants
        marlow_constants._wsl_detected = None
    ...
    def test_regular_linux(self):
        ...
        # supposed to intercept marlow_constants.is_wsl's `open` call,
...
class TestLinuxSave:
    ...
    def test_wsl_tried_first(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("marlow_cli.clipboard._is_wsl", return_value=True):
            with patch("marlow_cli.clipboard._wsl_save", return_value=True) as m:
    ...
    def test_wsl_fails_falls_through_to_xclip(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("marlow_cli.clipboard._is_wsl", return_value=True):
            with patch("marlow_cli.clipboard._wsl_save", return_value=False):
    ...
    def test_wayland_tried_when_display_set(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("marlow_cli.clipboard._is_wsl", return_value=False):
...
class TestWindowsHasImage:
    ...
    def test_clipboard_has_image(self):
        with patch("marlow_cli.clipboard._get_ps_exe", return_value="powershell"):
            with patch("marlow_cli.clipboard.subprocess.run") as mock_run:
                mock_run.return_value = MagicMock(stdout="True\n", returncode=0)
    ...
    def test_no_powershell_available(self):
        with patch("marlow_cli.clipboard._get_ps_exe", return_value=None):
            assert _windows_has_image() is False
...
class TestHasClipboardImageWin32:
    ...
    def test_dispatches_on_win32(self):
        with patch("marlow_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "win32"
            with patch("marlow_cli.clipboard._windows_has_image", return_value=True) as m:
...
class TestHasClipboardImage:
    ...
    def test_linux_wsl_dispatch(self):
        with patch("marlow_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("marlow_cli.clipboard._is_wsl", return_value=True):
                with patch("marlow_cli.clipboard._wsl_has_image", return_value=True) as m:
                    assert has_clipboard_image() is True
                    m.assert_called_once()
    ...
    def test_wsl_falls_through_to_wayland_when_windows_path_empty(self):
        ...
        with patch("marlow_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("marlow_cli.clipboard._is_wsl", return_value=True):
                with patch("marlow_cli.clipboard._wsl_has_image", return_value=False) as wsl:
                    with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                        with patch("marlow_cli.clipboard._wayland_has_image", return_value=True) as wl:
                            assert has_clipboard_image() is True
