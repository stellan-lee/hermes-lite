...
class TestKreaLocalReferences:
    def test_local_path_is_encoded_as_provider_usable_data_url(self, tmp_path):
        from plugins.image_gen.krea import _provider_usable_reference

        source = tmp_path / "reference.png"
        source.write_bytes(b"\x89PNG\r\nlocal-reference")

        result = _provider_usable_reference(str(source))

        assert result.startswith("data:image/png;base64,")
        assert base64.b64decode(result.split(",", 1)[1]) == source.read_bytes()

    def test_remote_and_existing_data_references_are_unchanged(self):
        from plugins.image_gen.krea import _provider_usable_reference

        assert _provider_usable_reference("https://example.test/ref.png") == "https://example.test/ref.png"
        assert _provider_usable_reference("data:image/png;base64,AA==") == "data:image/png;base64,AA=="
