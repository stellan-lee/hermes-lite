...
class TestWriteFileHandler:
    ...
    def test_writes_content(self, mock_get):
        ...
        mock_ops.write_file.assert_called_once_with("/private/tmp/out.txt", "hello world!\n")
...
class TestPatchHandler:
    ...
    def test_replace_mode_calls_patch_replace(self, mock_get):
        ...
        assert result["status"] == "ok"
        mock_ops.patch_replace.assert_called_once_with("/private/tmp/f.py", "foo", "bar", False)
    ...
    def test_replace_mode_replace_all_flag(self, mock_get):
        ...
        mock_ops.patch_replace.assert_called_once_with("/private/tmp/f.py", "x", "y", True)
...
class TestSensitivePathCheck:
    ...
    def test_marlow_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._marlow_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._marlow_config_resolved_loaded", True)

        from tools.file_tools import write_file_tool
        result = json.loads(write_file_tool(str(fake_config), "approvals:\n  mode: off\n"))
        assert "error" in result
        assert "Refusing to write to Marlow config file" in result["error"]

    def test_marlow_config_blocked_via_tilde_path(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._marlow_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._marlow_config_resolved_loaded", True)
    ...
    def test_marlow_config_blocked_for_patch(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("tools.file_tools._marlow_config_resolved", "/some/other/path")
        monkeypatch.setattr("tools.file_tools._marlow_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("tools.file_tools._marlow_config_resolved", "/home/user/.marlow/config.yaml")
        monkeypatch.setattr("tools.file_tools._marlow_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
