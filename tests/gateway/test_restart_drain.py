...
def test_load_busy_input_mode_prefers_env_then_config_then_default(
    ...
    monkeypatch.delenv("MARLOW_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("MARLOW_GATEWAY_BUSY_INPUT_MODE", "interrupt")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "interrupt"

    monkeypatch.setenv("MARLOW_GATEWAY_BUSY_INPUT_MODE", "steer")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "steer"

    # Unknown values fall through to the safe default
    monkeypatch.setenv("MARLOW_GATEWAY_BUSY_INPUT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "interrupt"
...
def test_load_busy_text_mode_defaults_to_queue_and_allows_interrupt(
    ...
    monkeypatch.setattr(gateway_run, "_marlow_home", tmp_path)
    monkeypatch.delenv("MARLOW_GATEWAY_BUSY_TEXT_MODE", raising=False)
    ...
    monkeypatch.setenv("MARLOW_GATEWAY_BUSY_TEXT_MODE", "queue")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"

    monkeypatch.setenv("MARLOW_GATEWAY_BUSY_TEXT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"
...
def test_load_restart_drain_timeout_prefers_env_then_config_then_default(
    ...
    monkeypatch.setenv("MARLOW_RESTART_DRAIN_TIMEOUT", "7")
