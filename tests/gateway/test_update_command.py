...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("MARLOW_MANAGED", "homebrew")

        result = await runner._handle_update_command(event)

        assert "managed by Homebrew" in result
        assert "brew upgrade marlow-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _marlow_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._marlow_home", tmp_path), \
        ...
        with patch("gateway.run._marlow_home", tmp_path):
            ...
            # project_root will be the real marlow-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_marlow_binary(self, tmp_path):
        """Returns error when marlow is not on PATH and marlow_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "marlow update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m marlow_cli.main when marlow not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._marlow_home", marlow_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting Marlow update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m marlow_cli.main
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._marlow_home", marlow_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/marlow" if x == "marlow" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = marlow_home / ".update_pending.json"
        ...
        assert not (marlow_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((marlow_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (marlow_home / ".update_pending.json").exists()
        assert not (marlow_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (marlow_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = marlow_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (marlow_home / ".update_output.txt").write_text("done")
        (marlow_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (marlow_home / ".update_pending.json").write_text(json.dumps(pending))
        (marlow_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (marlow_home / ".update_pending.json").write_text(json.dumps(pending))
        (marlow_home / ".update_output.txt").write_text("x" * 5000)
        (marlow_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (marlow_home / ".update_pending.json").write_text(json.dumps(pending))
        (marlow_home / ".update_output.txt").write_text("Traceback: boom")
        (marlow_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = marlow_home / ".update_pending.json"
        output_path = marlow_home / ".update_output.txt"
        exit_code_path = marlow_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
