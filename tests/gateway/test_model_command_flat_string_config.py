...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "marlow_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_marlow_home() / config.yaml`` — point it here.
    monkeypatch.setattr("marlow_constants.get_marlow_home", lambda: marlow_home)
    monkeypatch.setattr("marlow_cli.config.get_marlow_home", lambda: marlow_home)
    return cfg_path
