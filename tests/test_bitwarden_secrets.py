...
binary download are exercised manually by `marlow secrets bitwarden
...
def marlow_home(tmp_path, monkeypatch):
    """Point Marlow at an isolated home directory."""
    home = tmp_path / ".marlow"
    home.mkdir()
    monkeypatch.setenv("MARLOW_HOME", str(home))
    # Some modules cache get_marlow_home; clear if needed.
    import marlow_constants
    if hasattr(marlow_constants, "_MARLOW_HOME_CACHE"):
        marlow_constants._MARLOW_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(marlow_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(marlow_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(marlow_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from marlow_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under marlow_home/cache/."""
    home = tmp_path / ".marlow"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
