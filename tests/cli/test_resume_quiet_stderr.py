...
``tool_progress_mode == "off"`` (set by ``marlow chat --quiet``):
...
def _make_cli(quiet=False, session_id="20260524_111111_xyz", db=None):
    """Build a minimal MarlowCLI bound to only what _init_agent needs for
...
class TestResumeQuietStderr:
    def test_session_not_found_goes_to_stderr_in_quiet_mode(self, capsys):
        ...
        assert "marlow sessions list" in captured.err

    def test_session_not_found_goes_to_stdout_in_full_mode(self, capfd):
        db = MagicMock()
        db.get_session.return_value = None
