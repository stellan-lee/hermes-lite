...
from marlow_cli import main as marlow_main
...
def test_marlow_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in MarlowCLI.__init__")

    monkeypatch.setattr("marlow_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("marlow_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.MarlowCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("marlow_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("marlow_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.MarlowCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("marlow_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "marlow_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.MarlowCLI(compact=True, max_turns=1)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.MarlowCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.MarlowCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("marlow_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("marlow_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("marlow_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(marlow_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    marlow_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "marlow_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("marlow_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("marlow_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("marlow_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("marlow_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "marlow_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("marlow_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    marlow_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("marlow_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("marlow_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "marlow_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    monkeypatch.setattr("marlow_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("marlow_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "marlow_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("marlow_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    marlow_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_auto_provider_name_localhost():
    from marlow_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from marlow_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "marlow_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
