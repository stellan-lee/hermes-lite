...
class TestChromeDebugLaunch:
    ...
    def test_linux_candidates_prefer_chrome_before_brave_when_both_exist(self):
        ...
        with patch("marlow_cli.browser_connect.shutil.which", side_effect=fake_which), \
    ...
    def test_linux_candidates_include_arch_brave_install_path(self):
        ...
        with patch("marlow_cli.browser_connect.shutil.which", return_value=None), \
             patch("marlow_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == brave):
            candidates = get_chrome_debug_candidates("Linux")
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_linux_candidates_include_brave_binary_name(self):
        ...
        with patch("marlow_cli.browser_connect.shutil.which", side_effect=lambda name: brave if name == "brave" else None), \
             patch("marlow_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == brave):
            candidates = get_chrome_debug_candidates("Linux")
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_linux_candidates_include_official_brave_and_edge_stable_paths(self):
        ...
        with patch("marlow_cli.browser_connect.shutil.which", return_value=None), \
             patch("marlow_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {brave, edge}):
            candidates = get_chrome_debug_candidates("Linux")
    ...
    def test_launch_tries_next_browser_when_first_candidate_fails(self):
        ...
        with patch("marlow_cli.browser_connect.get_chrome_debug_candidates", return_value=[brave, chrome]), \
    ...
    def test_manual_command_returns_none_when_linux_browser_missing(self):
        with patch("marlow_cli.browser_connect.shutil.which", return_value=None), \
             patch("marlow_cli.browser_connect.os.path.isfile", return_value=False):
