...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("marlow/marlow-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned marlow/* and pr-* branches."""

    def test_prunes_orphaned_marlow_branch(self, git_repo):
        """marlow/marlow-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "marlow/marlow-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "marlow/marlow-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "marlow/marlow-deadbeef" in result.stdout
        ...
        assert "marlow/marlow-deadbeef" in orphaned
        ...
        assert "marlow/marlow-deadbeef" not in result.stdout
