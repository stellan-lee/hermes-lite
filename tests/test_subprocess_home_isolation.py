...
See: https://github.com/NousResearch/marlow-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for marlow_constants.get_subprocess_home()."""

    def test_returns_none_when_marlow_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MARLOW_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("MARLOW_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from marlow_constants import (
            get_marlow_home,
            reset_marlow_home_override,
            set_marlow_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_marlow_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_marlow_home_override(profile)
        try:
            assert get_marlow_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_marlow_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_marlow_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(marlow_home / "home")
    ...
    def test_no_injection_when_marlow_home_unset(self, monkeypatch):
        monkeypatch.delenv("MARLOW_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["MARLOW_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from marlow_cli.profiles import _PROFILE_DIRS
