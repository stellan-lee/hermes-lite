"""Tests for the top-level `./marlow` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./marlow` should use `marlow_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "marlow"
    called = []

    fake_main_module = types.ModuleType("marlow_cli.main")

    def fake_main():
        called.append("marlow_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "marlow_cli.main", fake_main_module)
