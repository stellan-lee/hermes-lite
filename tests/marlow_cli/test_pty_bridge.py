"""Unit tests for marlow_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf marlow-ok"])
        try:
            output = _read_until(bridge, b"marlow-ok")
            assert b"marlow-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$MARLOW_PTY_TEST\""],
            env={**os.environ, "MARLOW_PTY_TEST": "pty-env-works"},
        )
        try:
