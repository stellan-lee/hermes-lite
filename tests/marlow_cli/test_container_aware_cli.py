...
def container_env(tmp_path, monkeypatch):
    """Set up a fake MARLOW_HOME with .container-mode file."""
    marlow_home = tmp_path / ".marlow"
    marlow_home.mkdir()
    monkeypatch.setenv("MARLOW_HOME", str(marlow_home))
    monkeypatch.delenv("MARLOW_DEV", raising=False)

    container_mode = marlow_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=marlow-agent\n"
        "exec_user=marlow\n"
        "marlow_bin=/data/current-package/bin/marlow\n"
    )
    return marlow_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("marlow_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "marlow-agent"
    assert info["exec_user"] == "marlow"
    assert info["marlow_bin"] == "/data/current-package/bin/marlow"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("marlow_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_marlow_dev(container_env, monkeypatch):
    """Returns None when MARLOW_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("MARLOW_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_marlow_dev_zero(container_env, monkeypatch):
    """MARLOW_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("MARLOW_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        marlow_home = Path(tmpdir) / ".marlow"
        marlow_home.mkdir()
        (marlow_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("marlow_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_marlow_home": lambda: marlow_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MARLOW_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=marlow-custom\n"
        "exec_user=myuser\n"
        "marlow_bin=/opt/marlow/bin/marlow\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "marlow-custom"
    assert info["exec_user"] == "myuser"
    assert info["marlow_bin"] == "/opt/marlow/bin/marlow"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from marlow_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "marlow"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "marlow-agent" in cmd
    assert "/data/current-package/bin/marlow" in cmd
    assert "chat" in cmd
