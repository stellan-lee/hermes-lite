"""Regression tests for #34107 — Docker UID/GID handling in ensure_marlow_home.

When Marlow runs in Docker with ``MARLOW_UID=1000`` / ``MARLOW_GID=911``,
the entrypoint chowns the top-level ``MARLOW_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_marlow_home()`` were landing
as ``root:root`` and blocking subsequent uid-mapped worker invocations with
``PermissionError [Errno 13]``.

The fix is a ``_chown_to_marlow_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_marlow_uid_gid
# ---------------------------------------------------------------------------


class TestResolveMarlowUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("MARLOW_UID", "1000")
        monkeypatch.setenv("MARLOW_GID", "911")
        from marlow_cli.config import _resolve_marlow_uid_gid
        uid, gid = _resolve_marlow_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("MARLOW_UID", raising=False)
        monkeypatch.delenv("MARLOW_GID", raising=False)
        from marlow_cli.config import _resolve_marlow_uid_gid
        uid, gid = _resolve_marlow_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("MARLOW_UID", "not-a-number")
        monkeypatch.setenv("MARLOW_GID", "911")
        from marlow_cli.config import _resolve_marlow_uid_gid
        uid, gid = _resolve_marlow_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("MARLOW_UID", "")
        monkeypatch.setenv("MARLOW_GID", "")
        from marlow_cli.config import _resolve_marlow_uid_gid
        uid, gid = _resolve_marlow_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_marlow_uid
# ---------------------------------------------------------------------------


class TestChownToMarlowUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MARLOW_UID", "1000")
        monkeypatch.setenv("MARLOW_GID", "911")
        from marlow_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_marlow_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_marlow_uid(d)  # must not raise
