"""Tests for marlow_cli.managed_uv — one path, no guessing."""
...
class TestEnsureUv:
    def test_already_installed_no_bootstrap(self, tmp_path):
        _make_executable(tmp_path / "bin" / "uv")
        with patch("marlow_cli.managed_uv.get_marlow_home", return_value=tmp_path):
            from marlow_cli.managed_uv import ensure_uv
    ...
    def test_installs_if_missing_sets_bootstrap_flag(self, tmp_path):
        with patch("marlow_cli.managed_uv.get_marlow_home", return_value=tmp_path), \
             patch("marlow_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_failure_returns_none_false(self, tmp_path):
        with patch("marlow_cli.managed_uv.get_marlow_home", return_value=tmp_path), \
             patch("marlow_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from marlow_cli.managed_uv import ensure_uv
            path, fresh = ensure_uv()
            assert path is None
            assert fresh is False
...
class TestRebuildVenv:
    def test_removes_old_venv_and_creates_new(self, tmp_path):
        ...
        with patch("marlow_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch("marlow_cli.managed_uv.shutil.rmtree") as mock_rmtree:
            from marlow_cli.managed_uv import rebuild_venv
    ...
    def test_rebuild_failure_returns_false(self, tmp_path):
        ...
        with patch("marlow_cli.managed_uv.subprocess.run") as mock_run, \
             patch("marlow_cli.managed_uv.shutil.rmtree"):
...
class TestUpdateManagedUv:
    def test_no_uv_returns_none(self, tmp_path):
        with patch("marlow_cli.managed_uv.get_marlow_home", return_value=tmp_path):
            from marlow_cli.managed_uv import update_managed_uv
            assert update_managed_uv() is None

    def test_self_update_success(self, tmp_path):
        _make_executable(tmp_path / "bin" / "uv")
        with patch("marlow_cli.managed_uv.get_marlow_home", return_value=tmp_path), \
             patch("marlow_cli.managed_uv.subprocess.run") as mock_run:
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("marlow_cli.managed_uv._install_uv_posix") as mock_posix:
