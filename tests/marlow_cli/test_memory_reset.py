"""Tests for the `marlow memory reset` CLI command.
...
- Profile-scoped reset (uses MARLOW_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake MARLOW_HOME with memory files."""
    marlow_home = tmp_path / ".marlow"
    memories = marlow_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("MARLOW_HOME", str(marlow_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nMarlow repo is at ~/.marlow/marlow-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `marlow memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        marlow_home = tmp_path / ".marlow"
        (marlow_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("MARLOW_HOME", str(marlow_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's MARLOW_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_marlow_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
