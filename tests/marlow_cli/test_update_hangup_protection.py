"""Tests for SIGHUP protection and stdout mirroring in ``marlow update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``marlow_cli/main.py``.  These exist so
that ``marlow update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_marlow_home if present
        import marlow_cli.config as _cfg
        if hasattr(_cfg, "_MARLOW_HOME_CACHE"):
            _cfg._MARLOW_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            stream_class = _install_hangup_protection.__globals__["_UpdateOutputStream"]
            assert state["installed"] is True
            assert isinstance(sys.stdout, stream_class)
            assert isinstance(sys.stderr, stream_class)
            ...
            assert "marlow update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_marlow_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "marlow_cli.config.get_marlow_home", _boom, raising=True
        )
