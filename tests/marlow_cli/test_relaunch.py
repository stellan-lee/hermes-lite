"""Tests for marlow_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from marlow_cli import relaunch as relaunch_mod


class TestResolveMarlowBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/marlow"
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_marlow_bin() is None
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_marlow_bin", lambda: "/usr/bin/marlow")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/marlow"

    def test_falls_back_to_python_module(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_marlow_bin", lambda: None)
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv == [sys.executable, "-m", "marlow_cli.main", "--resume", "abc"]
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/marlow", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/marlow", ["/usr/bin/marlow", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        marlow).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_marlow_bin", lambda: r"C:\Users\test\marlow.exe")
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\marlow.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_marlow_bin", lambda: r"C:\marlow.exe")
    ...
    def test_windows_surfaces_oserror_with_help(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_marlow_bin", lambda: r"C:\missing.exe")
...
class TestResolveMarlowBinWindowsPyGuard:
    """On Windows, resolve_marlow_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a marlow.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\marlow.exe" if name == "marlow" else None,
        )

        bin_path = relaunch_mod.resolve_marlow_bin()
        # Must NOT be the .py — must be the marlow.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\marlow.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "marlow"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_marlow_bin() == str(script)

    def test_windows_py_argv0_with_no_marlow_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND marlow.exe
        isn't on PATH, return None so the caller falls back to
        python -m marlow_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
