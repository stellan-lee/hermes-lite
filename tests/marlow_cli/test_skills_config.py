"""Tests for marlow_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("marlow_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from marlow_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("marlow_cli.config.load_config")
    ...
    @patch("marlow_cli.config.load_config")
    @patch.dict("os.environ", {"MARLOW_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("MARLOW_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """MARLOW_SESSION_PLATFORM should be used when MARLOW_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_marlow_platform_takes_precedence(self, tmp_path, monkeypatch):
        """MARLOW_PLATFORM should win over MARLOW_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("MARLOW_HOME", str(tmp_path))
        monkeypatch.setenv("MARLOW_PLATFORM", "telegram")
        monkeypatch.setenv("MARLOW_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MARLOW_HOME", str(tmp_path))
        monkeypatch.setenv("MARLOW_PLATFORM", "telegram")
        monkeypatch.setenv("MARLOW_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from marlow_cli.skills_config import _get_categories
        skills = [
