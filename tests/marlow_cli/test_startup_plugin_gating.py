...
``marlow_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``marlow --help``, ``marlow version``,
``marlow logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``marlow --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from marlow_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["marlow", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["marlow", "-w", "--tui"], None),
        (["marlow", "version"], "version"),
        (["marlow", "--tui", "chat"], "chat"),
        (["marlow", "-w", "logs"], "logs"),
        (["marlow", "chat", "hello world"], "chat"),
        (["marlow", "gateway", "run"], "gateway"),
        ...
        (["marlow", "--", "chat"], "chat"),               # -- terminator
        (["marlow", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["marlow", "some-plugin-cmd"], "some-plugin-cmd"),
        (["marlow", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["marlow"],                          # bare → chat
        ["marlow", "--help"],                # top-level help
        ["marlow", "-h"],
        ["marlow", "version"],               # known built-in
        ["marlow", "logs"],
        ["marlow", "gateway", "run"],
        ["marlow", "--tui"],
        ["marlow", "-w", "--tui"],
        ["marlow", "chat", "hi"],
        ["marlow", "help"],                  # accepted built-in-ish
        ["marlow", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["marlow", "honcho", "status"],      # potential memory plugin
        ["marlow", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``marlow help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"marlow_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
