...
handling was unreliable in `marlow setup` (the provider->model sub-menu).
"""
...
def test_prompt_model_selection_uses_curses_radiolist():
    ...
    with patch("marlow_cli.curses_ui.curses_radiolist", side_effect=_fake), \
...
def test_prompt_model_selection_esc_cancels():
    ...
    with patch("marlow_cli.curses_ui.curses_radiolist", return_value=-1), \
         patch("builtins.print"):
        result = _prompt_model_selection(["model-a", "model-b"])
...
def test_reasoning_effort_uses_curses_radiolist():
    from marlow_cli.main import _prompt_reasoning_effort_selection

    with patch("marlow_cli.curses_ui.curses_radiolist", return_value=2), \
         patch("builtins.print"):
