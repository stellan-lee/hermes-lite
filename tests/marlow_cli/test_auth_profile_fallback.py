...
When ``MARLOW_HOME`` points to a named profile, ``read_credential_pool()``
...
# ``_load_provider_state`` itself, those helpers raise ``"Marlow is not
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    _write(marlow_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (MARLOW_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.marlow as the "real home" — which is NOT equal
    # to the MARLOW_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    marlow_home = tmp_path / "classic"
    ...
    _write(marlow_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from marlow_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: MARLOW_HOME is set to a custom path that is NOT under
    # ~/.marlow/profiles/ — get_default_marlow_root() returns MARLOW_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from marlow_cli.auth import read_credential_pool, write_credential_pool
