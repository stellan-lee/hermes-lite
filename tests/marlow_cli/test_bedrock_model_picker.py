...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (MARLOW_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via MARLOW_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in MARLOW_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from marlow_cli.providers import MARLOW_OVERLAYS
        assert "bedrock" in MARLOW_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from marlow_cli.providers import MARLOW_OVERLAYS
        assert MARLOW_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from marlow_cli.providers import MARLOW_OVERLAYS
        assert MARLOW_OVERLAYS["bedrock"].auth_type == "aws_sdk"
