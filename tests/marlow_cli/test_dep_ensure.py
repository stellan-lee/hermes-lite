...
def test_ensure_dependency_skips_when_present():
    ...
    with patch("marlow_cli.dep_ensure.shutil.which", return_value="/usr/bin/node"):
...
def test_ensure_dependency_returns_false_when_missing_noninteractive():
    ...
    with patch("marlow_cli.dep_ensure.shutil.which", return_value=None), patch(
        "marlow_cli.dep_ensure._find_install_script", return_value=(None, None)
    ):
...
def test_find_install_script_from_checkout(tmp_path):
    ...
    assert _find_install_script(tmp_path / "marlow_cli", tmp_path) == (expected, "bash")
...
def test_has_marlow_agent_browser_paths(tmp_path):
    ...
    for relative in ("node/bin/agent-browser", "node_modules/.bin/agent-browser"):
        ...
        with patch("marlow_constants.get_marlow_home", return_value=tmp_path):
...
def test_ensure_dependency_runs_bash_installer(tmp_path):
    ...
    with patch("marlow_cli.dep_ensure._DEP_CHECKS", {"node": lambda: next(checks)}), patch(
        "marlow_cli.dep_ensure._find_install_script", return_value=(script, "bash")
    ), patch("marlow_cli.dep_ensure.subprocess.run") as mock_run:
