...
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    ...
    assert _check_marlow_model_warning(model_name) == _MARLOW_MODEL_WARNING
...
def test_does_not_match_unrelated_models(model_name: str) -> None:
    ...
    assert _check_marlow_model_warning(model_name) == ""


def test_none_like_inputs_are_safe() -> None:
    assert is_nous_hermes_non_agentic("") is False
    # Defensive: the helper shouldn't crash on None-ish falsy input either.
    assert _check_marlow_model_warning("") == ""
