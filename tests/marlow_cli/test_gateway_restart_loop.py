...
Covers:
- Defense 1: gateway stop/restart refuse when _MARLOW_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "marlow gateway stop",
        "marlow gateway start",
        "marlow  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "MARLOW GATEWAY RESTART",           # uppercase
    ])
    def test_marlow_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.marlow.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.marlow.gateway.plist",
        "launchctl stop ai.marlow.gateway",
        "systemctl restart marlow-gateway",
        "systemctl stop marlow-gateway.service",
        "systemctl start marlow-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill marlow gateway process",
        "pkill -f marlow.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "marlow cron list",
        "marlow update",
        "marlow config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_marlow_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade marlow then run marlow gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.marlow.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify marlow gateway stop/restart refuse when _MARLOW_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_MARLOW_GATEWAY", raising=False)
