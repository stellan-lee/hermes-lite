"""Tests for IRC gateway configuration via `marlow setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Marlow install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Marlow install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#marlow")
            monkeypatch.setenv("IRC_NICKNAME", "marlow-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `marlow setup gateway` experience with IRC present."""
