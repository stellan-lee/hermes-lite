"""Tests for ``marlow update`` / ``--check`` inside the Docker container.
...
fix, ``marlow update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Marlow, not an update to the running container, so the message
...
source-rebuild guidance from ``format_docker_update_message`` and exit
...
from marlow_cli.main import _cmd_update_check, cmd_update


# ---------- cmd_update (apply path) ----------


@patch("marlow_cli.config.is_managed", return_value=False)
@patch("marlow_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``marlow update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``marlow update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "MARLOW_HOME" in msg or "/opt/data" in msg, (
        "must address config persistence across upgrades"
    )

    # Acknowledges custom registry deployments.
    assert "custom registry" in msg.lower()
