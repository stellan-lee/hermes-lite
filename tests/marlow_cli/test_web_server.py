"""Tests for marlow_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_marlow_home):
    """Drop the example-dashboard fixture into the per-test MARLOW_HOME
    ...
    isolated ``MARLOW_HOME``.

    The user-plugin source is preferred over a transient
    ``MARLOW_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — marlow-achievements, model providers
    ...
    from marlow_cli import web_server
    ...
    #      and start serving requests against a torn-down MARLOW_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Marlow vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Marlow env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints MARLOW_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import marlow_cli.web_server as ws

        monkeypatch.setenv("MARLOW_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("MARLOW_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_marlow_home):
        """Create a TestClient and isolate the state DB under the test MARLOW_HOME."""
        try:
        ...
        import marlow_state
        from marlow_constants import get_marlow_home
        from marlow_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(marlow_state, "DEFAULT_DB_PATH", get_marlow_home() / "state.db")
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("marlow_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from marlow_cli.web_server import app
    ...
    def test_elevenlabs_voices_unavailable_without_key(self, monkeypatch):
        import marlow_cli.web_server as web_server
    ...
    def test_update_marlow_returns_docker_guidance_without_spawning(self, monkeypatch):
        import marlow_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn marlow update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_marlow_action", fail_spawn)
        web_server._ACTION_PROCS.pop("marlow-update", None)
        web_server._ACTION_RESULTS.pop("marlow-update", None)

        resp = self.client.post("/api/marlow/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "marlow-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/marlow-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/marlow-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/marlow-agent:latest" in line for line in status_data["lines"])

    def test_update_marlow_spawns_on_non_docker_install(self, monkeypatch):
        import marlow_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_marlow_action", fake_spawn)
        web_server._ACTION_PROCS.pop("marlow-update", None)
        web_server._ACTION_RESULTS.pop("marlow-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "marlow-update"}
        assert calls == [(["update"], "marlow-update")]
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from marlow_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_bad_token(self, tmp_path):
        ...
        from marlow_cli.config import save_env_value
        from marlow_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from marlow_cli.config import save_env_value
        from marlow_cli.web_server import _SESSION_TOKEN
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from marlow_cli.config import load_config, load_env
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/marlow_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import marlow_cli.nous_subscription as ns
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `marlow model`), not the first curated paid entry."""
        import marlow_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from marlow_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from marlow_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_marlow_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("marlow://")
    ...
    def test_profiles_list_includes_default(self):
        from marlow_constants import get_marlow_home
        get_marlow_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from marlow_constants import get_marlow_home
        import marlow_cli.profiles as profiles_mod
        ...
        named = marlow_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_marlow_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_marlow_for_default_profile(self):
        from marlow_constants import get_marlow_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "marlow setup"
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_marlow_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_marlow_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_marlow_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import marlow_cli.skills_config as skills_config
        import marlow_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from marlow_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from marlow_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from marlow_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.marlow/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``marlow_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_marlow_home):
        ...
        monkeypatch.setattr(
            marlow_state, "DEFAULT_DB_PATH", get_marlow_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in marlow_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_marlow_home):
        ...
        # Pin the SessionDB to the isolated MARLOW_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_marlow_home, _install_example_plugin):
        ...
        installs it into the per-test ``MARLOW_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into MARLOW_HOME by the class-level
    ...
    def test_other_plugin_route_requires_auth(self):
        ...
        """
        # Real plugin path (marlow-achievements is loaded by default).
        resp = self.client.get("/api/plugins/marlow-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``marlow --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["MARLOW_TUI_INLINE"] == "1"
        assert env["MARLOW_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"marlow-ws-ok" in buf:
                    break
            assert b"marlow-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from marlow_cli.pty_bridge import PtyUnavailableError
        ...
        import marlow_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        MARLOW_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self, monkeypatch):
        ...
        import time
        from urllib.parse import urlencode
        from marlow_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("MARLOW_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_marlow_home, _install_example_plugin):
        ...
        the per-test ``MARLOW_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
