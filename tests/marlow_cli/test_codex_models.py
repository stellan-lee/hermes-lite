...
from marlow_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_get_codex_model_ids_prioritizes_default_and_cache(tmp_path, monkeypatch):
    ...
    # backend still accepts it via the OAuth-backed CLI/Marlow route.
...
def test_fetch_from_api_keeps_supported_in_api_false_models(monkeypatch):
    ...
    """
    import sys
    from marlow_cli import codex_models
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    ...
    monkeypatch.setattr(
        "marlow_cli.auth.get_codex_auth_status",
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(
    ...
    monkeypatch.setattr(
        "marlow_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "marlow-auth-store"},
    )
    monkeypatch.setattr(
        "marlow_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("marlow_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
        "marlow_cli.codex_models.get_codex_model_ids",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a MarlowCLI with minimal mocking."""
    ...
    with (
        ...
        cli = MarlowCLI(model=model, **kwargs)
    return cli
