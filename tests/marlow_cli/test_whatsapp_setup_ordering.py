...
Before the fix, ``marlow whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``marlow gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    marlow = home / ".marlow"
    marlow.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("MARLOW_HOME", str(marlow))
...
def _env_value(marlow_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("marlow_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``marlow whatsapp`` with an existing paired session and
