"""Tests for marlow_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate MARLOW_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from marlow_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from marlow_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://marlow-agent.nousresearch.com/docs/api/model-catalog.json"
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in marlow_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from marlow_cli import model_catalog
        from marlow_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from marlow_cli import model_catalog
        from marlow_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_has_no_nous_row(self, tmp_path, monkeypatch):
        ...
        # ``MARLOW_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` MARLOW_HOME directly instead.
        import importlib
        from marlow_cli import model_catalog
        importlib.reload(model_catalog)
        try:
            from marlow_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["MARLOW_HOME"])
...
# the bundled manifest at marlow_cli/data/model-catalog.json.
#
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        commit the regenerated ``marlow_cli/data/model-catalog.json``.
        """
        # Resolve the repo root from this test file's location.
        repo_root = Path(__file__).resolve().parents[2]
        manifest_path = repo_root / "marlow_cli" / "data" / "model-catalog.json"
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            "marlow_cli/data/model-catalog.json is out of sync with "
            "_PROVIDER_MODELS['nous'] / OPENROUTER_MODELS. "
            "Run: python scripts/build_model_catalog.py && "
            "git add marlow_cli/data/model-catalog.json"
        )
