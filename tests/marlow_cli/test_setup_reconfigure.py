...
- Bare `marlow setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `marlow setup --quick` runs the narrower "fill in missing items" flow.
- `marlow setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("marlow_cli.setup.ensure_marlow_home", {}),
        ("marlow_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("marlow_cli.config.is_managed", {"return_value": False}),
...
class TestExistingInstallDefault:
    """Bare `marlow setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="marlow_cli.setup.prompt_choice",
                quick="marlow_cli.setup._run_quick_setup",
                model="marlow_cli.setup.setup_model_provider",
                terminal="marlow_cli.setup.setup_terminal_backend",
                agent="marlow_cli.setup.setup_agent_settings",
                gateway="marlow_cli.setup.setup_gateway",
                tools="marlow_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`marlow setup --reconfigure` behaves the same as bare `marlow setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("marlow_cli.setup.prompt_choice", {"return_value": 0}),
                first="marlow_cli.setup._run_first_time_quick_setup",
            )
            from marlow_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "marlow_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["marlow", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["marlow", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["marlow", "setup"])
        try:
            main()
        except SystemExit:
            pass
