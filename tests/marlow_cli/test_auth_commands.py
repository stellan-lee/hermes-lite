...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (marlow_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from marlow_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "marlow" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_marlow_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:marlow_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:marlow_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "marlow_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "marlow-cli",
    ...
    # `marlow auth add nous` must also populate providers.nous so the
    ...
    # raises "Marlow is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "marlow_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`marlow auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from marlow_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`marlow logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (marlow_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `marlow logout` should target configured Codex if auth has no active provider."""
    marlow_home = tmp_path / "marlow"
    monkeypatch.setenv("MARLOW_HOME", str(marlow_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (marlow_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((marlow_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from marlow_cli.auth import _reset_config_provider
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from marlow_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "marlow_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("marlow_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("marlow_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("marlow_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "marlow_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((marlow_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from marlow_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from marlow_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `marlow auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("MARLOW_HOME", str(tmp_path / "marlow"))
    marlow_home = tmp_path / "marlow"
    marlow_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `marlow auth remove`)
    ...
    payload = json.loads((marlow_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("marlow_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((marlow_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`marlow auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.marlow/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.marlow/.env (not the shell), the
    ...
    (marlow_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (marlow_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `marlow auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from marlow_cli.auth import is_source_suppressed
    from marlow_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `marlow auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Marlow reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import marlow_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_marlow_pkce_suppression(tmp_path, monkeypatch):
    """anthropic marlow_pkce must not re-seed from ~/.marlow/.anthropic_oauth.json when suppressed."""
    marlow_home = tmp_path / "marlow"
    marlow_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("MARLOW_HOME", str(marlow_home))

    import yaml
    (marlow_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (marlow_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["marlow_pkce"]},
    }))

    # Stub the readers so only marlow_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_marlow_oauth_credentials", lambda: {
    ...
    # marlow_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "marlow_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (marlow_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "marlow_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `marlow auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `marlow auth add openai-codex`)
