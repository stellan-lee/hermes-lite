...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("marlow_cli.config.get_managed_system", return_value=None), \
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``marlow update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("marlow_cli.config.get_managed_system", return_value=None), \
         patch("marlow_cli.config.get_marlow_home", return_value=tmp_path), \
         patch("marlow_constants.is_container", return_value=True):
        from marlow_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("marlow_cli.config.get_marlow_home", return_value=hh), \
         patch("marlow_constants.get_marlow_home", return_value=hh):
        buf = io.StringIO()
