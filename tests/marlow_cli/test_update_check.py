"""Tests for the update check mechanism in marlow_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "marlow-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("marlow_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    """
    import marlow_cli.banner as banner

    # No local git checkout -> the PyPI path is exercised (pip-install class).
    fake_banner = tmp_path / "marlow_cli" / "banner.py"
    ...
    monkeypatch.setenv("MARLOW_HOME", str(tmp_path))
    monkeypatch.delenv("MARLOW_REVISION", raising=False)
    with patch("marlow_cli.banner.subprocess.run") as mock_run, \
         patch("marlow_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MARLOW_HOME", str(tmp_path))
    with patch("marlow_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MARLOW_HOME", str(tmp_path))
    with patch("marlow_cli.banner.subprocess.run") as mock_run:
        with patch("marlow_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when MARLOW_HOME has no git repo."""
    import marlow_cli.banner as banner
    ...
    # Point MARLOW_HOME at a temp dir with no marlow-agent/.git
    monkeypatch.setenv("MARLOW_HOME", str(tmp_path))
    with patch("marlow_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from marlow_cli.main import _invalidate_update_cache

    # Build a fake ~/.marlow with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"MARLOW_HOME": str(default_home)}):
