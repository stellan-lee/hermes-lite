...
@pytest.fixture
def main_mod():
    import marlow_cli.main as m

    return m


def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@marlow" / "ink" / "package.json"
...
def test_make_tui_argv_skips_build_when_bundle_is_current(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("a current bundle must not be rebuilt")
        ),
    )
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (marlow-ink etc.)
