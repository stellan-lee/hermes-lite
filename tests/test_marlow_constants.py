"""Tests for marlow_constants module."""
...
class TestGetDefaultMarlowRoot:
    """Tests for get_default_marlow_root() — Docker/custom deployment awareness."""

    def test_no_marlow_home_returns_native(self, tmp_path, monkeypatch):
        """When MARLOW_HOME is not set, returns ~/.marlow."""
        monkeypatch.delenv("MARLOW_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_marlow_root() == tmp_path / ".marlow"

    def test_marlow_home_is_native(self, tmp_path, monkeypatch):
        """When MARLOW_HOME = ~/.marlow, returns ~/.marlow."""
        native = tmp_path / ".marlow"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MARLOW_HOME", str(native))
        assert get_default_marlow_root() == native

    def test_marlow_home_is_profile(self, tmp_path, monkeypatch):
        """When MARLOW_HOME is a profile under ~/.marlow, returns ~/.marlow."""
        native = tmp_path / ".marlow"
    ...
    def test_marlow_home_is_docker(self, tmp_path, monkeypatch):
        """When MARLOW_HOME points outside ~/.marlow (Docker), returns MARLOW_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MARLOW_HOME", str(docker_home))
        assert get_default_marlow_root() == docker_home

    def test_marlow_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any MARLOW_HOME outside ~/.marlow is treated as the root."""
        custom = tmp_path / "my-marlow-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MARLOW_HOME", str(custom))
        assert get_default_marlow_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (MARLOW_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MARLOW_HOME", str(profile))
        assert get_default_marlow_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(marlow_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(marlow_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".marlow"
