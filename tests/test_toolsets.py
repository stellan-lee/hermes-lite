...
class TestToolsetConsistency:
    ...
    def test_marlow_platforms_share_core_tools(self):
        """All marlow-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on marlow-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["marlow-cli", "marlow-telegram", "marlow-discord", "marlow-whatsapp", "marlow-slack", "marlow-signal", "marlow-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_marlow_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("marlow-whatsapp")

    def test_marlow_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("marlow-api-server")
