...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    marlow_state = ModuleType("marlow_state")
    marlow_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "marlow_state", marlow_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Marlow"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Marlow"
