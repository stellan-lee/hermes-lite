...
Run from the PR worktree:
    ...
    cd ~/.marlow/marlow-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.marlow/marlow-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.marlow/marlow-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a marlow-agent checkout"
)
...
# Isolated MARLOW_HOME for the config write.
