"""Tests for marlow_state.py — SessionDB SQLite CRUD, FTS5 search, export."""

import sqlite3
import time
import pytest

from marlow_state import SCHEMA_SQL, SessionDB
...
class TestSessionLifecycle:
    ...
    def test_db_initializes_without_fts5_module(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("marlow_state.sqlite3.connect", connect_without_fts)
    ...
    def test_fts_runtime_restores_triggers_after_no_fts_open(
        ...
        monkeypatch.setattr("marlow_state.sqlite3.connect", real_connect)
...
class TestDeleteAndExport:
    ...
    def test_export_import_preserves_compacted_and_active_history(self, db, tmp_path):
        db.create_session(session_id="compacted", source="cli")
        db.append_message("compacted", role="user", content="original question")
        db.append_message("compacted", role="assistant", content="original answer")
        db.archive_and_compact(
            "compacted",
            [{"role": "user", "content": "[compacted summary]"}],
        )

        exported = db.export_session("compacted")
        assert [m["active"] for m in exported["messages"]] == [0, 0, 1]
        assert [m["compacted"] for m in exported["messages"]] == [1, 1, 0]

        target = SessionDB(db_path=tmp_path / "compacted_target.db")
        try:
            result = target.import_sessions([exported])
            assert result["ok"] is True
            assert result["imported"] == 1
            assert [m["content"] for m in target.get_messages("compacted")] == [
                "[compacted summary]"
            ]
            all_messages = target.get_messages("compacted", include_inactive=True)
            assert [m["active"] for m in all_messages] == [0, 0, 1]
            assert [m["compacted"] for m in all_messages] == [1, 1, 0]
            assert target.get_session("compacted")["message_count"] == 1
        finally:
            target.close()
...
class TestSchemaInit:
    ...
    def test_schema_version(self, db):
        from marlow_state import SCHEMA_VERSION
    ...
    def test_topic_mode_schema_is_not_auto_migrated_on_open(self, tmp_path):
        ...
        The gateway must remain rollback-safe: simply upgrading Marlow and starting
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from marlow_state import SCHEMA_SQL
...
class TestConcurrentWriteSafety:
    ...
    def test_sqlite_timeout_is_at_least_30s(self, db):
        ...
        import inspect
        from marlow_state import SessionDB as _SessionDB
