...
def fake_home(tmp_path, monkeypatch):
    """Point ``_marlow_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "marlow_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_marlow_home_path", lambda: home)
    return home
...
def test_arbitrary_marlow_home_file_not_blocked(fake_home):
    """Non-credential files inside MARLOW_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into MARLOW_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "marlow_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at MARLOW_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == MARLOW_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at MARLOW_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in MARLOW_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_marlow_files_outside_home_not_blocked(
    ...
    project.mkdir()
    # auth.json outside MARLOW_HOME — readable (per-location gate).
    p = project / "auth.json"
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, MARLOW_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_marlow_home_path", lambda: profile)
