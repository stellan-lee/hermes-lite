...
class TestRunTurn:
    ...
    def test_deadline_exceeded_records_error(self):
        ...
        s = make_session(client)
        r = s.run_turn("never finishes", turn_timeout=0.05, notification_poll_timeout=0.01)
        assert r.interrupted is True
...
class TestServerRequestRouting:
    ...
    def test_exec_approval_no_callback_denies(self):
        client = FakeClient()
        client.queue_server_request(
            "item/commandExecution/requestApproval", request_id="req-1", command="rm -rf /", cwd="/"
        )
        client.queue_notification(
            "turn/completed",
            threadId="t",
    ...
    def test_unknown_server_request_replied_with_error(self):
        ...
        s.run_turn("hi", turn_timeout=1.0)
        assert any(
            rid == "req-3" and code == -32601 for (rid, code, _msg) in client.error_responses
        )

    def test_routing_auto_approve_bypass(self):
        client = FakeClient()
        client.queue_server_request(
            "item/commandExecution/requestApproval", request_id="r1", command="ls", cwd="/"
        )
        client.queue_notification(
            "turn/completed",
            threadId="t",
