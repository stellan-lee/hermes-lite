...
COMMAND_EXEC_COMPLETED = {
    ...
    "params": {
        "item": {
            ...
            "source": "userShell",
            "status": "completed",
            "commandActions": [{"type": "listFiles", "command": "ls /tmp", "path": "tmp"}],
...
class TestProjectionInvariants:
    ...
    def test_streaming_deltas_dont_materialize(self) -> None:
        ...
        for delta_method in (
            ...
            assert r.messages == [], (
                f"{delta_method} should NOT produce messages — only item/completed materializes"
            )
        ...
        p.project(
            {
                "method": "item/completed",
                "params": {
                    "item": {"type": "reasoning", "id": "r1", "summary": ["once"], "content": []}
                },
            }
        )
        first = p.project(
...
class TestFileChangeProjection:
    def test_file_change_summary_no_inlined_content(self) -> None:
        ...
        p = CodexEventProjector()
        msgs = p.project({"method": "item/completed", "params": {"item": item}}).messages
        assert len(msgs) == 2
