...
engine plugins (e.g. marlow-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``marlow plugins`` picker."""
    from marlow_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "marlow_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "marlow_cli.plugins.get_plugin_context_engine",
