...
def test_format_responses_error_falls_back_to_status_when_empty():
    assert _format_responses_error(None, "failed") == "Responses API returned status 'failed'"
    assert _format_responses_error(None, "cancelled") == "Responses API returned status 'cancelled'"
