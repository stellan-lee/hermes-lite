...
to a generic "Your API key was rejected... run marlow setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give the exact re-auth command, not a generic "marlow setup".
    assert "marlow auth add nous --type oauth" in source
