...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate MARLOW_HOME under tmp_path."""
    monkeypatch.setenv("MARLOW_HOME", str(tmp_path / ".marlow"))
    (tmp_path / ".marlow").mkdir()

    # Force the constants/image cache helpers to re-read MARLOW_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("marlow_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_marlow_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under MARLOW_HOME — gateway cleanup
