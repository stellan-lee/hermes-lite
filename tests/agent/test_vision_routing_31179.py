...
def isolated_home(monkeypatch):
    """Temp MARLOW_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="marlow_test_31179_")
    marlow_home = os.path.join(test_home, ".marlow")
    os.makedirs(marlow_home)
...
def _fresh_modules():
    """Drop cached marlow modules so each test reloads against current env."""
