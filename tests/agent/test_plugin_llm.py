...
""",
        ...
        from marlow_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        marlow_home = tmp_path / ".marlow"
        marlow_home.mkdir()
        (marlow_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("MARLOW_HOME", str(marlow_home))
        from marlow_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from marlow_cli.plugins import PluginContext, PluginManifest, PluginManager
