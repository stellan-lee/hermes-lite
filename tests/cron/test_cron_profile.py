...
MARLOW_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated Marlow root with a named profile and temp cron store."""
    root = tmp_path / "marlow-root"
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "marlow profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets marlow_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("MARLOW_HOME")
                observed["profile_env_only_during_init"] = os.environ.get(
                    "MARLOW_PROFILE_TEST_ONLY"
                )
                observed["profile_env_shared_during_init"] = os.environ.get(
                    "MARLOW_PROFILE_TEST_SHARED"
                )
                observed["marlow_home_during_init"] = str(get_marlow_home())
                observed["scheduler_home_during_init"] = str(sched._get_marlow_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from marlow_constants import get_marlow_home

                observed["env_home_during_run"] = os.environ.get("MARLOW_HOME")
                ...
                observed["marlow_home_during_run"] = str(get_marlow_home())
                observed["scheduler_home_during_run"] = str(sched._get_marlow_home())
        ...
        from marlow_cli import runtime_provider as runtime_provider
        ...
        monkeypatch.setattr(sched, "_marlow_home", None)
        monkeypatch.setenv("MARLOW_CRON_TIMEOUT", "0")
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["marlow_home_during_init"] == str(profile_home.resolve())
        assert observed["marlow_home_during_run"] == str(profile_home.resolve())
        ...
        assert os.environ["MARLOW_HOME"] == str(root)
        assert sched._get_marlow_home() == root
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("MARLOW_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("MARLOW_PROFILE_TEST_ONLY", raising=False)

        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["MARLOW_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["MARLOW_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["MARLOW_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["MARLOW_PROFILE_TEST_SHARED"] == "outer"
        assert "MARLOW_PROFILE_TEST_ONLY" not in os.environ
        assert os.environ["MARLOW_CRON_TIMEOUT"] == "0"
        assert os.environ["MARLOW_HOME"] == str(root)
        assert sched._get_marlow_home() == root
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('MARLOW_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_marlow_home", None)
    ...
    def test_run_job_without_profile_leaves_marlow_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["marlow_home_during_init"] == str(root)
        assert os.environ["MARLOW_HOME"] == str(root)
