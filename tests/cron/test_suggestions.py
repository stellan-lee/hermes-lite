...
class TestStore:
    ...
    def test_concurrent_accept_creates_exactly_one_job(self, store):
        _add(store, key="race", title="Race Job")
        ...
        def fake_create_job(**kwargs):
            calls.append(kwargs)
            return {"id": "job-race", **kwargs}

        def accept():
            start.wait(timeout=5)
            results.append(store.accept_suggestion("Race Job"))
    ...
    def test_cross_process_claim_has_exactly_one_winner(self, store):
        _add(store, key="process-race", title="Race Job")
        ctx = multiprocessing.get_context("spawn")
        barrier = ctx.Barrier(2)
        results = ctx.Queue()
        home = str(store.CRON_DIR.parent)
        processes = [
            ctx.Process(
                target=_claim_suggestion_process,
                args=(home, barrier, results),
            )
            for _ in range(2)
        ]
        ...
        claimed = store.get_suggestion("Race Job")
        assert claimed["status"] == "accepting"

    def test_failed_accept_rolls_claim_back_to_pending(self, store):
        _add(store, key="retry", title="Retry Job")
        with patch("cron.jobs.create_job", side_effect=OSError("disk full")):
            with pytest.raises(OSError, match="disk full"):
                store.accept_suggestion("Retry Job")
        assert [s["title"] for s in store.list_pending()] == ["Retry Job"]
