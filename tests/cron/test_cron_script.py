...
- Path resolution (absolute, relative to MARLOW_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp MARLOW_HOME."""
    marlow_home = tmp_path / ".marlow"
    marlow_home.mkdir()
    (marlow_home / "cron").mkdir()
    (marlow_home / "cron" / "output").mkdir()
    (marlow_home / "scripts").mkdir()
    monkeypatch.setenv("MARLOW_HOME", str(marlow_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "MARLOW_DIR", marlow_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", marlow_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", marlow_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", marlow_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.marlow/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("MARLOW_SESSION_PLATFORM") is None
        assert os.environ.get("MARLOW_SESSION_CHAT_ID") is None
        assert os.environ.get("MARLOW_SESSION_CHAT_NAME") is None
