...
the shared /blueprint command handler (marlow_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated MARLOW_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    def test_form_schema_fields(self):
        ...
        deliver = next(field for field in schema["fields"] if field["name"] == "deliver")
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("marlow://blueprint/morning-brief?")
    ...
    def test_catalog_entry_has_all_surfaces(self):
        ...
        assert entry["appUrl"].startswith("marlow://")
...
class TestCommandHandler:
    ...
    def test_name_match_is_forgiving(self, isolated_home):
        from marlow_cli.blueprint_cmd import handle_blueprint_command, match_blueprint
