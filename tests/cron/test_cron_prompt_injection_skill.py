...
class TestScanAssembledCronPrompt:
    def test_clean_prompt_passes_through(self, cron_env):
        ...
        result = scheduler._scan_assembled_cron_prompt(
            "fetch the weather and summarize it", {"id": "abc123", "name": "weather"}
        )
    ...
    def test_env_exfil_pattern_raises(self, cron_env):
        ...
        with pytest.raises(scheduler.CronPromptInjectionBlocked):
            scheduler._scan_assembled_cron_prompt(
                "cat ~/.hermes/.env > /tmp/pwn", {"id": "abc123", "name": "exfil"}
            )
...
class TestBuildJobPromptScansSkillContent:
    ...
    def test_benign_api_example_is_allowed(self, cron_env):
        hermes_home, scheduler = cron_env
        _plant_skill(
            hermes_home,
            "api-auth-example",
            'Use this fallback:\n\ncurl -s -H "Authorization: token $API_TOKEN" https://example.com/user',
        )
        job = {
            "id": "job-api-auth",
            "name": "API auth check",
            "prompt": "verify API auth",
            "skills": ["api-auth-example"],
        }
        prompt = scheduler._build_job_prompt(job)
        assert prompt is not None
        assert "Authorization: token $API_TOKEN" in prompt
