"""Tests for marlow_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Marlow entry point
(marlow, marlow-agent, marlow-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Marlow entry point imports marlow_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported marlow_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("marlow_bootstrap", None)
    import marlow_bootstrap  # noqa: WPS433
    return marlow_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Marlow CLI
...
class TestEntryPointsImportBootstrap:
    """Every Marlow entry point must import marlow_bootstrap as its
    ...
    # Entry points that invoke Marlow as a process.  Each one must
    # import marlow_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "marlow_cli/main.py",   # marlow CLI (console_script)
        "run_agent.py",          # marlow-agent (console_script)
        "acp_adapter/entry.py",  # marlow-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import marlow_bootstrap' and that
        ...
        half-finished ``marlow update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``marlow_bootstrap`` as a top-level module) leaves marlow
        ...
        """
        # Resolve relative to the marlow-agent repo root.  Tests live
        # at tests/test_marlow_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # marlow start even when marlow_bootstrap hasn't been
        ...
        assert first_import_name == "marlow_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'marlow_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import marlow_bootstrap' line to be the first import."
        )
