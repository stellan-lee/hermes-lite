...
(SQLITE_PROTOCOL — typical on NFS/SMB), Marlow must fall back to
``journal_mode=DELETE`` so SQLite-backed state remains usable.

Without this fallback, users on NFS-mounted ``MARLOW_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    marlow_state._set_last_init_error(None)
    yield
    marlow_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    marlow_state._wal_fallback_warned_paths.clear()
    yield
    marlow_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_falls_back_to_delete_on_locking_protocol(self, tmp_path, caplog):
        ...
        with caplog.at_level("WARNING", logger="marlow_state"):
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        marlow_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("marlow_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        marlow_state._set_last_init_error("OperationalError: generic SQLite error")
