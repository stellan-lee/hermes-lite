"""
Tests for mcp_serve — Marlow MCP server.
...
def _isolate_marlow_home(tmp_path, monkeypatch):
    """Redirect MARLOW_HOME to a temp directory."""
    monkeypatch.setenv("MARLOW_HOME", str(tmp_path))
    try:
        import marlow_constants
        monkeypatch.setattr(marlow_constants, "get_marlow_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking marlow_state schema."""
