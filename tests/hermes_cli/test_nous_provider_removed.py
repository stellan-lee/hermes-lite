...
Note: the low-level Nous *credential* subsystem (``hermes auth add nous``,
the Portal subscription / Tool Gateway, dashboard auth, ``hermes proxy``
...
def test_no_cli_overlay_or_label():
    from hermes_cli.providers import HERMES_OVERLAYS, _LABEL_OVERRIDES, get_provider

    assert "nous" not in HERMES_OVERLAYS
...
def test_not_in_bundled_model_catalog():
    ...
    catalog = json.loads(
        (repo_root / "hermes_cli" / "data" / "model-catalog.json").read_text()
    )
