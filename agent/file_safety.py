...
def _marlow_home_path() -> Path:
    """Resolve the active MARLOW_HOME (profile-aware) without circular imports."""
    try:
        from marlow_constants import get_marlow_home  # local import to avoid cycles
        return get_marlow_home()
    except Exception:
        return Path(os.path.expanduser("~/.marlow"))


def _marlow_root_path() -> Path:
    """Resolve the Marlow root dir (always the parent of any profile, never per-profile)."""
    try:
        from marlow_constants import get_default_marlow_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    marlow_home = _marlow_home_path()
    marlow_root = _marlow_root_path()
    return {
        os.path.realpath(p)
        for p in [
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved MARLOW_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("MARLOW_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Marlow control-plane files: block both the ACTIVE profile's view
    # (marlow_home) AND the global root view. Without the root pass, a
    ...
    marlow_dirs = []
    for base in (_marlow_home_path(), _marlow_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in marlow_dirs:
                marlow_dirs.append(real)
        except Exception:
            continue

    for base_real in marlow_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.marlow/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active MARLOW_HOME (profile-aware) AND the global
    # Marlow root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (MARLOW_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    marlow_dirs: list[Path] = []
    for base in (_marlow_home_path(), _marlow_root_path()):
        try:
            real = base.resolve()
            if real not in marlow_dirs:
    ...
    for hd in marlow_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Marlow credential store "
    ...
    for hd in marlow_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Marlow MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Marlow MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Marlow profiles are separate MARLOW_HOME dirs under
...
#
# Reference: May 2026 incident where a marlow-security profile session
# edited skills under both ``~/.marlow/profiles/marlow-security/skills/``
# AND ``~/.marlow/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under MARLOW_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from MARLOW_HOME.
    ...
    """
    try:
        home_real = _marlow_home_path().resolve()
        root_real = _marlow_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Marlow scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Marlow entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Marlow profile {info['target_profile']!r}, but the "
...
# ``…/sandboxes/<backend>/<task>/home/.marlow/…`` segment and warns
# regardless of which Marlow profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.marlow/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.marlow`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.marlow/…`` and returns the
    index where the inner Marlow-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .marlow / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".marlow":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Marlow state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.marlow``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.marlow`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Marlow resolver to
    succeed, so it works correctly even when called from contexts where
    MARLOW_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
# full ``…/sandboxes/<backend>/<task>/home/.marlow/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.marlow/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host MARLOW_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Marlow state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Marlow "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real MARLOW_HOME. Use the "
