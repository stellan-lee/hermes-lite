...
as ``kind: backend``) or ``~/.marlow/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``marlow tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$MARLOW_HOME/cache/videos/``, creating parents as needed."""
    from marlow_constants import get_marlow_home

    path = get_marlow_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$MARLOW_HOME/cache/videos/``.
