...
upstream prefix cache warm.  See ``marlow-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Marlow never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want MARLOW_HOME persona while keeping
    ...
    # Pointer to the marlow-agent skill + docs for user questions about Marlow itself.
    stable_parts.append(MARLOW_AGENT_HELP_GUIDANCE)
    ...
    # Active-profile hint — names the Marlow profile the agent is running
    # under so it doesn't conflate ~/.marlow/skills/ (default profile) with
    # ~/.marlow/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Marlow profile: default. Other profiles (if any) live "
            "under ~/.marlow/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Marlow profile: {active_profile}. This session reads "
            f"and writes ~/.marlow/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.marlow/skills/, ~/.marlow/plugins/, "
            f"~/.marlow/cron/, ~/.marlow/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Marlow never rebuilds or reinjects parts of it
