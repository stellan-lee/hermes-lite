...
Storage
-------
Bundles live in ``~/.marlow/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``MARLOW_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``marlow bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under MARLOW_HOME.

    Honors ``MARLOW_BUNDLES_DIR`` for tests; falls back to
    ``<MARLOW_HOME>/skill-bundles``.
    """
    override = os.environ.get("MARLOW_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_marlow_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `marlow bundles` CLI subcommand.
# ---------------------------------------------------------------------------
