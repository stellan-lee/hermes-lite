...
Pure format-conversion and normalization logic for the OpenAI Codex Responses
API.
...
def _classify_responses_issuer(
    *,
    ...
    Stamping the issuer on persisted reasoning items and filtering at replay
    time prevents an encrypted reasoning block from being sent to a different
    endpoint that cannot decrypt it.
    """
    if is_codex_backend:
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    endpoint that minted it. When this argument is provided and a reasoning
    item carries an ``_issuer_kind`` stamp from a different endpoint, the item
    is dropped from the replayed input. Legacy items without a stamp are still
    replayed (backwards-compatible). The two guards compose:
    ...
    for msg in messages:
        ...
        if role in {"user", "assistant"}:
            ...
            if role == "assistant":
                ...
                codex_reasoning = (
                    msg.get("codex_reasoning_items") if replay_encrypted_reasoning else None
                )
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            replay_item = {
                                k: v for k, v in ri.items() if k not in ("id", "_issuer_kind")
                            }
    ...
    for idx, item in enumerate(raw_items):
        ...
        if item_type == "message":
            ...
            for part_idx, part in enumerate(content):
                if not isinstance(part, dict):
                    raise ValueError(
                        f"Codex Responses input[{idx}] message content"
                        f"[{part_idx}] must be an object."
                    )
                part_type = part.get("type")
                if part_type not in {"output_text", "text"}:
                    raise ValueError(
                        f"Codex Responses input[{idx}] message content"
                        f"[{part_idx}] has unsupported type {part_type!r}."
                    )
                text = part.get("text", "")
                if text is None:
                    text = ""
            ...
            if not normalized_content:
                raise ValueError(
                    f"Codex Responses input[{idx}] message item must contain "
        ...
        if role in {"user", "assistant"}:
            ...
            if isinstance(content, list):
                ...
                for part_idx, part in enumerate(content):
                    ...
                    if not isinstance(part, dict):
                        raise ValueError(
                            f"Codex Responses input[{idx}].content[{part_idx}] "
                    ...
                    if ptype in {"input_text", "text", "output_text"}:
                        ...
                        raise ValueError(
                            f"Codex Responses input[{idx}].content[{part_idx}] "
                            f"has unsupported type {part.get('type')!r}."
                        )
        ...
        raise ValueError(
            f"Codex Responses input[{idx}] has unsupported item shape "
            f"(type={item_type!r}, role={role!r})."
        )
...
def _preflight_codex_api_kwargs(
    ...
    *,
    ...
    if extra_body is not None:
        ...
        # The OpenAI SDK serializes extra_body into the JSON body without
...
def _normalize_codex_response(
    ...
    *,
    ...
    response_status = response_status.strip().lower() if isinstance(response_status, str) else None
    ...
    for item in output:
        ...
        item_status = item_status.strip().lower() if isinstance(item_status, str) else None
    ...
    if tool_calls:
        ...
        or (saw_commentary_phase and not saw_final_answer_phase)
    ):
