...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Marlow with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.marlow/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Marlow
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``marlow tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``marlow setup`` for
