...
External providers such as Honcho are registered
...
class MemoryProvider(ABC):
    ...
    @abstractmethod
    def name(self) -> str:
        """Short identifier for this provider (e.g. 'builtin' or 'honcho')."""
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - marlow_home (str): The active MARLOW_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.marlow``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "marlow").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'marlow memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], marlow_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'marlow memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``marlow_home`` is the active MARLOW_HOME directory path.
