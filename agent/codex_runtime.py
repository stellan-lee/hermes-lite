...
class _StreamErrorEvent(Exception):
    """Structured error synthesized from a Codex Responses SSE error frame."""
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # CLI thread has installed one. Other contexts fail closed.
        try:
...
def _raise_stream_error(event: Any) -> None:
    ...
    The exception carries the SDK-shaped fields used by callers that classify
    Codex request failures.
    """
