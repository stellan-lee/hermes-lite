...
``auth.json``.  They intentionally have no dependency on ``marlow_cli.auth`` so
...
# Sources Marlow owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Marlow-owned OAuth/device-code state)
