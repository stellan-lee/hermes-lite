...
Lifecycle:
    ...
    # result.tool_iterations     → how many tool-shaped items completed
...
_HERMES_TO_CODEX_PERMISSION_PROFILE = {
    ...
    # Backstop alias retained for older callers and tests.
    "yolo": "full-access",
}
...
class _ServerRequestRouting:
    ...
    callback is wired in. The live CLI path passes an approval callback."""
    ...
    def run_turn(
        ...
        *,
        ...
        try:
            ...
            result.error = self._format_error_with_stderr("codex app-server startup failed", exc)
        ...
        try:
            ...
            result.error = hint or self._format_error_with_stderr("turn/start timed out", exc)
            result.should_retire = True
            return result
        ...
        while time.monotonic() < deadline and not turn_complete:
            ...
            if method == "turn/completed":
                turn_complete = True
                turn_status = ((note.get("params") or {}).get("turn") or {}).get("status")
                if turn_status and turn_status not in {"completed", "interrupted"}:
                    err_obj = ((note.get("params") or {}).get("turn") or {}).get("error")
                    if err_obj:
