...
def _state_file() -> Path:
    return get_marlow_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.marlow/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``marlow update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``marlow curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `marlow curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Marlow' background skill CURATOR. This is an "
    ...
    "~/.marlow/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.marlow/logs/curator/``)
    ...
    authored skill data in ``~/.marlow/skills/``.

    ``ensure_marlow_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_marlow_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every marlow install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: marlow curator status
        keep an umbrella stable: marlow curator pin document-tools
    ...
    lines.append("full report: marlow curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: marlow curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.marlow/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.marlow/skills/.archive/` for "
            "safety and can be restored via `marlow curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.marlow/skills/.archive/`. "
            "Restore any via `marlow curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `marlow curator restore <name>`")
    lines.append("- All archives live under `~/.marlow/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `marlow curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `marlow curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
