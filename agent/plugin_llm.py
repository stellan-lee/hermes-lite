...
Plugins built on Marlow Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Marlow
...
:class:`~marlow_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Marlow' main config
...
handles every provider, fallback chain, and per-task override Marlow
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`marlow_cli.plugins.PluginContext`
