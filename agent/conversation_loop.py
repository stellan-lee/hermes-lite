        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            # Under concurrent load (cron/delegation), the initial
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: marlow auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `marlow doctor` for credential-chain diagnostics, or")
                    ...
                    from marlow_constants import display_marlow_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Marlow-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: marlow config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: marlow config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Marlow).  A 429 can
                ...
                # which Marlow' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Marlow' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`marlow",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `marlow model`.", force=True)
                            else:  # nous
                                ...
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: marlow auth add nous --type oauth", force=True)
