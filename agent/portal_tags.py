...
Every Marlow request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Marlow Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=marlow-agent",
        "client=marlow-client-v<__version__>",
    ]

The version is sourced live from ``marlow_cli.__version__`` so it auto-aligns
...
``marlow_cli.__version__`` is the canonical source of truth.
"""
...
def _marlow_version() -> str:
    """Return the current Marlow release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``marlow_cli`` cannot be imported (should
...
def marlow_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=marlow-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=marlow-client-v{_marlow_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=marlow-agent", marlow_client_tag()]
