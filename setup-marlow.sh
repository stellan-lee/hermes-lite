...
# 5. Symlinks the 'marlow' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Marlow Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/marlow-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink marlow into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up marlow command..."

MARLOW_BIN="$SCRIPT_DIR/venv/bin/marlow"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MARLOW_BIN" "$COMMAND_LINK_DIR/marlow"
echo -e "${GREEN}✓${NC} Symlinked marlow → $COMMAND_LINK_DISPLAY_DIR/marlow"
...
# ============================================================================
# Seed bundled skills into ~/.marlow/skills/
# ============================================================================

MARLOW_SKILLS_DIR="${MARLOW_HOME:-$HOME/.marlow}/skills"
mkdir -p "$MARLOW_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.marlow/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MARLOW_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  marlow status        # Check configuration"
if is_termux; then
    echo "  marlow gateway       # Run gateway in foreground"
else
    echo "  marlow gateway install # Install gateway service (messaging + cron)"
fi
echo "  marlow cron list     # View scheduled jobs"
echo "  marlow doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m marlow_cli.main setup
fi
