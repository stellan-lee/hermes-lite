"""Persistent slash-command worker — one MarlowCLI per TUI session.
...
def _run(cli: MarlowCLI, command: str) -> str:
...
def main():
    ...
    os.environ["MARLOW_SESSION_KEY"] = args.session_key
    os.environ["MARLOW_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = MarlowCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
