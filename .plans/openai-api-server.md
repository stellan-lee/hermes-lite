# OpenAI-Compatible API Server for Marlow Agent
...
SSE streaming. By exposing this endpoint, marlow-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  marlow-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.marlow/.env`
2. Run `marlow gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with marlow-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns marlow-agent as a model) |
...
Authorization: Bearer marlow-api-key-here
...
But marlow-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "marlow-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured marlow-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of marlow-agent's core prompt
...
Once implemented, marlow-agent works as a drop-in backend for:
