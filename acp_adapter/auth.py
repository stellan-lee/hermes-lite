"""ACP auth helpers — detect and advertise Marlow authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "marlow-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Marlow runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Marlow can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Marlow.
    ...
    not have Marlow provider credentials configured yet, so Marlow always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Marlow using the currently configured "
