"""ACP session manager — maps ACP sessions to Marlow AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.marlow/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Marlow itself is running in WSL.

    Windows ACP clients can launch ``marlow acp`` inside WSL while still sending
    ...
    """
    from marlow_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Marlow from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["marlow-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Marlow agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Marlow AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Marlow runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.marlow/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``MARLOW_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_marlow_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=marlow_home / "state.db")
            return self._db_instance
        except Exception:
