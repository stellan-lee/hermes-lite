"""CLI entry point for the marlow-agent ACP adapter.

Loads environment variables from ``~/.marlow/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from MARLOW_HOME (default ``~/.marlow``)."""
    from marlow_cli.env_loader import load_marlow_dotenv

    marlow_home = get_marlow_home()
    loaded = load_marlow_dotenv(marlow_home=marlow_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", marlow_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="marlow-acp",
        description="Run Marlow Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Marlow version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(marlow_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import MarlowACPAgent  # noqa: F401

    print("Marlow ACP check OK")


def _run_setup() -> None:
    from marlow_cli.main import main as marlow_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "marlow", "model"]
        marlow_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``marlow postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting marlow-agent ACP adapter")
    ...
    import acp
    from .server import MarlowACPAgent
